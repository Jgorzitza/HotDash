...
-- Enable Row Level Security on DashboardFact table
ALTER TABLE public."DashboardFact" ENABLE ROW LEVEL SECURITY;
...
COMMENT ON TABLE public."DashboardFact" IS 'Analytics facts with RLS enabled (project isolation). Immutable audit records.';
COMMENT ON COLUMN public."DashboardFact".project IS 'Project namespace for RLS isolation (e.g., occ, chatwoot).';

-- Grant necessary permissions
GRANT SELECT ON public."DashboardFact" TO authenticated;
GRANT SELECT ON public."DashboardFact" TO ai_readonly;
GRANT SELECT ON public."DashboardFact" TO analytics_reader;
GRANT INSERT ON public."DashboardFact" TO authenticated;
GRANT ALL ON public."DashboardFact" TO service_role;
