...
SELECT 
  EXTRACT(MONTH FROM dates.month_date) as month_num,
  TO_CHAR(dates.month_date, 'Month') as month_name,
  CASE 
    WHEN EXTRACT(MONTH FROM dates.month_date) BETWEEN 3 AND 9 THEN 'racing_season'
    ELSE 'off_season'
  END as season,
  pc.category_l1,
  COUNT(*) as record_count
FROM product_categories pc
CROSS JOIN generate_series(NOW() - INTERVAL '2 years', NOW(), INTERVAL '1 month') as dates(month_date)
