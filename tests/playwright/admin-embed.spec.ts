...
const hasAdminCredentials = Boolean(
  process.env.PLAYWRIGHT_SHOPIFY_EMAIL &&
    process.env.PLAYWRIGHT_SHOPIFY_PASSWORD,
);
...
test.describe("shopify admin embed", () => {
  ...
  test("loads operator control center in configured mode", async ({
    shopifyAdmin,
    page,
  }) => {
    ...
    if (shopifyAdmin.isMockMode) {
      ...
      if (foundIndicator) {
        ...
        console.log(
          "⚠️  Mock mode indicator not found (may not be implemented)",
        );
      }
    }
  ...
  test("handles authentication correctly per mode", async ({
    shopifyAdmin,
    page,
  }) => {
