...
test.describe("Dashboard to Approvals Happy Path", () => {
  ...
  test("should open approval drawer from queue", async ({ page }) => {
    await page.goto("/approvals");
    
    const firstApproval = page.getByText("Conversation #").first();
    await firstApproval.click();
    
    await expect(page.getByText("Evidence")).toBeVisible();
    await expect(page.getByText("Projected Impact")).toBeVisible();
    await expect(page.getByText("Risks & Rollback")).toBeVisible();
  });
});
