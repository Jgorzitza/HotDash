...
describe("Environment utilities", () => {
  beforeEach(() => {
    ...
    Object.keys(process.env).forEach(key => {
      if (key.startsWith('SHOPIFY_') || key === 'SCOPES' || key === 'NODE_ENV' || key === 'SHOP_CUSTOM_DOMAIN') {
  ...
  describe("getEnvironmentConfig", () => {
    it("returns valid configuration with all required env vars", async () => {
      const { getEnvironmentConfig } = await import("../../app/utils/env.server");
    ...
    it("throws error when SHOPIFY_API_KEY is missing", async () => {
      ...
      expect(() => getEnvironmentConfig()).toThrow("SHOPIFY_API_KEY environment variable is required");
    });
    ...
    it("throws error when SCOPES is missing", async () => {
      ...
      expect(() => getEnvironmentConfig()).toThrow("SCOPES environment variable is required");
    });
  ...
  describe("getAuthRedirectUrls", () => {
    it("generates correct redirect URLs with base URL", async () => {
      const { getAuthRedirectUrls } = await import("../../app/utils/env.server");
