...
describe("Shopify Inventory Cost Sync", () => {
  ...
  describe("syncInventoryCostToShopify", () => {
    it("should successfully update inventory cost", async () => {
      ...
      expect(result.previousCost).toBe(10.0);
      expect(result.newCost).toBe(15.5);
      expect(result.shopifyInventoryItemId).toBe(
    ...
    it("should handle inventory item not found", async () => {
      ...
      vi.mocked(mockContext.admin.graphql).mockResolvedValueOnce(
        getResponse as any,
      );
      ...
      expect(result.newCost).toBe(25.0);
    });
    ...
    it("should handle network errors", async () => {
      vi.mocked(mockContext.admin.graphql).mockRejectedValueOnce(
      ...
      expect(result.variantId).toBe("gid://shopify/ProductVariant/67890");
      expect(result.newCost).toBe(20.0);
    });
  });
  ...
  describe("syncMultipleInventoryCosts", () => {
    it("should successfully update multiple variants", async () => {
              ...
              data: {
                inventoryItemUpdate: {
                  inventoryItem: {
                    ...
                    updatedAt: "2025-10-21T16:01:00Z",
                  },
                  userErrors: [],
                },
              },
      ...
      const promise = syncMultipleInventoryCosts(mockContext, [
        { variantId: "gid://shopify/ProductVariant/1", newCost: 12.0 },
        { variantId: "gid://shopify/ProductVariant/2", newCost: 25.0 },
      ]);
      ...
      expect(results[0].newCost).toBe(12.0);
      expect(results[1].success).toBe(true);
      expect(results[1].newCost).toBe(25.0);
    });
  });
});
