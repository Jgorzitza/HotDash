...
const { parseHeartbeatSection, verifyHeartbeat } = require('../../scripts/ci/verify-heartbeat.cjs');
const fs = require('fs');
...
`;
      ...
      expect(() => parseHeartbeatSection(prBody)).toThrow(/Heartbeat section missing/);
...
`;
      ...
      expect(() => parseHeartbeatSection(prBody)).toThrow(/No heartbeat file path found/);
    });
...
`;
  ...
  describe('verifyHeartbeat', () => {
    ...
    test('TS-015: should throw error for invalid timestamp format', () => {
      ...
      fs.writeFileSync(testFile, '{"timestamp":"2025-10-21 14:30:00","task":"TEST","status":"doing"}\n');

      expect(() => verifyHeartbeat(testFile)).toThrow(/Invalid timestamp format/);
    });
    ...
    test('TS-016: should read last heartbeat from multiple entries', () => {
      ...
      const now = new Date();
      const entries = [
        { timestamp: new Date(now - 30 * 60 * 1000).toISOString(), task: 'TEST', status: 'doing', progress: '30%' },
        { timestamp: new Date(now - 20 * 60 * 1000).toISOString(), task: 'TEST', status: 'doing', progress: '60%' },
        { timestamp: now.toISOString(), task: 'TEST', status: 'doing', progress: '90%' }
      ];
