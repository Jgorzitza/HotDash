...
const { parsePRBody, validateJSONL } = require('../../scripts/ci/verify-mcp-evidence.cjs');
const fs = require('fs');
...
`;
      ...
      expect(() => parsePRBody(prBody)).toThrow(/No MCP evidence JSONL paths found/);
...
{"tool":"context7","doc_ref":"https://context7.com/docs","request_id":"124","timestamp":"2025-10-21T14:35:00Z","purpose":"Learn Prisma"}`;
    ...
    test('TS-011: should throw error if required fields missing', () => {
      ...
      // Missing 'tool' field
      fs.writeFileSync(testFile, '{"timestamp":"2025-10-21T14:30:00Z","purpose":"Test"}\n');
    ...
    test('TS-012: should throw error for invalid timestamp format', () => {
      ...
      fs.writeFileSync(testFile, '{"tool":"shopify-dev","timestamp":"2025-10-21 14:30:00","purpose":"Test"}\n');
  ...
  describe('Error Messages', () => {
    test('TS-015: should provide clear error message for missing section', () => {
      ...
      try {
        ...
        expect(error.message).toContain('artifacts/<agent>/<date>/mcp/<tool>.jsonl');
      }
    });
