...
describe('Telemetry Pipeline Integration Tests', () => {
  ...
  describe('GA4 Data Collection', () => {
    ...
    it('should process GA4 data for attribution', async () => {
      ...
      expect(attribution.total_revenue).toBe(26200);
  ...
  async function processGA4DataForAttribution(ga4Data: any) {
    const totalRevenue = ga4Data.events.reduce((sum: number, event: any) => {
      const revenue = event.total_revenue ?? event.revenue ?? 0;
      return sum + revenue;
    }, 0);

    const totalSessions = ga4Data.events.reduce((sum: number, event: any) => {
      const sessions = event.event_count ?? event.sessions ?? 0;
      return sum + sessions;
    }, 0);

    const totalPurchases = ga4Data.events.reduce((sum: number, event: any) => {
      if (event.event_name === 'purchase') {
        return sum + (event.event_count ?? event.purchases ?? 0);
      }

      if (typeof event.purchases === 'number') {
        return sum + event.purchases;
      }
