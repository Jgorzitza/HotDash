...
**Non-Critical**: 4. **Modal E2E** — Component implementation (engineer dependency) 5. **GA MCP** — OCC-INF-221 credentials (manager dependency)
...
- **Design blockers (2025-10-10 11:45 UTC):**
  ...
  2. Supabase decision log sync unresolved → live evidence IDs unavailable for screenshots; relying on mock mode and interim copy until reliability unlocks DSN/runbook.
  3. Need confirmation once engineering mirrors `modal-inventory-heatmap-annotations.svg` (and existing modal assets) into staging bundle before next deploy; request logged in `feedback/engineer.md:8`.
...
- ✓ Border tokens applied (--occ-border-_, --occ-radius-_)
