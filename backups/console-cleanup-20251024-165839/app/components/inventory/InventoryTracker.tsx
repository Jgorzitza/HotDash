...
export const InventoryTracker: React.FC<InventoryTrackerProps> = ({
  ...
  const handleInventoryUpdate = useCallback(async (variantId: string, newStock: number, reason: string) => {
    if (!socket || !isConnected) return;
  ...
  const handleAcknowledgeAlert = useCallback(async (alertId: string, userId: string) => {
    if (!socket || !isConnected) return;
