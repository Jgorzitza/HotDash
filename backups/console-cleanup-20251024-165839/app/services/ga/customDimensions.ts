 ...
 */
export async function createActionAttributionDimension(
  config: CustomDimensionConfig,
): Promise<CustomDimensionResult> {
  ...
  try {
    ...
    const result: CustomDimensionResult = {
      name: response.name || "",
    ...
    throw new Error(
      `Failed to create custom dimension: ${error.message}`,
    );
  }
}
...
export async function listCustomDimensions(
  ...
  try {
    ...
    throw new Error(
      `Failed to list custom dimensions: ${error.message}`,
    );
  }
}
 ...
 */
export async function validateActionAttributionDimension(
  propertyId: string,
): Promise<boolean> {
  try {
    const dimensions = await listCustomDimensions(propertyId);
    
    const actionDimension = dimensions.find(
      (dim) => dim.parameterName === "hd_action_key",
    );
 ...
 */
export function getActionAttributionConfig(): CustomDimensionConfig {
  return {
    parameterName: "hd_action_key",
    displayName: "Action Attribution Key",
 ...
 * @returns Promise<boolean>
 */
export async function testActionAttributionTracking(
  propertyId: string,
  testActionKey: string = "test-seo-fix-powder-board-2025-10-22",
): Promise<boolean> {
  try {
