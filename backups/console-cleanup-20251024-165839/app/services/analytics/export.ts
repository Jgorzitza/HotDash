...
export async function exportProductsToCSV(): Promise<string> {
  ...
  const headers =
    "Product Name,Product ID,Views,Add to Carts,Purchases,Revenue,Add to Cart Rate,Purchase Rate,Avg Price";
...
export async function exportUTMToCSV(): Promise<string> {
  ...
  const headers =
    "Source,Medium,Campaign,Sessions,Users,Conversions,Revenue,Conversion Rate";
  const rows = utm.map(
    (u) =>
