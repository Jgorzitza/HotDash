...
// Action handlers
async function handleBulkOperation(request: Request) {
...
async function handleResetMetrics(request: Request) {
  const body = await request.json();
...
async function handleResetCircuitBreaker(request: Request) {
  const body = await request.json();
  const { integrationName } = body;
