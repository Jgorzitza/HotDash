...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    const body: BingSetupRequest = await request.json();

    if (!body.domain || !body.sitemapUrl) {
      return Response.json(
        { success: false, error: "domain and sitemapUrl are required" },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      return Response.json(
        { success: false, error: "Invalid sitemap URL format" },
        { status: 400 }
      );
    }
    ...
    const response: BingSetupResponse = {
...
export async function loader() {
  return Response.json(
    { error: "Method not allowed. Use POST to setup Bing integration." },
    { status: 405 }
  );
}
