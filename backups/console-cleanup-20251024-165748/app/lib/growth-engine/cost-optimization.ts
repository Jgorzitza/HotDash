...
export class CostOptimizationEngine {
   ...
   * Check budget alerts
   */
  async checkBudgetAlerts(metrics: CostMetrics): Promise<void> {
  ...
  async generateOptimizationRecommendations(): Promise<OptimizationRecommendation[]> {
    ...
    if (this.costHistory.length < 7) return recommendations;
  ...
  async analyzeResourceUtilization(): Promise<ResourceUtilization[]> {
    ...
    if (this.costHistory.length === 0) return utilization;
   ...
   */
  getCostHistory(days: number = 30): CostMetrics[] {
