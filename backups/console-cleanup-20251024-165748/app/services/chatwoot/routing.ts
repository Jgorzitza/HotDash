...
export interface RoutingAction {
  ...
  target?: number | string; // Agent ID, tag name, etc.
 ...
 */
async function assignConversationAPI(
 ...
 */
export async function routeUnassignedConversations(): Promise<RoutingResult[]> {
  const config = getChatwootConfig();
  ...
  try {
    ...
    const results = await Promise.all(
      unassigned.map((conv) => routeConversation(conv, agents)),
    );
