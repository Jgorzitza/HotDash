...
export interface ROPEngineParams {
  ...
  promotionalUplift?: number; // Percentage uplift for promotions
  seasonalAdjustment?: number; // Percentage seasonal adjustment
  historicalDays?: number; // Days of historical data to analyze
}
 ...
 */
export async function calculateDailyVelocity(
  productId: string,
  variantId: string,
  ...
  const confidenceScore = Math.min(0.95, Math.max(0.5, 
    0.7 + (orderCount / 100) + (demandVariance < 2 ? 0.2 : 0)
  ));
 ...
 */
export async function calculateLeadTimeDemand(
  dailyVelocity: number,
  vendorId: string
): Promise<{
  leadTimeDays: number;
 ...
 */
export async function storeROPSuggestion(
  params: ROPEngineParams,
  result: ROPEngineResult
): Promise<string> {
 ...
 */
export async function calculateROPEngine(params: ROPEngineParams): Promise<ROPEngineResult> {
  const {
    productId,
    variantId,
    shopDomain,
  ...
  const safetyStock = calculateSafetyStock(
    ...
    leadTimeData.leadTimeDays,
    velocityData.demandVariance,
 ...
 */
export async function batchCalculateROP(
  productIds: string[],
  shopDomain: string,
  calculationMethod: 'standard' | 'seasonal' | 'promotional' | 'emergency' = 'standard'
): Promise<ROPEngineResult[]> {
 ...
 */
export async function getROPSuggestions(
  productId: string,
  shopDomain: string,
 ...
 */
export async function updateROPSuggestionStatus(
  suggestionId: string,
  status: 'pending' | 'approved' | 'rejected' | 'ordered' | 'cancelled',
  approvedBy?: string,
  notes?: string
