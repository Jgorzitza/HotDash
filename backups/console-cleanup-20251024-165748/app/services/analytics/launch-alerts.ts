...
export interface AlertThresholds {
  // User Engagement
  dauMauRatio: { warning: number; critical: number };
  signupTarget: { warning: number; critical: number };
  
  // Feature Adoption
  activationRate: { warning: number; critical: number };
  ttfvMedian: { warning: number; critical: number };
  
  // Satisfaction
  npsScore: { warning: number; critical: number };
  csatAverage: { warning: number; critical: number };
  sentimentPositive: { warning: number; critical: number };
  
  // Performance (to be added)
  loadTimeP95: { warning: number; critical: number };
...
export class LaunchAlertsService {
   ...
   */
  async checkMetrics(metrics: LaunchMetrics): Promise<LaunchAlert[]> {
  ...
  private checkUserEngagement(metrics: LaunchMetrics): LaunchAlert[] {
    const alerts: LaunchAlert[] = [];
  ...
  private checkFeatureAdoption(metrics: LaunchMetrics): LaunchAlert[] {
    const alerts: LaunchAlert[] = [];
  ...
  private checkSatisfaction(metrics: LaunchMetrics): LaunchAlert[] {
    const alerts: LaunchAlert[] = [];
  ...
  private async logAlerts(alerts: LaunchAlert[]): Promise<void> {
    await logDecision({
      scope: 'build',
      actor: 'analytics',
