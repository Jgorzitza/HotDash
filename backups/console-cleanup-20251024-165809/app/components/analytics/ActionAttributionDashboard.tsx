...
export function ActionAttributionDashboard({
  ...
  const [selectedTimeframe, setSelectedTimeframe] = useState<
    "7d" | "14d" | "28d"
  >("28d");
  const [sortBy, setSortBy] = useState<"revenue" | "conversion" | "roi">(
    "revenue",
  );
  ...
  return (
    <div style={{ padding: "var(--occ-space-4)" }}>
        ...
        >
          ...
          >
            <select
              ...
              onChange={(e) =>
                setSelectedTimeframe(e.target.value as "7d" | "14d" | "28d")
              }
              style={{
      ...
      {viewMode === "roi-tracking" && (
        <ROITrackingView
          attributionData={attributionData}
          timeframe={selectedTimeframe}
        />
      )}
      {viewMode === "attribution-analysis" && (
          ...
          return (
            ...
            >
              ...
              >
                <MetricCard
                  title="Sessions"
                  value={timeframeData.sessions.toLocaleString()}
                />
                <MetricCard
                  title="Conversions"
                  value={timeframeData.purchases.toLocaleString()}
                />
                <MetricCard
                  title="CVR"
                  value={`${timeframeData.conversionRate.toFixed(1)}%`}
                />
                <MetricCard
                  title="AOV"
                  value={`$${timeframeData.averageOrderValue.toFixed(0)}`}
                />
              </div>
            </div>
          );
...
function AttributionAnalysisView({
  ...
  const calculateAttributionEfficiency = (data: AttributionSummary) => {
    ...
    const efficiency7d =
      roi7d.sessions > 0 ? roi7d.revenue / roi7d.sessions : 0;
    const efficiency14d =
      roi14d.sessions > 0 ? roi14d.revenue / roi14d.sessions : 0;
    const efficiency28d =
      roi28d.sessions > 0 ? roi28d.revenue / roi28d.sessions : 0;
...
function RecommendationsView({
  ...
  const generateRecommendations = () => {
    ...
    if (sortedData.length > 0) {
      ...
      const bottomPerformer = getTimeframeData(
