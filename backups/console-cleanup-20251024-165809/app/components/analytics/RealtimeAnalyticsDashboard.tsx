  ...
  function handleSSEMessage(message: any) {
    switch (message.type) {
      ...
      case 'performance-alert':
        // Add new alert
        setAlerts(prev => [message.data as Alert, ...prev].slice(0, 10));
        break;
    ...
    <Page
      ...
      titleMetadata={
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          ...
          {alerts.length > 0 && (
            <LiveBadge count={alerts.length} label="Alerts" showPulse={true} />
          )}
        </div>
      }
    >
      <Layout>
        ...
        {alerts.length > 0 && (
          <Layout.Section>
            <Banner
              tone={alerts[0].type === 'critical' ? 'critical' : 'warning'}
              title={`${alerts.length} Active Alert${alerts.length > 1 ? 's' : ''}`}
              action={{ content: 'View All', onAction: () => setViewMode('alerts') }}
            >
...
function OverviewView({ metrics }: { metrics: RealtimeMetrics }) {
  return (
    <>
      {/* Growth Engine KPIs */}
      <Layout.Section>
          ...
          <div style={{ padding: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginTop: '16px' }}>
              <MetricCard
                ...
                value={metrics.growthEngine.activeActions}
                trend="neutral"
              />
              <MetricCard
                label="Pending Approvals"
                value={metrics.growthEngine.pendingApprovals}
                trend={metrics.growthEngine.pendingApprovals > 5 ? 'down' : 'neutral'}
              />
              <MetricCard
                label="Completed Today"
                value={metrics.growthEngine.completedToday}
                trend="up"
              />
              <MetricCard
                label="Avg ROI"
                value={`$${metrics.growthEngine.avgROI.toFixed(2)}`}
                trend={metrics.growthEngine.avgROI > 0 ? 'up' : 'down'}
              />
              <MetricCard
                label="Success Rate"
                value={`${(metrics.growthEngine.successRate * 100).toFixed(1)}%`}
                trend={metrics.growthEngine.successRate > 0.8 ? 'up' : 'down'}
              />
            </div>
          </div>
        </Card>
          ...
          <div style={{ padding: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginTop: '16px' }}>
              <MetricCard
                ...
                value={`${metrics.performance.responseTimeP95}ms`}
                trend={metrics.performance.responseTimeP95 < 3000 ? 'up' : 'down'}
              />
              <MetricCard
                label="Error Rate"
                value={`${(metrics.performance.errorRate * 100).toFixed(2)}%`}
                trend={metrics.performance.errorRate < 0.005 ? 'up' : 'down'}
              />
              <MetricCard
                label="Uptime"
                value={`${(metrics.performance.uptime * 100).toFixed(2)}%`}
                trend={metrics.performance.uptime > 0.999 ? 'up' : 'down'}
              />
              <MetricCard
                label="Throughput"
        ...
        <Card>
          <div style={{ padding: '16px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginTop: '16px' }}>
              <MetricCard
                label="Revenue"
                value={`$${metrics.kpis.revenue24h.toFixed(2)}`}
                trend="up"
              />
              <MetricCard
                label="Conversion Rate"
                value={`${(metrics.kpis.conversionRate * 100).toFixed(2)}%`}
                trend={metrics.kpis.conversionRate > 0.02 ? 'up' : 'down'}
              />
              <MetricCard
                label="Avg Order Value"
                value={`$${metrics.kpis.avgOrderValue.toFixed(2)}`}
                trend="neutral"
              />
              <MetricCard
                label="Sessions"
                value={metrics.kpis.sessions24h.toLocaleString()}
                trend="up"
              />
            </div>
          </div>
...
function DetailedView({ metrics }: { metrics: RealtimeMetrics }) {
  return (
    <Layout.Section>
      <Card>
        <div style={{ padding: '16px' }}>
...
function getHealthTone(status: string): 'success' | 'warning' | 'critical' {
  switch (status) {
