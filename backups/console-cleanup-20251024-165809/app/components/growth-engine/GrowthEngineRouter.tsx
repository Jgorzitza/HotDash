...
import { 
  ...
  growthEnginePerformanceOptimizer,
  type GrowthEngineRoute 
} from '~/services/growth-engine/core-infrastructure';
...
export function GrowthEngineRouter({ 
  children, 
  userPermissions, 
  currentPhase 
}: GrowthEngineRouterProps) {
  const navigate = useNavigate();
  const location = useLocation();
  
  const [currentRoute, setCurrentRoute] = useState<GrowthEngineRoute | null>(null);
  const [availableRoutes, setAvailableRoutes] = useState<GrowthEngineRoute[]>([]);
  ...
  const navigateToRoute = useCallback(async (routeId: string) => {
    setIsLoading(true);
    setError(null);
  ...
  const refreshRoute = useCallback(async () => {
    ...
    try {
      ...
      // Reload route data
      await growthEnginePerformanceOptimizer.optimizeRouteLoading(
        currentRoute.id, 
        userPermissions
      );
  ...
  useEffect(() => {
    const handleStateChange = (data: any) => {
      ...
      if (data.newState.currentPhase !== currentPhase) {
        ...
        const newRoutes = growthEngineRouter.getRoutesForPhase(
          data.newState.currentPhase, 
          userPermissions
        );
...
export function RouteGuard({ routeId, children, fallback }: RouteGuardProps) {
        ...
        <div style={{ marginBottom: 'var(--occ-space-2)' }}>
          ...
          <ul style={{ margin: 'var(--occ-space-1) 0', paddingLeft: 'var(--occ-space-4)' }}>
            {recommendations.map((rec, index) => (
              <li key={index} style={{ fontSize: 'var(--occ-font-size-xs)' }}>
                {rec}
              </li>
            ))}
          </ul>
