 ...
 * Get growth phases data
 */
async function getGrowthPhases(): Promise<GrowthPhase[]> {
 ...
 * Get growth actions data
 */
async function getGrowthActions(startDate: string, endDate: string): Promise<GrowthAction[]> {
  // Mock data - in production this would come from database
  return [
    {
      actionId: 'action-001',
 ...
 * Calculate overview metrics
 */
function calculateOverviewMetrics(phases: GrowthPhase[], actions: GrowthAction[]) {
 ...
 */
function analyzeActionPerformance(actions: GrowthAction[]) {
 ...
 */
function generateGrowthInsights(phases: GrowthPhase[], actions: GrowthAction[]) {
  ...
  const actionEffectiveness = ['seo', 'ads', 'content', 'social', 'email'].map(actionType => {
    ...
    if (averageROI > 3) recommendations.push('Scale this channel');
    if (averageROI < 2) recommendations.push('Optimize or pause this channel');
    if (successRate < 50) recommendations.push('Review strategy and execution');
