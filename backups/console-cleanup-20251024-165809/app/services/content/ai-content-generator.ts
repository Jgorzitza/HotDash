...
export interface ProductDescriptionRequest {
  ...
  tone?: ContentTone;
  length?: 'short' | 'medium' | 'long'; // short: 50-100 words, medium: 100-200, long: 200-300
...
export class AIContentGenerator {
  ...
  async generateProductDescription(
    ...
    try {
      ...
      // Assess quality
      const qualityScore = await this.assessContentQuality(
        content,
        'product_description',
  ...
  async generateVariations(
    type: 'product_description' | 'blog_post',
    request: ProductDescriptionRequest | BlogPostRequest,
    count: number = 3
  ): Promise<GeneratedContent[]> {
    const variations: GeneratedContent[] = [];
    const tones: ContentTone[] = ['professional', 'casual', 'enthusiastic'];
    ...
    for (let i = 0; i < Math.min(count, 3); i++) {
      ...
      if (type === 'product_description') {
        const variation = await this.generateProductDescription(
          modifiedRequest as ProductDescriptionRequest
        );
        variations.push(variation);
      } else {
        const variation = await this.generateBlogPost(
          modifiedRequest as BlogPostRequest
        );
        variations.push(variation);
      }
    }
  ...
  async assessContentQuality(
    ...
    const seoAnalysis = await analyzeContent(
      'https://hotrodan.com/temp', // Placeholder URL
      content,
      keywords?.[0] || ''
    );
    ...
    const overall = Math.round(
      (seoAnalysis.overallScore.score * 0.3) +
      (engagementScore * 0.3) +
   ...
   */
  private buildProductDescriptionPrompt(request: ProductDescriptionRequest): string {
    const parts: string[] = [
   ...
   */
  private buildBlogPostPrompt(request: BlogPostRequest): string {
    const parts: string[] = [
...
${contentType === 'product_description' ? 
   ...
   */
  private calculateMetadata(content: string, tone: ContentTone) {
  ...
  private calculateEngagementScore(content: string): number {
    ...
    if (avgSentenceLength < 100) score += 5; // Shorter sentences are more engaging
   ...
   */
  private generateImprovementSuggestions(qualityScore: ContentQualityScore): string[] {
