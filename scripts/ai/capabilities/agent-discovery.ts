...
export const AGENT_CAPABILITIES: AgentCapability[] = [
  {
    ...
    limitations: ['Cannot answer questions directly', 'Cannot access order data'],
  },
  {
    agent: 'OrderSupport',
    capabilities: ['order_lookup', 'return_processing', 'refund_handling', 'tracking'],
  ...
  {
    agent: 'ProductQA',
    capabilities: ['product_info', 'availability_check', 'recommendations', 'specifications'],
...
export function discoverCapabilities(query: string): string[] {
  const capableAgents = AGENT_CAPABILITIES.filter(a =>
    a.capabilities.some(c => query.toLowerCase().includes(c.replace('_', ' ')))
  );
