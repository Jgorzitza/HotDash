...
export class QualityScorer {
  ...
  async scoreResponse(params: {
    ...
    const automated_scores = {
      bleu_score: this.calculateBLEU(params.agentResponse, params.referenceAnswer),
      rouge_l_score: this.calculateROUGEL(params.agentResponse, params.referenceAnswer),
      ...
      completeness_score: this.scoreCompleteness(params.customerQuestion, params.agentResponse),
    ...
    // Set flags
    const flags = {
      needs_human_review: overall_score < 0.70 || quality_tier === 'poor',
      policy_violation: automated_scores.policy_alignment_score < 0.60,
      low_quality: overall_score < 0.60,
      exemplar: overall_score >= 0.90 && quality_tier === 'excellent',
    };
  ...
  private calculateBLEU(response: string, reference?: string): number {
    ...
    const matches = responseWords.filter(w => referenceWords.includes(w)).length;
  ...
  private scoreTone(response: string): number {
    ...
    const positiveMarkers = [
      'i understand', 'i apologize', 'thank you', 'happy to help',
      'i can help', 'great question', 'absolutely', 'of course',
      'i appreciate', 'let me', 'i\'ll'
    ];
    ...
    const negativeMarkers = [
      'unfortunately you', 'you should have', 'as i said',
      'policy states', 'you must', 'you failed to'
    ];
    
    negativeMarkers.forEach(marker => {
      if (lowerResponse.includes(marker)) score -= 0.10;
    });
  ...
  private scorePolicyAlignment(response: string): number {
    ...
    const compliantTerms = [
      '30 days', 'prepaid label', 'free return shipping',
      'within 5-7 business days', 'original tags', 'unworn'
    ];
    ...
    const prohibitedPromises = [
      'guarantee', '100% refund', 'definitely', 'always',
      'never have problems', 'best price anywhere'
    ];
  ...
  private scoreCitations(response: string): number {
    ...
    const hasSources = response.includes('according to') ||
    ...
    const hasSpecificRef = response.includes('section') ||
  ...
  private scoreActionClarity(response: string): number {
    ...
    const actionIndicators = [
      'step 1', 'step 2', 'first', 'next', 'then',
      'you can', 'please', 'would you like', 'let me know'
    ];
   ...
   */
  private calculateOverallScore(scores: QualityScore['automated_scores']): number {
    const weights = {
      bleu_score: 0.15,
      rouge_l_score: 0.15,
      length_score: 0.10,
   ...
   * Determine quality tier
   */
  private determineQualityTier(score: number): 'excellent' | 'good' | 'fair' | 'poor' {
...
export const ALERT_RULES = [
  {
    ...
    condition: (dashboard: QualityDashboard) => 
      dashboard.metrics.by_tier.poor / dashboard.metrics.total_responses > 0.10,
