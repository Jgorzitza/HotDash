...
async function fetchSitemap(url) {
    try {
        ...
        return urlMatches
            ...
            .filter((url) => url !== null && url.startsWith('http') && !url.endsWith('.xml'))
            .slice(0, 50); // Limit to first 50 URLs to avoid overwhelming the system
    }
    catch (error) {
        console.warn(`Failed to fetch sitemap ${url}:`, error instanceof Error ? error.message : error);
        return [];
    }
}
export async function fetchHotrodanContent() {
    ...
    try {
        ...
        console.log(`Successfully fetched ${documents.length} documents from web sources`);
        ...
        if (sampleDoc) {
            ...
            await fs.writeFile(path.join(outputDir, 'sample.json'), JSON.stringify(sample, null, 2));
        }
        // Write manifest
        const manifest = {
            source: 'web',
        ...
        try {
            await fs.mkdir(outputDir, { recursive: true });
            await fs.writeFile(path.join(outputDir, 'errors.log'), JSON.stringify(errorLog, null, 2));
        }
        catch (writeError) {
