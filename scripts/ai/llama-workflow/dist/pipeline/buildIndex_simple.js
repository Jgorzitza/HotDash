...
export async function buildAll(logDir, options = {}) {
    ...
    try {
        ...
        const sources = ['hotrodan.com', 'decision_log', 'telemetry_events', 'curated_replies'];
        ...
        await fs.writeFile(path.join(runDir, 'index.json'), JSON.stringify({ mock: true, documents: mockDocumentCount }, null, 2));
        ...
        console.log(`📊 Processed ${mockDocumentCount} documents from ${sources.length} sources`);
        return {
            success: true,
            runDir,
        ...
        try {
            await fs.mkdir(runDir, { recursive: true });
            await fs.writeFile(path.join(runDir, 'error.log'), JSON.stringify(errorLog, null, 2));
        }
        catch (writeError) {
