...
function loadSupportDocuments(): Promise<DocumentRecord[]> {
...
function computeRelevanceScore(doc: DocumentRecord, query: string): number {
...
async function runQuery(query: string, topK: number): Promise<string> {
...
function runRefresh(forceMock: boolean, additionalArgs: string[]): number {
...
async function runInsight(window: string, format: 'md' | 'json' | 'txt'): Promise<string> {
  ...
  try {
    ...
    const metadata = JSON.parse(metadataRaw) as {
      ...
      sources: { fileName: string; size: number }[];
    };
...
program
  ...
  .action(async (options) => {
    const format = options.format as 'md' | 'json' | 'txt';
