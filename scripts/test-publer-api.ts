...
async function main() {
  ...
  try {
    ...
    if (workspacesResult.success && workspacesResult.data) {
      ...
      if (workspacesResult.rateLimitInfo) {
        console.log(
          `Rate Limit: ${workspacesResult.rateLimitInfo.remaining}/${workspacesResult.rateLimitInfo.limit} remaining`,
        );
      }
    } else {
      console.log("❌ FAILED");
  ...
  try {
    ...
    if (accountsResult.success && accountsResult.data) {
      ...
      accountsResult.data.forEach((acc, i) => {
        console.log(
          `  ${i + 1}. ${acc.platform || "Unknown"}: ${acc.name} (ID: ${acc.id})`,
        );
      ...
      if (accountsResult.rateLimitInfo) {
        console.log(
          `Rate Limit: ${accountsResult.rateLimitInfo.remaining}/${accountsResult.rateLimitInfo.limit} remaining`,
        );
      }
    } else {
      console.log("❌ FAILED");
    ...
    try {
      ...
      if (accountResult.success && accountResult.data) {
        ...
        if (accountResult.rateLimitInfo) {
          console.log(
            `Rate Limit: ${accountResult.rateLimitInfo.remaining}/${accountResult.rateLimitInfo.limit} remaining`,
          );
        }
      } else {
        console.log("❌ FAILED");
    ...
    try {
      ...
      if (scheduleResult.success && scheduleResult.data) {
        ...
        if (scheduleResult.rateLimitInfo) {
          console.log(
            `Rate Limit: ${scheduleResult.rateLimitInfo.remaining}/${scheduleResult.rateLimitInfo.limit} remaining`,
          );
        }
      } else {
        console.log("❌ FAILED");
    ...
    try {
      ...
      if (statusResult.success && statusResult.data) {
        ...
        if (statusResult.rateLimitInfo) {
          console.log(
            `Rate Limit: ${statusResult.rateLimitInfo.remaining}/${statusResult.rateLimitInfo.limit} remaining`,
          );
        }
      } else {
        console.log("❌ FAILED");
  ...
  console.log(
    "   This function works similarly to schedulePost() but publishes immediately",
  );
  ...
  console.log(
    testAccountId
      ? "✅ TEST 3: getAccount() - Completed"
      : "⚠️  TEST 3: getAccount() - Skipped (no account)",
  );
  console.log(
    jobId
      ? "✅ TEST 4: schedulePost() - Completed (DRAFT)"
      : "⚠️  TEST 4: schedulePost() - Skipped (no account)",
  );
  console.log(
    jobId
      ? "✅ TEST 5: getJobStatus() - Completed"
      : "⚠️  TEST 5: getJobStatus() - Skipped (no job)",
  );
