...
async function testSampleQuery(): Promise<ValidationResult> {
  ...
  try {
    ...
    if (eventCount > 0) {
      ...
      console.log(
        "     Engineers need to implement client tracking (ENG-032, 033)",
      );
      ...
      return {
        ...
        status: "WARN",
        message:
          "Query executed successfully but no events found (expected before client tracking)",
        details: {
