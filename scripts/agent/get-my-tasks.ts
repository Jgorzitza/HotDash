...
if (!agent) {
  console.error(
    "❌ Usage: npx tsx --env-file=.env scripts/agent/get-my-tasks.ts <agent-name>",
  );
  process.exit(1);
}
...
tasks.forEach((t, i) => {
  ...
  console.log(
    `   Acceptance: ${(t.acceptanceCriteria as string[]).length} criteria`,
  );
...
if (next) {
  ...
  console.log(`\n   To start this task:`);
  console.log(
    `   npx tsx --env-file=.env scripts/agent/start-task.ts ${next.taskId}`,
  );
} else {
