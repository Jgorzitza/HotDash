...
if (!taskId) {
  console.error('❌ Usage: npx tsx --env-file=.env scripts/agent/complete-task.ts <TASK-ID> <completion notes>');
  process.exit(1);
}

if (!completionNotes) {
  console.error('❌ Completion notes required (preserves context)');
  console.error('   Example: npx tsx scripts/agent/complete-task.ts ENG-029 "PII Card complete, 22/22 tests passing"');
  process.exit(1);
}
...
await logDecision({
  ...
  evidenceUrl: task.evidenceUrl || `artifacts/${task.assignedTo}/tasks/${taskId.toLowerCase()}.md`
});
