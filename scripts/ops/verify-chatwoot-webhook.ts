...
Options:
  ...
  if (args.length < 3) {
    console.error('Error: Required arguments: <payload-file> <signature> <secret>');
  ...
  try {
    const payload = fs.readFileSync(payloadFile, 'utf-8');
    const result = verifyWebhookSignature(payload, receivedSignature, webhookSecret);
    ...
    console.log(`${result.valid ? '✅' : '❌'} Verification Status: ${result.valid ? 'VALID' : 'INVALID'}\n`);
