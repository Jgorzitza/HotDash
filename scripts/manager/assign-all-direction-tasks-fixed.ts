...
async function assignAllDirectionTasksFixed() {
  console.log('🚨 ASSIGNING ALL DIRECTION TASKS - PROPER ERROR HANDLING');
  console.log('================================================================================');
  ...
  console.log(`⚠️  Already existed: ${alreadyExists} tasks`);
  ...
  if (failedTasks.length > 0) {
    ...
    failedTasks.forEach(failed => {
      console.log(`   • ${failed.taskId} (${failed.agent}): ${failed.error}`);
    });
  }
  ...
  // Log the decision with proper error handling
  try {
    await logDecision({
      scope: 'manager',
      actor: 'manager',
      action: 'all_direction_tasks_assigned_fixed',
      rationale: `Comprehensive fix with proper error handling: Processed ${directionFiles.length} direction files, assigned ${successfulAssignments} tasks, ${alreadyExists} already existed, ${failedAssignments} failed.`,
      payload: {
    ...
    console.error('❌ Failed to log decision:', logError.message);
  }
