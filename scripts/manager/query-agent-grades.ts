...
async function queryAgentGrades(agentName?: string) {
  ...
  if (shutdowns.length === 0) {
    ...
    console.log(
      "💡 Agents should include selfGrade and retrospective in shutdown payload.\n",
    );
    return;
  }
  ...
  shutdowns.forEach((entry, i) => {
    ...
    if (payload?.retrospective) {
      ...
      if (retro.toChange?.length > 0) {
        ...
        retro.toChange.forEach((item: string) =>
          console.log(`      - ${item}`),
        );
      }
      if (retro.toStop) {
  ...
  if (gradesWithAvg.length > 0) {
    const overallAvg =
      gradesWithAvg.reduce((sum, avg) => sum + avg, 0) / gradesWithAvg.length;
    console.log(`\n📈 Overall Performance:`);
    console.log(`   Average Self-Grade: ${overallAvg.toFixed(1)}/5.0`);
    console.log(
      `   Entries with grades: ${gradesWithAvg.length}/${shutdowns.length}`,
    );
  }
