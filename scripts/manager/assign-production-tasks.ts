...
const tasks = [
...

  {
    ...
    description:
      "Apply all pending migrations from supabase/migrations/ to create complete database schema for Growth Engine. Includes: approvals, vendors, inventory, SEO, ads, social, knowledge base, product suggestions.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create vendor master tables: vendors, vendor_skus, vendor_product_mappings. Implement reliability scoring logic. Set up RLS policies.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create bundle tables: bundles, bom_components. Implement virtual stock calculation (min of component availability). Nightly reconciliation job schema.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create receiving tables: purchase_orders, purchase_order_receipts, purchase_order_line_items, product_cost_history. Implement ALC recalculation logic with freight/duty allocation.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Wire ApprovalsDrawer.tsx to database: load approvals, update status, execute tool calls, store audit logs. Implement /validate endpoint.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create server-side tool handlers for Shopify mutations: product updates, inventory adjustments, order operations. All guarded by approvals system.",
    acceptanceCriteria: [
    ...
    allowedPaths: [
      ...
      "app/routes/api.approvals.execute.**",
    ],
    priority: "P0" as const,
    phase: "Phase 9",
    estimatedHours: 5,
...

  {
    ...
    description:
      "Implement Reorder Point calculation: ROP = LeadTimeDemand + SafetyStock. Use 30-90 day velocity, vendor lead times, 95% service level. Generate reorder suggestions.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Implement emergency sourcing recommendations: when OOS component blocks profitable bundle, recommend fast local vendor if Expected Lost Profit > Incremental Cost.",
    acceptanceCriteria: [
    ...
    allowedPaths: [
      "app/services/inventory/**",
      "app/routes/api.inventory.emergency.**",
    ],
    priority: "P1" as const,
    phase: "Phase 11",
    estimatedHours: 6,
...

  {
    ...
    description:
      "Create nightly job: enforce Canada available=0, recompute virtual bundle stock, sync Shopify inventory levels, alert on discrepancies.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Build Customer-Front agent using OpenAI Agents SDK: CX triage, transfer to Accounts/Storefront sub-agents, compose redacted reply with PII Card, HITL approval required.",
    acceptanceCriteria: [
    ...
    allowedPaths: [
      "packages/agents/**",
      "app/agents/config/**",
      "app/routes/api.agents.customer.**",
    ],
    priority: "P1" as const,
    phase: "Phase 9",
    estimatedHours: 12,
...

  {
    ...
    description:
      "Build Accounts sub-agent: order lookups, refund processing, return handling. Returns structured JSON to Customer-Front agent.",
    acceptanceCriteria: [
    ...
    allowedPaths: [
      "packages/agents/sub-agents/**",
      "app/services/shopify/orders/**",
    ],
    priority: "P1" as const,
    phase: "Phase 9",
    estimatedHours: 8,
...

  {
    ...
    description:
      "Build Storefront sub-agent: product search, inventory checks, collection browsing. Read-only Storefront MCP.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Build CEO-Front agent: business intelligence queries, read-only Storefront MCP + Action Queue, no writes, no Customer Accounts MCP.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      'Create GA4 custom dimension "hd_action_key" (event scope). Configure tracking for page_view, add_to_cart, purchase events. Document tracking implementation.',
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Build attribution panel: for each approved Action, show 7/14/28-day performance (conversions, revenue, ROAS). Update expected_impact based on observed results.",
    acceptanceCriteria: [
    ...
    allowedPaths: [
      "app/components/attribution/**",
      "app/routes/api.attribution.**",
      "app/services/ga/**",
    ],
    priority: "P1" as const,
    phase: "Phase 11",
...

  {
    ...
    description:
      "Build pipeline: sanitize CX conversations (remove PII), embed chunks to pgvector, enable semantic search for recurring themes.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Query embeddings for recurring themes (fit/missing/confusion). Generate mini-task Action cards with draft content: add size table, clarify policy, add adapter SKU.",
    acceptanceCriteria: [
    ...
    allowedPaths: [
      ...
      "app/routes/api.product.cx-loop.**",
    ],
    priority: "P2" as const,
    phase: "Phase 12",
    estimatedHours: 6,
...

  {
    ...
    description:
      "Integrate Publer for social posting: authenticate, post scheduling, platform selection (FB/IG/Twitter), post preview, receipt storage.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Add Bing Webmaster Tools verification and basic metrics: site verification, submit sitemap, fetch search performance data.",
    acceptanceCriteria: [
    ...
    allowedPaths: [
      "app/services/bing/**",
      "docs/integrations/bing-webmaster/**",
    ],
    priority: "P2" as const,
    phase: "Phase 12",
    estimatedHours: 3,
...

  {
    ...
    description:
      "Implement store switch from hotroddash.myshopify.com (dev) to fm8vte-ex.myshopify.com (prod/hotrodan.com). Environment-based configuration, OAuth redirects, telemetry IDs.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Implement CI guards: guard-mcp (validates MCP evidence JSONL), idle-guard (checks heartbeat freshness), dev-mcp-ban (fails if Dev MCP in prod bundles).",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Refine ApprovalCard and ApprovalsDrawer to match spec: evidence tabs (Diffs/Samples/Queries/Screenshots), tool calls preview, audit section, sticky footer.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create comprehensive E2E tests: approval workflows, agent interactions, inventory operations, CX flows, social posting, attribution tracking.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Complete accessibility audit: all tiles, modals, approvals drawer, forms. Zero violations. Screen reader tested.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create social post templates for common scenarios: new product launch, sale announcement, customer testimonial, inventory alert, tip/education.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Implement daily sync of Google Search Console data to seo_search_console_metrics table. Track: queries, pages, clicks, impressions, CTR, position over time.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create automated SEO audit system: check title tags, meta descriptions, alt text, structured data, page speed. Store in seo_audits table.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Implement daily sync of ad campaign data to ad_campaigns and ad_performance tables. Track ROAS, spend, conversions, CTR.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Create runbooks for all operational procedures: vendor onboarding, PO processing, receiving workflow, emergency sourcing, agent troubleshooting, deployment.",
    acceptanceCriteria: [
...

  {
    ...
    description:
      "Execute full UAT for all workflows: approvals, inventory, CX, growth, agents. Document findings, verify against acceptance criteria.",
    acceptanceCriteria: [
...
for (const task of tasks) {
  try {
    ...
    console.log(
      `✅ ${task.taskId}: ${task.assignedTo} - ${task.title.substring(0, 50)}`,
    );
    assigned++;
  } catch (error) {
    console.error(
