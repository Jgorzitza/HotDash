...
import {
  logDecision,
  calculateSelfGradeAverage,
} from "../../app/services/decisions.server";
...
async function testPayloadMetadata() {
  ...
  await logDecision({
    ...
    payload: {
      ...
      files: [
        {
          path: "app/components/TestComponent.tsx",
          lines: 245,
          type: "created",
        },
        {
          path: "app/components/TestComponent.test.tsx",
          lines: 180,
          type: "created",
        },
      ...
      technicalNotes:
        "Used Polaris Card as base component with custom redaction logic",
    },
  });
  ...
  await logDecision({
    ...
    payload: {
      ...
      tradeoffs:
        "Zod: better TypeScript integration, Yup: more mature ecosystem",
  ...
  await logDecision({
    ...
    progressPct: 65,
    rationale:
      "Daily shutdown - TEST-001 complete, TEST-002 blocked, TEST-003 awaiting decision",
    durationActual: 5.5,
    payload: {
      dailySummary:
        "TEST-001 complete (15/15 tests), TEST-002 blocked on review, TEST-003 needs decision",
      selfGrade: {
        ...grades,
      ...
      retrospective: {
        ...
        toStop:
          "Assuming library behavior without checking official docs first",
      },
