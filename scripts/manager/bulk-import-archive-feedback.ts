...
function parseFeedbackFile(filePath: string): FeedbackEntry[] {
  ...
  const patterns = [
    ...
    new RegExp(
      `##\\s+${timestampPattern}\\s+—\\s+${agentPattern}:\\s+${statusPattern}`,
      "gim",
    ),
    // Alternative format: ## 2025-10-21T14:30Z — ENGINEER: Status
    new RegExp(
      `##\\s+${timestampPattern}\\s+—\\s+${agentPattern.toUpperCase()}:\\s+${statusPattern}`,
      "gim",
    ),
    ...
    new RegExp(
      `##\\s+${timestampPattern}\\s+—\\s+Manager:\\s+${statusPattern}`,
      "gim",
    ),
  ];
      ...
      for (const entry of entries) {
        try {
          ...
          const existing = await logDecision({
            ...
            status: entry.status.includes("completed")
              ? "completed"
              : entry.status.includes("blocked")
                ? "blocked"
                : entry.status.includes("progress")
                  ? "in_progress"
                  : "pending",
            progressPct: entry.status.includes("100%")
              ? 100
              : entry.status.includes("50%")
                ? 50
                : entry.status.includes("25%")
                  ? 25
                  : 0,
          ...
          importedEntries++;
          console.log(
            `    ✓ Imported: ${entry.timestamp} — ${entry.agent}: ${entry.status.substring(0, 50)}...`,
          );
        } catch (error) {
          skippedEntries++;
          console.log(
            `    ⚠ Skipped: ${entry.timestamp} — ${entry.agent} (${error.message})`,
          );
        }
      }
    } catch (error) {
