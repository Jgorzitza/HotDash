...
async function queryTaskDetails(taskId: string) {
  ...
  entries.forEach((entry, i) => {
    ...
    console.log(`   Progress: ${entry.progressPct !== null ? entry.progressPct + '%' : 'N/A'}`);
    ...
    if (payload && Object.keys(payload).length > 0) {
      ...
      if (payload.tests) {
        if (payload.tests.overall) {
          ...
          const parts = [];
          if (payload.tests.unit) parts.push(`Unit: ${payload.tests.unit.passing}/${payload.tests.unit.total}`);
          if (payload.tests.integration) parts.push(`Integration: ${payload.tests.integration.passing}/${payload.tests.integration.total}`);
          if (parts.length > 0) console.log(`      Tests: ${parts.join(', ')}`);
        }
      }
      if (payload.mcpEvidence) {
        console.log(`      MCP: ${payload.mcpEvidence.calls} calls (${payload.mcpEvidence.tools?.join(', ')})`);
      }
      if (payload.linesChanged) {
        console.log(`      Lines: +${payload.linesChanged.added}, -${payload.linesChanged.deleted}`);
      }
  ...
  const totalDuration = entries.reduce((sum, e) => sum + (Number(e.durationActual) || 0), 0);
  
  console.log('\n📊 Task Summary:');
  console.log(`   Total Entries: ${entries.length}`);
  console.log(`   Status: ${completed.length > 0 ? '✅ COMPLETED' : '🔵 IN PROGRESS'}`);
...
if (!taskId) {
  ...
  console.error('\nExample:');
  console.error('  npx tsx --env-file=.env scripts/manager/query-task-details.ts ENG-029');
  process.exit(1);
}
