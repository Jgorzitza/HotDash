...
function parseDirectionFile(filePath: string, agentName: string): ParsedTask[] {
  ...
  const taskRegex = /###?\s+(?:✅\s+)?([A-Z]{2,}-\d{3,}|PHASE\s+\d+):\s+(.+?)(?:\((\d+(?:\.\d+)?)\s*h?,?\s*(P\d)\)|$)/gim;
  ...
  while ((match = taskRegex.exec(content)) !== null) {
    ...
    if (sectionContent.includes('✅ COMPLETE') || sectionContent.includes('COMPLETE ✅')) {
      status = 'completed';
      // Extract completion notes
      const notesMatch = /\*\*Completion\*\*:\s*(.+?)(?:\n\n|\n###|\n##|$)/s.exec(sectionContent);
      completionNotes = notesMatch ? notesMatch[1].trim() : undefined;
    } else if (sectionContent.includes('BLOCKED') || sectionContent.includes('⏭️ DEFERRED')) {
      status = 'blocked';
    }
    ...
    if (acceptanceCriteria.length > 0 || allowedPaths.length > 0 || description.length > title.length) {
      tasks.push({
        taskId: taskId.toUpperCase(),
        title,
        description,
        acceptanceCriteria: acceptanceCriteria.length > 0 ? acceptanceCriteria : ['Complete task as described'],
...
// Main execution
const agents = process.argv[2] ? [process.argv[2]] : [
  'engineer', 'designer', 'data', 'devops', 'integrations', 'analytics',
  'inventory', 'seo', 'ads', 'content', 'product', 'pilot', 'qa',
  'support', 'ai-customer', 'ai-knowledge', 'manager'
];
