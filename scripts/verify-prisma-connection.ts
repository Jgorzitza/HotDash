...
async function verifyConnection() {
  ...
  console.log(
    `   Using IPv4:        ${hasIPv4 ? "✅ YES" : "⚠️  NO (using hostname)"}`,
  );
  ...
  if (!hasPort6543 || !hasPgBouncer) {
    ...
    console.error(
      "   Expected: postgresql://USER:PASS@IPv4:6543/postgres?pgbouncer=true",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      `   ✅ Query successful (found ${tableCount} tables in public schema)`,
    );
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `   ✅ Model query successful (${sessionCount} sessions in database)`,
    );
  } catch (error) {
  ...
  console.log(
    "   • inet_server_addr() showing IPv6 is EXPECTED (backend server)",
  );
  console.log(
    "   • Connection goes: App → Pooler (IPv4:6543) → DB (IPv6:5432)",
  );
  console.log("");
}
