...
function parseFeedbackFile(
  ...
  while ((match = entryRegex.exec(content)) !== null) {
    ...
    if (
      ...
      title.includes("DONE") ||
      progressMatch?.[1].includes("100%") ||
      progressMatch?.[1].includes("✅")
    ) {
      status = "completed";
    ...
    if (title.includes("BLOCKER") || title.includes("FIX"))
...
if (!agentName) {
  ...
  console.error(
    "  npx tsx --env-file=.env scripts/upload-my-feedback.ts integrations",
  );
  console.error("\nValid agent names:");
