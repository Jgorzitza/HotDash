...
async function detectRecurringThemes(
  ...
  console.log(
    `[CX Mining] Detecting themes with ${minOccurrences}+ occurrences in last ${dayWindow} days...`,
  );
...
async function main() {
  ...
  try {
    ...
    const shopDomain =
      process.env.SHOP_DOMAIN || "hot-rodan-ski-shop.myshopify.com";
    ...
    console.log(
      `✓ Generated ${actions.length} Action cards from ${themes.length} themes`,
    );
    console.log();
    ...
    if (actions.length > 0) {
      ...
      actions.slice(0, 3).forEach((action, i) => {
        ...
        console.log(
          `     Type: ${action.type} | Confidence: ${(action.confidence * 100).toFixed(0)}% | Ease: ${(action.ease * 100).toFixed(0)}%`,
        );
      });
      console.log();
    }
