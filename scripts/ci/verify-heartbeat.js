...
function parseHeartbeatSection(prBody) {
  ...
  if (
    section.includes("<2h single session") ||
    section.includes("<2 hours") ||
    section.includes("< 2h") ||
    section.includes("single session")
  ) {
    console.log(
      "✅ Single session (<2h) exemption found - heartbeat not required",
    );
    return null;
  }
  ...
  const pathMatch = section.match(
    /artifacts\/([^\/\s]+)\/([^\/\s]+)\/heartbeat\.ndjson/,
  );
...
function verifyHeartbeat(filePath) {
  ...
  if (!lastHeartbeat.status) {
    throw new Error(
      "❌ Last heartbeat missing 'status' field (must be 'doing' or 'done')",
    );
  }
  ...
  if (isNaN(lastTimestamp.getTime())) {
    throw new Error(
      `❌ Invalid timestamp format: ${lastHeartbeat.timestamp}. Expected: YYYY-MM-DDTHH:MM:SSZ`,
    );
  }
...
async function main() {
  ...
  try {
    ...
    console.error(
      "1. Append heartbeat entries every 15 minutes for tasks >2 hours",
    );
    console.error(
      '2. Format: {"timestamp":"2025-10-21T14:00:00Z","task":"TASK-001","status":"doing|done","progress":"40%"}',
    );
