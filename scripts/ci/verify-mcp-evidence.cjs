...
function parsePRBody(prBody) {
  ...
  if (section.includes("No MCP usage - non-code change") || 
    ...
    console.log("✅ Non-code change exemption found - MCP evidence not required");
    return null;
  }
  
  // Extract file paths matching pattern: artifacts/<agent>/<date>/mcp/*.jsonl
  const pathMatches = section.matchAll(/artifacts\/([^\/\s]+)\/([^\/\s]+)\/mcp\/([^\s\)]+\.jsonl)/g);
  ...
  lines.forEach((line, idx) => {
    ...
    try {
      ...
      if (!timestampRegex.test(obj.timestamp)) {
        throw new Error(`Invalid timestamp format: ${obj.timestamp}. Expected: YYYY-MM-DDTHH:MM:SSZ`);
      }
      
    } catch (error) {
      throw new Error(`❌ ${filePath} - Line ${idx + 1} is not valid JSON: ${error.message}\n\nExpected format:\n{"tool":"shopify-dev|context7|web-search","doc_ref":"<url>","timestamp":"2025-10-21T14:30:00Z","purpose":"Learn X"}`);
    }
  });
...
async function main() {
  ...
  try {
    ...
    console.log(`Found ${paths.length} MCP evidence file(s):\n${paths.map(p => `  - ${p}`).join('\n')}\n`);
