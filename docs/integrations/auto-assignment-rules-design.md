...
function detectTopic(message: string, context: Context): Topic {
  const scores = Object.entries(TOPIC_ROUTING).map(([topic, config]) => {
    ...
    const score = keywordMatches * 0.3 + 
...
async function calculateAgentLoad(agentId: number): Promise<AgentLoad> {
  ...
  const openConversations = await chatwoot.getAgentConversations(agentId, 'open');
...
async function assignToLeastBusyAgent(team: string): Promise<number> {
  ...
  const available = agentLoads.filter(a => 
    a.online_status === 'online' &&
    a.active_conversations < a.max_capacity
  );
  
  if (available.length === 0) {
    // All busy - assign to least loaded
    return agentLoads.sort((a, b) => 
      a.active_conversations - b.active_conversations
    )[0].agent_id;
  }
  ...
  const scored = available.map(a => ({
    agent_id: a.agent_id,
    score: (a.max_capacity - a.active_conversations) * 0.6 +
...
function calculateComplexity(context: ConversationContext): number {
  ...
  // Technical jargon
  const technicalWords = ['API', 'webhook', 'integration', 'database', 'error code'];
  score += technicalWords.filter(w => context.message.includes(w)).length * 0.8;
...
async function rebalanceAssignments() {
  ...
  for (const agent of agents) {
    ...
    if (load.active_conversations < 3 && queue.length > 0) {
      ...
      const queuedConv = queue.filter(c => 
        matchesAgentSkills(c, agent)
      ).slice(0, 3);
...
const ASSIGNMENT_RULES = [
  { id: 1, name: 'critical_escalation', priority: 100, handler: checkCriticalEscalation },
  ...
  { id: 4, name: 'low_confidence', priority: 80, handler: checkDraftConfidence },
  ...
  { id: 7, name: 'agent_preference', priority: 50, handler: checkAgentPreferences },
  { id: 8, name: 'load_balance', priority: 10, handler: loadBalanceAgents }
];

async function executeAssignmentRules(context: ConversationContext): Promise<Assignment> {
