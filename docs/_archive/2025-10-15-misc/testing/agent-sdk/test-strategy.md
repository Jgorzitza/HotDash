...
describe('Approval Queue Display', () => {
  ...
  test('should show realtime updates when new items arrive', async ({ page }) => {
    await page.goto('/app/approvals');
...
describe('Approve Action', () => {
  test('should approve draft and send reply', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="queue-item-test-item-1"]')).not.toBeVisible();
  ...
  test('should show confirmation modal before approving', async ({ page }) => {
    ...
    await expect(page.locator('text=Send this response to the customer?')).toBeVisible();
  });
  ...
  test('should track approve action metrics', async ({ page }) => {
    await seedApprovalQueue([{ id: 'metrics-test', created_at: new Date(Date.now() - 120000) }]);
...
describe('Edit & Approve Action', () => {
  ...
  test('should send edited version and track diff', async ({ page }) => {
    ...
    await page.fill('[data-testid="draft-editor"]', 'Edited draft with changes');
  ...
  test('should allow canceling edit and return to queue', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="draft-editor"]')).not.toBeVisible();
    await expect(page.locator('[data-testid="queue-item-cancel-test"]')).toBeVisible();
  });
});
```
...
describe('Escalate Action', () => {
  test('should show escalation dialog with assignee selection', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="escalation-reason-input"]')).toBeVisible();
  });
  ...
  test('should escalate with reason and assign agent', async ({ page }) => {
    ...
    await page.fill('[data-testid="escalation-reason-input"]', 'Customer extremely upset, needs manager');
...
describe('Reject Action', () => {
  ...
  test('should reject with reason and log for improvement', async ({ page }) => {
    await seedApprovalQueue([{
      id: 'reject-log-test',
    ...
    await page.selectOption('[data-testid="rejection-reason-select"]', 'factually_incorrect');
    await page.fill('[data-testid="rejection-notes"]', 'Wrong product information');
  ...
  test('should require rejection reason', async ({ page }) => {
    ...
    await expect(page.locator('text=Please select a rejection reason')).toBeVisible();
  });
});
```
...
describe('Real-Time Updates', () => {
  ...
  test('should remove item from queue when approved by another operator', async ({ page }) => {
    const queueItem = await seedApprovalQueue([{
      id: 'concurrent-test',
      conversation_id: 888
    }]);
    ...
    await page.goto('/app/approvals');
    await expect(page.locator('[data-testid="queue-item-concurrent-test"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="queue-item-concurrent-test"]')).not.toBeVisible();
  });

  test('should show urgent alert modal for high-priority items', async ({ page }) => {
    await page.goto('/app/approvals');
...
describe('Input Validation & Sanitization', () => {
  ...
  test('should reject SQL injection in queue filters', async () => {
    ...
    const response = await GET(`/api/approvals/queue?priority=${encodeURIComponent(sqlInjection)}`);
...
describe('Rate Limiting', () => {
  ...
  test('should rate limit approval actions per operator', async () => {
    await seedApprovalQueue(Array.from({ length: 50 }, (_, i) => ({
      id: `rate-limit-${i}`
    })));
...
export async function waitForQueueStatus(id: string, status: string, timeout = 5000) {
  const start = Date.now();
