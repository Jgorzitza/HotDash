...
export function chunkDocument(doc: string, type: keyof typeof CHUNKING_STRATEGIES) {
...
export const RAG_METRICS = {
  retrieval_metrics: {
    precision_at_k: (retrieved: any[], relevant: any[], k: number) => {
      const relevantRetrieved = retrieved.slice(0, k).filter(r => relevant.includes(r.id));
  ...
  generation_metrics: {
    ...
    context_utilization: (response: string, sources: any[]) => {
      ...
      return sources.filter(s => response.includes(s.text.substring(0, 20))).length / sources.length;
    },
  },
};
