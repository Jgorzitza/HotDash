...
export class ActiveLearningSystem {
  async identifyHardExamples(responses: AgentResponse[]) {
    const hardExamples = responses.filter(
      (r) =>
        r.confidence < 0.7 ||
    ...
    return hardExamples.sort(
      (a, b) => this.calculateLearningValue(b) - this.calculateLearningValue(a),
    );
  }
  ...
  private calculateLearningValue(response: AgentResponse): number {
    ...
    if (response.confidence < 0.6) value += 5; // Very uncertain
    if (response.edit_rate > 0.5) value += 3; // Heavily edited
