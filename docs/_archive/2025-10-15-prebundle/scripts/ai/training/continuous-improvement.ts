...
export class ContinuousImprovementCycle {
  ...
  private async labelHardExamples(dataset: TrainingSample[]) {
    const hardExamples = dataset.filter(
      (s) => !s.training_flags.quality_reviewed,
    );
  ...
  private decideFineTune(metrics: any, labeledCount: number): boolean {
    return (
      ...
      metrics.approval_rate < 0.9 && // Room for improvement
      metrics.cost_per_sample < 0.1 // Cost-effective
    );
  }
  ...
  private async fineTuneModel(data: TrainingSample[]) {
    ...
    const jobId = await openai.fineTuning.create({
        ...
        JSON.stringify({
          messages: [
            ...
            { role: "user", content: s.query.text },
            {
              role: "assistant",
              content:
                s.human_feedback.human_edited_text || s.agent_response.text,
            },
          ],
        }),
      )
