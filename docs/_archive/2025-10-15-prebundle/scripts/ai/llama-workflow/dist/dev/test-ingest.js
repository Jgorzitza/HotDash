...
async function testLoader(name, loader, retries = 2) {
        ...
        try {
            ...
            if (documents.length > 0) {
                result.sampleDocument = {
                    ...
                    // @ts-ignore
                    text_preview: (documents[0].text || documents[0].getText?.() || 'No text').slice(0, 200),
                    // @ts-ignore  
            ...
            if (attempt < retries) {
                ...
                console.warn(`⚠️  ${name} attempt ${attempt + 1} failed: ${error instanceof Error ? error.message : error}`);
                console.log(`   Retrying in ${delay}ms...`);
                await new Promise(resolve => setTimeout(resolve, delay));
            }
            else {
                console.error(`❌ ${name} failed after ${retries + 1} attempts: ${error instanceof Error ? error.message : error}`);
                return {
                    name,
                    success: false,
                    documentCount: 0,
...
//# sourceMappingURL=test-ingest.js.map