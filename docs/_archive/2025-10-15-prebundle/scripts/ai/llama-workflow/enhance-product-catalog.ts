...
async function enhanceProductCatalog() {
  ...
  const indexPath = path.join(config.LOG_DIR, 'indexes', '2025-10-12T0324', 'operator_knowledge.json');
  ...
  const enhancedProducts = products.map((product: any) => {
    ...
    else if (name.toLowerCase().includes('fitting') || name.toLowerCase().includes('adapter')) category = 'Fittings & Adapters';
    ...
    else if (name.toLowerCase().includes('tool') || name.toLowerCase().includes('wrench') || name.toLowerCase().includes('shear')) category = 'Tools';
    ...
    if (name.toLowerCase().includes('stainless')) material = 'Stainless Steel Braided';
  ...
  const qaText = enhancedProducts.map(p => {
    return [
      `# ${p.productName}`,
      `**Category:** ${p.category}`,
  ...
  console.log(`✓ Total Q&A entries: ${enhancedProducts.reduce((sum: number, p: any) => sum + p.questions.length, 0)}`);
  ...
  const summary = {
    ...
    stats: {
      ...
      with_material_info: enhancedProducts.filter((p: any) => p.material).length,
      total_qa_pairs: enhancedProducts.reduce((sum: number, p: any) => sum + p.questions.length, 0)
    }
  };
