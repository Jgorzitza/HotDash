...
program
  ...
  .option(
    "--sources <list>",
    "Sources to include: all|web|supabase|curated",
    "all",
  )
...
program
  .command("query")
  .description(
    "Query the support knowledge index and return answer with citations",
  )
...
program
  ...
  .action(async (options) => {
    try {
      console.log(
        `📊 Generating insights report: window=${options.window}, format=${options.format}`,
      );
