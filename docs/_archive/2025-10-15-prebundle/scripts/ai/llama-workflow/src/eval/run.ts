...
async function runEvaluation(): Promise<void> {
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      ...
      console.log(
        `   📊 BLEU: ${result.bleu.toFixed(3)} | ROUGE-L: ${result.rouge_l.toFixed(3)} | Citations: ${citationCheck.score.toFixed(3)} (${citationCheck.found.length}/${testCase.must_cite.length})`,
      );

      if (citationCheck.missing.length > 0) {
        console.log(
          `   ⚠️  Missing citations: ${citationCheck.missing.join(", ")}`,
        );
      }
    } catch (error) {
  ...
  const report: EvalReport = {
    ...
    summary: {
      ...
      citation_pass_count: results.filter((r) => r.citation_check.score === 1)
        .length,
      total_missing_citations: results.reduce(
        (sum, r) => sum + r.citation_check.missing.length,
        0,
      ),
    },
  };
...
`;
  ...
  console.log(
    `✅ Perfect Citations: ${report.summary.citation_pass_count}/${report.total_cases}`,
  );
  ...
  console.log(
    JSON.stringify({
      ...
      metrics: {
        ...
        citation_pass_rate:
          report.summary.citation_pass_count / report.total_cases,
      },
    }),
  );
}
