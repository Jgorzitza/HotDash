...
const configSchema = z
  .object({
    ...
    SUPABASE_ANON_KEY: z
      ...
      .optional(),
    SUPABASE_READONLY_KEY: z
      .string()
      .min(1, "Supabase readonly key is required")
      .optional(),
  ...
  .refine((data) => data.SUPABASE_ANON_KEY || data.SUPABASE_READONLY_KEY, {
    message:
      "Either SUPABASE_ANON_KEY or SUPABASE_READONLY_KEY must be provided",
...
export function validateConfig(): void {
  ...
  console.log(
    `  - Using ${config.SUPABASE_READONLY_KEY ? "READONLY" : "ANON"} Supabase key`,
  );
}
