...
export async function fetchCuratedDocs(): Promise<Document[]> {
  ...
  try {
    ...
    if (error) {
      console.warn(
        "Failed to fetch from support_curated_replies table:",
        error,
      );
      ...
      await fs.writeFile(
        path.join(outputDir, "curated_errors.log"),
        ...
        if (row.question && row.answer) {
          ...
          if (questionText.length > 0 && answerText.length > 0) {
            ...
            const {
              ...
              answer: cleanAnswer,
              redacted,
            } = sanitizeCuratedReply(questionText, answerText);
      ...
      console.log(
        `Successfully processed ${documents.length} curated support documents`,
      );
    }
    ...
    if (documents.length > 0) {
      ...
      await fs.writeFile(
        path.join(outputDir, "curated_sample.json"),
    ...
    const manifest = {
      ...
      is_placeholder:
        documents.length === 1 && documents[0].id_ === "curated:placeholder",
    };

    await fs.writeFile(
      path.join(outputDir, "curated_manifest.json"),
