...
export async function fetchDecisionDocs(): Promise<Document[]> {
  ...
  try {
    ...
    for (const row of rows) {
      const text =
        row.summary || row.detail || row.description || JSON.stringify(row);

      if (text && text.length > 10) {
        // Only include entries with substantial content
    ...
    if (documents.length > 0) {
      ...
      await fs.writeFile(
        path.join(outputDir, "decision_sample.json"),
    ...
    await fs.writeFile(
      path.join(outputDir, "decision_manifest.json"),
      JSON.stringify(manifest, null, 2),
    );

    console.log(
      `Successfully processed ${documents.length} decision log documents`,
    );
    return documents;
  } catch (error) {
    // Write error log
    ...
    try {
      await fs.mkdir(outputDir, { recursive: true });
      await fs.writeFile(
        path.join(outputDir, "decision_errors.log"),
...
export async function fetchTelemetryDocs(): Promise<Document[]> {
  const config = getConfig();
  const documents: Document[] = [];
  const timestamp = new Date().toISOString().replace(/[:]/g, "").slice(0, 15);
  const outputDir = path.join(
  ...
  try {
    ...
    for (const row of rows) {
      ...
      if (cleanText.length > 5) {
        // Only include events with some content
        const doc = new Document({
    ...
    if (documents.length > 0) {
      ...
      await fs.writeFile(
        path.join(outputDir, "telemetry_sample.json"),
        JSON.stringify(sample, null, 2),
      );
    }
    ...
    await fs.writeFile(
      path.join(outputDir, "telemetry_manifest.json"),
      JSON.stringify(manifest, null, 2),
    );

    console.log(
      `Successfully processed ${documents.length} telemetry documents`,
    );
    return documents;
  } catch (error) {
    // Write error log
    ...
    try {
      await fs.mkdir(outputDir, { recursive: true });
      await fs.writeFile(
        path.join(outputDir, "telemetry_errors.log"),
        JSON.stringify(errorLog, null, 2),
      );
    } catch (writeError) {
      console.error("Failed to write error log:", writeError);
