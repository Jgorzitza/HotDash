...
async function symlinkTarget(symlinkPath: string): Promise<string | null> {
  try {
...
export async function rollbackIndex(): Promise<{
  success: boolean;
  message: string;
  prevIndex?: string;
}> {
...
export async function verifyRollback(): Promise<{
  success: boolean;
  message: string;
  details?: any;
}> {
  ...
  try {
    ...
    if (latestTarget === prevTarget) {
      return {
        success: true,
        message:
          "System is in rollback state - latest points to previous index",
        details,
      };
    } else {
      return {
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log("Usage:");
      console.log(
        "  node rollback.js rollback    - Switch latest to prev index",
      );
      console.log(
        "  node rollback.js verify      - Check current rollback status",
      );
      process.exit(1);
  }
}
