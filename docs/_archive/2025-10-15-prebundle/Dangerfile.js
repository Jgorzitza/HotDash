...
const allowedMd = [
  ...
  /^docs\/growthengine\/(GOALS|PIPELINE|METRICS)\.md$/,
...
if (badMd.length)
  fail("Disallowed Markdown paths:\n" + badMd.map((f) => "- " + f).join("\n"));

// Require Issue linkage
if (!/Fixes\s+#\d+/.test(pr.body || ""))
  fail("PR must reference an Issue, e.g., Fixes #123");
...
// Tests nudge
const appTouched = changed.some((f) =>
  /^(app|apps|packages|supabase|prisma|scripts)\//.test(f),
);
const testsTouched = changed.some(
  (f) => /^tests\//.test(f) || /(\.|\/)test\./.test(f),
);
...
try {
  const cfg = JSON.parse(
    fs.readFileSync("app/agents/config/agents.json", "utf8"),
  );
  const cust = (cfg.agents || []).find((a) => a.id === "ai-customer");
  if (!cust || !cust.human_review)
    fail("ai-customer must have human_review: true");
  if (!Array.isArray(cust.reviewers) || !cust.reviewers.length)
    fail("ai-customer reviewers are required");
} catch (e) {
  warn("Missing or invalid app/agents/config/agents.json");
}
