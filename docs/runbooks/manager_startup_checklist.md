...
# Identify bloated direction files (>300 lines = needs cleanup)
wc -l docs/directions/*.md | awk '$1 > 300 {print $2, $1" lines"}' | tee /tmp/bloated-directions.txt
```

**For each bloated file**:
- Does it have completed tasks still listed?
- Does it have 10X expansion tasks burying current P0 work?
- Does it have outdated priorities?

**Note which files need cleanup in shutdown**

**Checkpoint**: ✅ Know which directions need cleanup

---

### Step 10: Prepare Agent Priorities for Today (2-3 minutes)

**Based on blockers and North Star, decide**:
