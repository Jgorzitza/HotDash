    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <MetricCard
          ...
          value={`${report.metrics.analytics.conversionRate.toFixed(2)}%`}
          status={report.metrics.analytics.conversionRate >= 2 ? 'good' : 'warning'}
        />
        <MetricCard
          title="Revenue"
          value={`$${report.metrics.analytics.revenue.toFixed(0)}`}
          status="good"
        />
        <MetricCard
          title="Response Time"
          value={`${report.metrics.performance.avgResponseTime.toFixed(0)}ms`}
          status={report.metrics.performance.avgResponseTime < 500 ? 'good' : 'warning'}
        />
        <MetricCard
          title="Error Rate"
          value={`${report.metrics.performance.errorRate.toFixed(2)}%`}
          status={report.metrics.performance.errorRate < 1 ? 'good' : 'critical'}
        />
      </div>
      ...
      {report.anomalies.anomalies.length > 0 && (
        <div style={{ marginBottom: 'var(--occ-space-4)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--occ-space-2)' }}>
            {report.anomalies.anomalies.slice(0, 5).map((anomaly, idx) => (
...
function HealthCard({ status, score }: { status: string; score: number }) {
  const colors = {
  ...
  return (
    <div style={{
      ...
      background: 'var(--occ-color-bg-secondary)',
      borderRadius: 'var(--occ-radius-md)',
      borderLeft: `6px solid ${colors[status as keyof typeof colors]}`
    }}>
...
function MetricCard({ title, value, status }: { title: string; value: string; status: string }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--occ-color-bg-secondary)',
      borderRadius: 'var(--occ-radius-md)',
      borderLeft: `4px solid ${colors[status as keyof typeof colors]}`
    }}>
      <div style={{ 
        fontSize: 'var(--occ-font-size-sm)',
        color: 'var(--occ-color-text-secondary)',
  ...
  return (
    <div style={{
      ...
      {stages.map((stage, idx) => (
        <div key={idx} style={{ marginBottom: idx < stages.length - 1 ? 'var(--occ-space-2)' : 0 }}>
...
function AnomalyCard({ anomaly }: { anomaly: any }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--occ-color-bg-secondary)',
      borderRadius: 'var(--occ-radius-md)',
      borderLeft: `4px solid ${severityColors[anomaly.significance as keyof typeof severityColors]}`
    }}>
      <div style={{ 
        fontSize: 'var(--occ-font-size-xs)',
        color: severityColors[anomaly.significance as keyof typeof severityColors],
