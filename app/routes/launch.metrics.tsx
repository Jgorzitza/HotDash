...
export default function LaunchMetrics() {
  const { metrics, lastUpdated } = useLoaderData<typeof loader>();
  
  return (
    <Page
      title="Production Launch Metrics"
    ...
    >
      <BlockStack gap="500">
        ...
        <InlineGrid columns={{ xs: 1, sm: 2, md: 4 }} gap="400">
          <MetricCard
            title="DAU/MAU Ratio"
            value={`${metrics.adoption.dauMau.ratio}%`}
            target="40%"
            trend={metrics.adoption.dauMau.trend}
            status={metrics.adoption.dauMau.ratio >= 40 ? 'success' : 'warning'}
          />
          
          <MetricCard
            title="New Signups (This Week)"
            value={metrics.adoption.signups.signups.toString()}
            target={metrics.adoption.signups.target.toString()}
            trend={metrics.adoption.signups.trend}
            status={
              metrics.adoption.signups.percentOfTarget >= 100
                ? 'success'
                : 'warning'
            }
          />
          ...
          <MetricCard
            ...
            value={Math.round(metrics.satisfaction.nps.npsScore).toString()}
            target="50"
            trend={metrics.satisfaction.nps.trend}
            status={metrics.satisfaction.nps.npsScore >= 50 ? 'success' : 'warning'}
          />
          
          <MetricCard
            title="Activation Rate"
            value={`${metrics.adoption.activation.activationRate}%`}
            target="60%"
            trend={metrics.adoption.activation.activationRate >= 60 ? 'up' : 'down'}
            status={
              metrics.adoption.activation.activationRate >= 60
                ? 'success'
                : 'warning'
            }
          />
          ...
          <BlockStack gap="400">
              ...
              <Card>
                <BlockStack gap="200">
                  ...
                  <BlockStack gap="100">
                    <MilestoneRow
                      label="Profile Setup"
                      percentage={metrics.adoption.activation.milestoneCompletion.profileSetup}
                    />
                    <MilestoneRow
                      label="First Integration"
                      percentage={metrics.adoption.activation.milestoneCompletion.firstIntegration}
                    />
                    <MilestoneRow
                      label="View Dashboard"
                      percentage={metrics.adoption.activation.milestoneCompletion.viewDashboard}
                    />
                    <MilestoneRow
                      label="First Approval"
                      percentage={metrics.adoption.activation.milestoneCompletion.firstApproval}
                    />
                    <MilestoneRow
                      label="First Workflow"
                      percentage={metrics.adoption.activation.milestoneCompletion.firstWorkflow}
                    />
                  </BlockStack>
                </BlockStack>
              </Card>
            ...
            <Card>
              <BlockStack gap="200">
                ...
                <BlockStack gap="100">
                  {metrics.adoption.featureAdoption.map((feature) => (
                    <FeatureAdoptionRow
                      key={feature.feature}
            ...
            <InlineGrid columns={{ xs: 1, md: 3 }} gap="400">
              ...
              <Card>
                <BlockStack gap="200">
                  ...
                  <BlockStack gap="100">
                    {metrics.satisfaction.csat.map((csat) => (
                      <div key={csat.interactionType}>
                        <Text as="p" variant="bodySm">
...
function MetricCard({ title, value, target, trend, status }: MetricCardProps) {
  ...
  const badgeTone =
    status === 'success' ? 'success' : status === 'warning' ? 'warning' : 'critical';
...
function MilestoneRow({ label, percentage }: MilestoneRowProps) {
  return (
...
interface FeatureAdoptionRowProps {
  feature: string;
  adoptionRate: number;
