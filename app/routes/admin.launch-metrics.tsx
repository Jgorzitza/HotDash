...
import { getLaunchMetrics, type LaunchMetrics } from "~/services/metrics/launch-metrics";
...
export default function LaunchMetricsDashboard() {
  ...
  const [metrics, setMetrics] = useState<LaunchMetrics | null>(initialData.metrics);
  ...
  useEffect(() => {
    if (lastMessage) {
      if (lastMessage.type === 'analytics-refresh') {
        ...
        setAlerts(prev => [...prev, lastMessage.data as any].slice(0, 10));
      }
    }
  ...
  useEffect(() => {
    if (revalidator.state === 'idle' && revalidator.data) {
      const data = revalidator.data as LoaderData;
      if (data.metrics) {
  ...
  return (
    <Page
      ...
      subtitle={`Last updated: ${new Date(initialData.timestamp).toLocaleString()}`}
      primaryAction={{
