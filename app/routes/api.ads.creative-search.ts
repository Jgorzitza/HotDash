...
import {
  searchSimilarCreatives,
  findSimilarHighPerformers,
  searchCreativesByText,
  getTopPerformingCreatives,
} from "~/services/ads/creative-search";
import type {
...
export async function loader({ request }: LoaderFunctionArgs) {
  try {
    ...
    const platforms = platformsParam
      ? (platformsParam.split(",") as any[])
      : undefined;

    // Build filters
    const filters: CreativeSearchFilters = {
      minRoas,
      minCtr,
      minConversions,
      platforms,
    ...
    const searchRequest: CreativeSearchRequest = {
