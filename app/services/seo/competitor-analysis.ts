...
function generateOpportunities(
  ...
  keywordGaps
    ...
    .forEach((kw) => {
      opportunities.push({
        ...
        difficulty:
          kw.difficulty < 40 ? "easy" : kw.difficulty < 60 ? "medium" : "hard",
      });
    });
  ...
  const rankingOpps = keywordGaps.filter(
    (kw) => kw.position >= 11 && kw.position <= 20,
  );
  rankingOpps.slice(0, 3).forEach((kw) => {
    opportunities.push({
      ...
      competitor:
        competitors.find((c) =>
          c.topKeywords.some((ck) => ck.keyword === kw.keyword),
        )?.domain || "",
...
function generateRecommendedActions(
  ...
  const keywordOpps = opportunities.filter(
    (o) => o.type === "keyword-gap" && o.estimatedImpact === "high",
  );
  if (keywordOpps.length > 0) {
    actions.push(
      `Target ${keywordOpps.length} high-impact keyword${keywordOpps.length > 1 ? "s" : ""} with low competition`,
    );
  }
  ...
  if (contentGaps.length > 0) {
    actions.push(
      `Create ${contentGaps[0].contentTopic} content to match competitor offerings`,
    );
  }
  ...
  actions.push(
    `Build backlinks (competitor average: ${Math.round(avgBacklinks)} links)`,
  );

  // Content frequency
  const updateFrequencies = competitors.map(
    (c) => c.contentMetrics.updateFrequency,
  );
  if (
    updateFrequencies.includes("daily") ||
  ...
  const easyWins = opportunities.filter(
    (o) => o.type === "ranking-opportunity",
  );
  if (easyWins.length > 0) {
    actions.push(
      `Quick wins: Outrank competitors on ${easyWins.length} keyword${easyWins.length > 1 ? "s" : ""} where they rank #11-20`,
    );
  }
...
export async function analyzeCompetitors(
  ...
  try {
    ...
    const keywordGaps = identifyKeywordGaps(
      userKeywords,
