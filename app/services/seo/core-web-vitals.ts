...
export interface VitalMetric {
  ...
  percentile: number; // 75th percentile
  threshold: {
    good: number;
...
export async function analyzeWebVitals(url: string): Promise<PageSpeedAnalysis> {
  ...
  const cached = getCached<PageSpeedAnalysis>(cacheKey);
  if (cached) {
 ...
 */
function extractOpportunities(data: any): PerformanceOpportunity[] {
  const audits = data.lighthouseResult?.audits || {};
  ...
  opportunityAudits.forEach(({ key, title }) => {
    ...
    if (audit && audit.details) {
      ...
      if (savings > 0) {
        ...
        else if (savings > 200) impact = "medium";
        else impact = "low";
 ...
 */
function extractDiagnostics(data: any): PerformanceDiagnostic[] {
  const audits = data.lighthouseResult?.audits || {};
