...
export class TrainingDataService {
  ...
  async searchKnowledgeBase(query: string, category?: string): Promise<KnowledgeBaseEntry[]> {
    try {
      ...
      const scoredResults = results.map(entry => {
        ...
        const keywordMatches = entry.searchKeywords.filter(keyword => 
   ...
   */
  async updateTrainingDataPerformance(
   ...
   * Generate improvement recommendations
   */
  private generateImprovementRecommendations(trainingData: TrainingData[]): string[] {
   ...
   */
  private getDefaultTrainingData(): TrainingData[] {
    return [
      {
        id: 'training-1',
