...
export class AccountsSubAgent {
  ...
  async getCustomerAccountInfo(
    customerId: string,
    token: string
  ): Promise<CustomerAccountInfo | null> {
    try {
      // Check ABAC policy
      const abacResult = await this.checkABACPolicy({
        agent: 'accounts-sub-agent',
   ...
   */
  private async checkABACPolicy(policy: Omit<ABACPolicy, 'allowed' | 'reason'>): Promise<ABACPolicy> {
    try {
   ...
   */
  private async simulateCustomerAccountsMCPCall(mcpCall: any): Promise<CustomerOrder[]> {
   ...
   */
  private async simulateOrderDetailsMCPCall(mcpCall: any): Promise<CustomerOrder | null> {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  private async simulateAccountInfoMCPCall(mcpCall: any): Promise<CustomerAccountInfo | null> {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  private async simulateUpdatePreferencesMCPCall(mcpCall: any): Promise<boolean> {
    await new Promise(resolve => setTimeout(resolve, 100));
   ...
   */
  private getMockCustomerOrders(customerId: string): CustomerOrder[] {
    return [
      {
        id: 'order-1',
        orderNumber: 'ORD-001',
   ...
   */
  private getMockOrderDetails(orderId: string): CustomerOrder | null {
   ...
   */
  private getMockCustomerAccountInfo(customerId: string): CustomerAccountInfo | null {
    return {
      id: customerId,
