 ...
 */
export function startTest(testId: string): ABTest {
 ...
 */
export function completeTest(testId: string): {
  test: ABTest;
 ...
 */
export function getCampaignTests(campaignId: string): ABTest[] {
 ...
 * Cancel a test
 */
export function cancelTest(testId: string, reason: string): ABTest {
  const test = tests.get(testId);
  if (!test) {
    throw new Error(`Test not found: ${testId}`);
  }
