...
import { generateAllAlerts, type AlertThresholds, DEFAULT_THRESHOLDS } from './budget-alerts';
import { generateAutomationRecommendations, type AutomationThresholds, DEFAULT_AUTOMATION_THRESHOLDS } from './campaign-automation';
import { generateAIOptimizationRecommendations, DEFAULT_AI_OPTIMIZER_CONFIG } from '~/lib/ads/ai-optimizer';
import type { Campaign, CampaignPerformance, CampaignSummary, BudgetAlert } from './types';
...
export interface MonitoringConfig {
  refreshIntervalMs: number; // How often to check campaigns
  roasThreshold: number; // Minimum acceptable ROAS
 ...
 */
export function monitorCampaignPerformance(
  campaigns: Campaign[],
  performances: CampaignPerformance[],
  config: MonitoringConfig = DEFAULT_MONITORING_CONFIG
): CampaignMonitoringStatus[] {
  const statuses: CampaignMonitoringStatus[] = [];
  ...
  for (const campaign of campaigns) {
    ...
    const campaignAlerts = generateAllAlerts(
      [campaign],
      [performance],
      undefined,
      undefined,
      config.budgetAlertThresholds
    );
 ...
 */
export function generateMonitoringDashboard(
  campaigns: Campaign[],
  performances: CampaignPerformance[],
  config: MonitoringConfig = DEFAULT_MONITORING_CONFIG
): MonitoringDashboard {
 ...
 */
export function formatAlertNotification(alert: BudgetAlert): string {
 ...
 */
export function getCampaignHealthScore(status: CampaignMonitoringStatus): number {
  let score = 100;
