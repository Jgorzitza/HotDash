...
export interface InventoryTurnoverMetrics {
  ...
  costOfGoodsSold: number; // COGS for the period
  turnoverRate: number; // Times inventory is sold/replaced per year
  daysInventoryOutstanding: number; // DIO - average days to sell inventory
...
export interface AgingAnalysis {
  ...
  estimatedValue: number; // Current stock × cost per unit
}
...
export interface InventoryAnalyticsSummary {
  ...
  abcAnalysis: {
    classACount: number; // ~20% of SKUs, 80% of revenue
    classBCount: number; // ~30% of SKUs, 15% of revenue
    classCCount: number; // ~50% of SKUs, 5% of revenue
...
export function calculateTurnoverRate(
  ...
  let classification: "fast" | "normal" | "slow" | "very_slow";
  if (turnoverRate > 12) {
    classification = "fast";
 ...
 */
export function classifyStockAge(daysSinceLastSale: number | null): StockAge {
...
export function performABCAnalysis(
  ...
  const results: ABCAnalysisItem[] = sortedProducts.map((product) => {
...
export async function generateInventoryAnalytics(
  products: Array<{
    ...
    productName: string;
    currentStock: number;
    avgInventory: number; // Average inventory during period
    costPerUnit: number;
  ...
  // ABC Analysis
  const abcProducts = performABCAnalysis(
    products.map((p) => ({
      productId: p.productId,
      productName: p.productName,
