...
export class ProductAnalyticsService {
  ...
  async getTileEngagementMetrics(
    ...
    for (const [tileName, data] of tileMap.entries()) {
      ...
      const avgTimeToClick = data.timesToClick.length > 0
        ? data.timesToClick.reduce((sum, t) => sum + t, 0) / data.timesToClick.length
        : 0;
  ...
  async getModalActionMetrics(
    ...
    for (const decision of decisions) {
      ...
      const metadata = decision.metadata ? JSON.parse(decision.metadata as string) : {};
    ...
    for (const [modalType, data] of modalMap.entries()) {
      ...
      const avgDecisionTime = data.decisionTimes.length > 0
        ? data.decisionTimes.reduce((sum, t) => sum + t, 0) / data.decisionTimes.length
        : 0;
  ...
  async getUnusedFeatures(
    ...
    return allMetrics.filter(m => 
      m.adoptionRate < threshold || 
      (m.lastUsed && m.lastUsed < sevenDaysAgo)
    );
  }
  ...
  async generateInsights(
    ...
    const unusedFeatures = await this.getUnusedFeatures(0.10, startDate, endDate);

    // Insight 1: Low tile engagement
    const lowEngagementTiles = tileMetrics.filter(t => t.clickRate < 0.20); // < 20% click rate
    ...
    const highRejectionModals = modalMetrics.filter(m => m.approvalRate < 0.50); // < 50% approval
