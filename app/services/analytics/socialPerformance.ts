...
export interface SocialPostMetrics {
  ...
  metrics: {
    ...
    impressions: number;
    clicks: number;
    engagement: number; // Calculated: (likes + shares + comments) / reach
  };
  performance: {
 ...
 */
export function calculateSocialPerformanceSummary(
  ...
  const topPerformingPost = posts.reduce((top, post) => 
    post.metrics.engagement > (top?.metrics.engagement || 0) ? post : top
  );
 ...
 */
export function calculateSocialPerformanceTrends(
  currentPosts: SocialPostMetrics[],
  previousPosts: SocialPostMetrics[]
): SocialPostMetrics[] {
  // Create a map of previous posts for comparison
  const previousMap = new Map<string, SocialPostMetrics>();
  ...
  return currentPosts.map(post => {
    ...
    const engagementChange = calculatePercentageChange(
      ...
      previousPost.metrics.engagement
    );
    const reachChange = calculatePercentageChange(
      post.metrics.reach,
      previousPost.metrics.reach
    );
    const clicksChange = calculatePercentageChange(
      post.metrics.clicks,
      previousPost.metrics.clicks
    );
 ...
 */
export function exportSocialPerformanceData(summary: SocialPerformanceSummary) {
  return {
    metrics: {
