...
export class UserSegmentationService {
   ...
   */
  async getUserSegmentData(userId: string): Promise<UserSegmentData> {
  ...
  async getSegmentAnalytics(): Promise<SegmentAnalytics[]> {
    ...
    const segmentSessions = new Map<UserSegment, number[]>();
    ...
    const segments: UserSegment[] = ["power", "casual", "new", "churned"];
   ...
   */
  private getSegmentCharacteristics(segment: UserSegment): string[] {
    const characteristics: Record<UserSegment, string[]> = {
      power: [
        "Daily active users",
   ...
   */
  private getSegmentRecommendations(segment: UserSegment): string[] {
    const recommendations: Record<UserSegment, string[]> = {
      power: [
