 ...
 */
function detectOutliers(facts: any[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  ...
  for (const factType of factTypes) {
    ...
    if (typedFacts.length < 4) continue; // Need at least 4 points for IQR

    // Extract numeric values for key metrics
    const revenues = typedFacts
      .map((f) => (f.value as any).revenue || 0)
      .filter((v) => v > 0);
...
function findOutliersIQR(
  ...
  const outlierDates: Date[] = [];
  facts.forEach((fact, index) => {
    const value = (fact.value as any)[metricKey] || 0;
 ...
 */
function detectInconsistencies(facts: any[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
 ...
 */
function detectStaleData(facts: any[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
