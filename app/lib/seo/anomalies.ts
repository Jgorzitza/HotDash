...
export function detectVitalsAnomalies(
  ...
  return inputs
    ...
    .map((input) => {
      const thresholds =
        ANOMALY_THRESHOLDS.vitals[
          input.metric.toLowerCase() as "lcp" | "fid" | "cls"
        ];
      const severity: AnomalySeverity =
        input.value > thresholds.needsImprovement ? "critical" : "warning";
 ...
 */
export function detectCrawlAnomalies(inputs: CrawlErrorInput[]): SEOAnomaly[] {
  return inputs
