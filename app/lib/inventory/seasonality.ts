...
export interface SeasonalityPattern {
  ...
  peakMonths: number[]; // 0-11 (Jan=0, Dec=11)
  peakFactor: number; // e.g., 1.25 for 25% increase
  offSeasonFactor: number; // e.g., 0.8 for 20% decrease
}
 ...
 */
export function calculateSeasonalAdjustedSales(
 ...
 */
export function getMonthsUntilPeak(
  category: ProductCategory,
  currentMonth: number,
): number {
  if (currentMonth < 0 || currentMonth > 11) {
