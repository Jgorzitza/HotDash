...
import { calculateROAS, calculateCTR, calculateCPC } from './metrics';
...
export interface BidAdjustmentRecommendation {
  ...
  recommendedBid: number; // in cents
...
export interface AIOptimizerConfig {
  bidAdjustment: {
    ...
    minConfidence: number; // Minimum confidence to recommend
    learningRate: number; // How aggressively to adjust (0-1)
  };
  audienceTargeting: {
    enabled: boolean;
  ...
  budgetAllocation: {
    enabled: boolean;
    rebalanceThreshold: number; // Min ROAS difference to trigger rebalance
    minBudgetPerCampaign: number; // in cents
  };
 ...
 */
export function generateBidAdjustmentRecommendations(
  campaigns: CampaignSummary[],
  performances: CampaignPerformance[],
  config: AIOptimizerConfig = DEFAULT_AI_OPTIMIZER_CONFIG
): BidAdjustmentRecommendation[] {
  if (!config.bidAdjustment.enabled) {
    return [];
  }

  const recommendations: BidAdjustmentRecommendation[] = [];
  ...
  for (const campaign of campaigns) {
    ...
    if (roas > config.performanceThresholds.minROAS * 1.5) {
      bidAdjustment = Math.min(
        config.bidAdjustment.maxAdjustmentPercent,
        (roas - config.performanceThresholds.minROAS) * 5 * config.bidAdjustment.learningRate
      );
    ...
    // Low ROAS campaigns: decrease bids to improve efficiency
      ...
      bidAdjustment = -Math.min(
        config.bidAdjustment.maxAdjustmentPercent,
        (config.performanceThresholds.minROAS - roas) * 10 * config.bidAdjustment.learningRate
      );
 ...
 */
export function generateAudienceTargetingRecommendations(
  campaigns: CampaignSummary[],
  performances: CampaignPerformance[],
  config: AIOptimizerConfig = DEFAULT_AI_OPTIMIZER_CONFIG
  ...
  const recommendations: AudienceTargetingRecommendation[] = [];
 ...
 */
export function generateBudgetAllocationRecommendations(
  campaigns: CampaignSummary[],
  totalBudget: number,
  config: AIOptimizerConfig = DEFAULT_AI_OPTIMIZER_CONFIG
...
export function generateAIOptimizationRecommendations(
  ...
  const avgConfidence = (
    bidAdjustments.reduce((sum, r) => sum + r.confidence, 0) / Math.max(bidAdjustments.length, 1) +
    audienceTargeting.reduce((sum, r) => sum + r.confidence, 0) / Math.max(audienceTargeting.length, 1) +
    budgetAllocation.confidence
  ) / 3;
