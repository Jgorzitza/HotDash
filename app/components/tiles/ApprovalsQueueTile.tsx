...
interface ApprovalsQueueTileProps {
  // Optional props for testing or external data
  pendingCount?: number;
  oldestPendingTime?: string;
}

export function ApprovalsQueueTile({
  pendingCount: externalPendingCount,
  oldestPendingTime: externalOldestTime,
}: ApprovalsQueueTileProps) {
  const [pendingCount, setPendingCount] = useState(externalPendingCount || 0);
  const [oldestPendingTime, setOldestPendingTime] = useState(
    externalOldestTime || "",
  );
  ...
  const badgeStyle = {
    ...
    background:
      pendingCount > 0 ? "var(--occ-bg-warning)" : "var(--occ-bg-success)",
    color:
      pendingCount > 0 ? "var(--occ-text-warning)" : "var(--occ-text-success)",
  };
