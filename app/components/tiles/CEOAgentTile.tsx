import type { CEOAgentStatsResponse } from "~/routes/api.ceo-agent.stats";

interface CEOAgentTileProps {
  stats: CEOAgentStatsResponse["data"];
}

export function CEOAgentTile({ stats }: CEOAgentTileProps) {
  if (!stats) {
    return (
      <p
        style={{
    ...
    >
      ...
      {recent_actions && recent_actions.length > 0 && (
        <section>
          ...
          >
            {recent_actions.slice(0, 3).map((action) => (
              <li
                key={action.id}
                style={{
                  display: "flex",
