  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--occ-space-4)", height: "100%" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--occ-space-4)" }}>
        <div>
          <div style={{ fontSize: "var(--occ-font-size-sm)", color: "var(--occ-text-secondary)", marginBottom: "var(--occ-space-1)" }}>
            Keywords
          </div>
          <div style={{ fontSize: "var(--occ-font-size-2xl)", fontWeight: "var(--occ-font-weight-bold)" }}>
            {data.totalKeywords}
          </div>
        </div>
        <div>
      ...
      <div style={{ padding: "var(--occ-space-3)", background: "var(--occ-bg-secondary)", borderRadius: "var(--occ-radius-md)", borderLeft: `3px solid ${isImprovement ? 'var(--occ-border-success)' : 'var(--occ-border-critical)'}` }}>
        ...
        <div style={{ fontSize: "var(--occ-font-size-xs)", color: isImprovement ? "var(--occ-text-success)" : "var(--occ-text-critical)" }}>
          #{data.topMover.oldPosition} → #{data.topMover.newPosition} ({isImprovement ? '' : '+'}{data.topMover.change})
        </div>
      </div>
