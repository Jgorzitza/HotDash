...
type TimeWindow = '7d' | '14d' | '28d';
type SortBy = 'roi' | 'revenue' | 'conversions' | 'confidence';

export function AttributionPanel({ data, loading = false, error }: AttributionPanelProps) {
  const [selectedWindow, setSelectedWindow] = useState<TimeWindow>('28d');
  ...
  return (
    <div style={{ padding: 'var(--occ-space-4)' }}>
      ...
      <div style={{
        ...
        <SummaryCard
          title="Average ROI"
          value={`${data.summary.averageROI.toFixed(1)}x`}
          icon="📈"
        />
        <SummaryCard
          title="Overall Confidence"
          value={`${data.summary.overallConfidence.toFixed(0)}%`}
          icon="🎯"
        />
      </div>
          ...
          <table style={{
            ...
            <thead>
              <tr style={{ backgroundColor: 'var(--occ-bg-subdued)' }}>
                ...
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>Impressions</th>
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>Clicks</th>
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>Conversions</th>
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>Revenue</th>
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>ROAS</th>
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>CTR</th>
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>Confidence</th>
                <th style={{ padding: 'var(--occ-space-3)', textAlign: 'center' }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {sortedActions.map((action) => (
                <ActionRow
                  ...
                  isSelected={selectedAction === action.actionId}
                  onSelect={() => setSelectedAction(
                    selectedAction === action.actionId ? null : action.actionId
                  )}
                />
              ))}
            </tbody>
      ...
      {selectedAction && (
        <ActionDetailModal
          action={data.actions.find(a => a.actionId === selectedAction)!}
...
function ActionRow({ 
  ...
  const getDeltaColor = (value: number) => {
    if (value > 0) return 'var(--occ-color-success)';
    if (value < 0) return 'var(--occ-color-error)';
  ...
  return (
    <tr style={{ 
      backgroundColor: isSelected ? 'var(--occ-bg-primary-subdued)' : 'transparent',
      cursor: 'pointer'
    }}>
      ...
      <td style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>
        <div>
          ...
          <div style={{ 
            ...
            {delta.impressions > 0 ? '+' : ''}{delta.impressions.toLocaleString()}
          </div>
        </div>
      </td>
      <td style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>
      ...
      <td style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>
        <div>
          ...
          <div style={{ 
            ...
            {delta.conversions > 0 ? '+' : ''}{delta.conversions.toLocaleString()}
          </div>
        </div>
      </td>
      <td style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>
      ...
      <td style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>
        <div>
          ...
          <div style={{ 
            ...
            {delta.roas > 0 ? '+' : ''}{delta.roas.toFixed(1)}x
          </div>
        </div>
      </td>
      <td style={{ padding: 'var(--occ-space-3)', textAlign: 'right' }}>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--occ-space-4)' }}>
          {/* Expected vs Actual */}
          <div>
            <h4 style={{ margin: 0, marginBottom: 'var(--occ-space-2)' }}>Expected vs Actual (28d)</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--occ-space-2)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Revenue:</span>
                <span>
                  ${action.expectedImpact.revenue.toLocaleString()} → ${action.actualImpact['28d'].revenue.toLocaleString()}
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Conversions:</span>
                <span>
                  {action.expectedImpact.conversions} → {action.actualImpact['28d'].conversions}
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>ROAS:</span>
                <span>
                  {action.expectedImpact.roas.toFixed(1)}x → {action.actualImpact['28d'].roas.toFixed(1)}x
                </span>
              </div>
            </div>
          </div>
          ...
          {/* Performance Metrics */}
          <div>
            <h4 style={{ margin: 0, marginBottom: 'var(--occ-space-2)' }}>Performance Metrics</h4>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--occ-space-2)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>Confidence Score:</span>
                <span style={{ 
                  color: action.confidenceScore >= 80 ? 'var(--occ-color-success)' : 
                         action.confidenceScore >= 60 ? 'var(--occ-color-warning)' : 'var(--occ-color-error)'
                }}>
                  {action.confidenceScore.toFixed(0)}%
                </span>
              </div>
        ...
        <div style={{ marginTop: 'var(--occ-space-4)' }}>
          <h4 style={{ margin: 0, marginBottom: 'var(--occ-space-2)' }}>Performance by Time Window</h4>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 'var(--occ-space-3)' }}>
            {(['7d', '14d', '28d'] as const).map((window) => {
              const metrics = action.actualImpact[window];
              return (
                <div key={window} style={{
                  ...
                  <h5 style={{ margin: 0, marginBottom: 'var(--occ-space-2)' }}>{window}</h5>
...
function getSortedActions(actions: ActionAttribution[], sortBy: SortBy): ActionAttribution[] {
