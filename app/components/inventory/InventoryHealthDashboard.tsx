...
export function InventoryHealthDashboard({
  ...
  const [localMetrics, setLocalMetrics] = useState<InventoryHealthMetrics | null>(metrics || null);
  ...
  const getHealthTone = (health: string): 'success' | 'warning' | 'critical' => {
    switch (health) {
  ...
  const stockDistribution = [
    { label: 'In Stock', value: localMetrics.inStock, tone: 'success' as const },
    { label: 'Low Stock', value: localMetrics.lowStock, tone: 'warning' as const },
    { label: 'Out of Stock', value: localMetrics.outOfStock, tone: 'critical' as const },
    { label: 'Overstock', value: localMetrics.overstock, tone: 'info' as const },
  ];
      ...
      <BlockStack gap="500">
        ...
        <BlockStack gap="200">
          ...
          {stockDistribution.map((item) => (
            <div key={item.label}>
              ...
              <ProgressBar
                progress={(item.value / localMetrics.totalProducts) * 100}
                tone={item.tone}
                size="small"
              />
            </div>
        ...
        <BlockStack gap="200">
          ...
          <div>
            ...
            <ProgressBar
              progress={localMetrics.ropCompliance}
              tone={localMetrics.ropCompliance >= 90 ? 'success' : localMetrics.ropCompliance >= 75 ? 'warning' : 'critical'}
              size="small"
            />
          </div>
        ...
        {localMetrics.emergencySourcingNeeded > 0 && (
          <div style={{ 
            padding: '12px', 
            backgroundColor: '#FFF4E5', 
