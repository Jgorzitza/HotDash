...
import { z } from "zod";
/** Social post tool (Publer) with human approval */
...
const socialPost = tool({
    name: "social.post",
    description: "Schedule or publish a social post via aggregator (Publer)",
    inputSchema: PostSchema,
    async handler({ text, firstComment, mediaUrls = [], platforms }) {
        // NOTE: call out to your backend `/api/social/post` which proxies to Publer with server-side secret
        // return { id: 'post_123', status: 'pending_approval' };
        return { draft: { text, firstComment, mediaUrls, platforms } };
    },
...
//# sourceMappingURL=ai-customer.js.map