...
const PUBLER_BASE_URL = process.env.PUBLER_BASE_URL || "https://app.publer.com/api/v1/";
...
async function publerFetch(path, init = {}) {
    const apiKey = assertEnv("PUBLER_API_KEY");
    const workspace = init.workspaceId || process.env.PUBLER_WORKSPACE_ID;
    if (!workspace)
        throw new Error("Missing Publer workspace id (PUBLER_WORKSPACE_ID or pass workspaceId)");
    const headers = {
...
export async function listWorkspaces() {
    ...
    if (!res.ok)
        throw new Error(`Publer API error ${res.status}`);
    return res.json();
}
...
export async function getJobStatus(jobId, workspaceId) {
    return publerFetch(`job_status/${encodeURIComponent(jobId)}`, {
        method: "GET",
        workspaceId,
    });
}
//# sourceMappingURL=publer.js.map