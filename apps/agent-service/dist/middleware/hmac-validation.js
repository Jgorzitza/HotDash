 ...
 */
export function hmacValidation(secretEnvVar, options = {}) {
    ...
    return async (req, res, next) => {
        try {
            ...
            const signature = req.headers[opts.header];
            if (!signature) {
                console.warn('[HMAC] Missing signature header');
                return res.status(401).json({ error: 'Missing signature' });
            }
            // Get timestamp (for replay protection)
            const timestamp = req.headers[opts.timestampHeader];
            if (timestamp) {
 ...
 */
export function genericHMACValidation(secretEnvVar) {
...
export function webhookRateLimit(maxRequests = 100, windowMs = 60000 // 1 minute
) {
    return (req, res, next) => {
...
//# sourceMappingURL=hmac-validation.js.map