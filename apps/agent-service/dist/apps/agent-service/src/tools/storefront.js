...
export const storefrontSearchProducts = tool({
    ...
    description: 'Search the storefront catalog using Storefront MCP. Returns products, collections, and available filters.',
    parameters: z.object({
...
export const storefrontCheckAvailability = tool({
    ...
    description: 'Check inventory availability for a product or specific variant using Storefront MCP. Supports optional location scoping.',
    parameters: z.object({
    ...
    async execute({ customerId, productId, variantId, location }) {
        try {
            const storefrontService = await getStorefrontService();
            const availability = await storefrontService.checkAvailability(customerId, productId, variantId, location);
            return {
                success: true,
