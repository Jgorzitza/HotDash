 ...
 */
import { VectorStoreIndex, Document, storageContextFromDefaults, Settings, BaseEmbedding, SentenceSplitter, } from "llamaindex";
...
async function loadOpenAIKey() {
    ...
    try {
        ...
        console.warn("⚠️  Could not load OpenAI key from vault:", error.message);
    }
...
function getCategoryFromFilename(fileName) {
    ...
    if (lower.includes("shipping"))
        return "shipping";
    if (lower.includes("tracking") || lower.includes("order"))
        return "tracking";
 ...
 */
async function loadSupportDocuments(sourcesDir) {
...
async function buildIndex(options = {}) {
    ...
    const { forceMock = false, persistDir = path.join(PROJECT_ROOT, "packages/memory/indexes/operator_knowledge"), sourcesDir = path.join(PROJECT_ROOT, "data/support"), } = options;
    ...
    try {
        ...
        if (!useMock && apiKey) {
            ...
            Settings.embedModel = new OpenAIEmbeddingProvider(apiKey, "text-embedding-3-small");
        }
        else {
 ...
 */
async function testIndex(indexPath) {
    ...
    try {
        ...
        console.log("⚠️  Query testing requires LLM configuration (not yet implemented)");
