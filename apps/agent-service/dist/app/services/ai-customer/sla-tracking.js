 ...
 */
export async function trackSLA(timeRange, thresholds, supabaseUrl, supabaseKey) {
    ...
    const slaThresholds = {
        ...
        businessHoursOnly: thresholds.businessHoursOnly !== undefined
    ...
    try {
        ...
        const conversations = records.map((record) => calculateConversationSLA(record, slaThresholds));
        // Generate alerts
 ...
 */
function calculateConversationSLA(record, thresholds) {
    const createdAt = new Date(record.createdAt);
    const now = new Date();
    // Calculate First Response Time
    let firstResponseTime;
    let firstResponseBreached = false;
    let firstResponseRemaining;
    ...
    else if ((firstResponseRemaining !== undefined && firstResponseRemaining < 15) ||
 ...
 */
function calculateMinutes(start, end, businessHoursOnly) {
 ...
 */
function generateAlerts(conversations, thresholds) {
    const alerts = [];
    for (const conversation of conversations) {
        ...
        else if (conversation.remainingTime.firstResponse !== undefined &&
        ...
        // Resolution at risk
        else if (conversation.remainingTime.resolution !== undefined &&
 ...
 */
function createEmptySLAResult(thresholds) {
    return {
        conversations: [],
        summary: {
...
//# sourceMappingURL=sla-tracking.js.map