 ...
 * ```
 */
export async function getGradingTrends(days = 30) {
    try {
...
export async function identifyLowScoringPatterns() {
    try {
        ...
        for (const record of records) {
            ...
            if (grades) {
                const hasLowScore = grades.tone < 3 || grades.accuracy < 3 || grades.policy < 3;
                if (hasLowScore) {
                    lowScores.push({
 ...
 */
function groupByTemplate(grades) {
 ...
 */
function groupByTimePeriod(grades, days) {
    const grouped = new Map();
 ...
 */
function generateInsights(grades, avgTone, avgAccuracy, avgPolicy, byTemplate) {
    const insights = [];
    if (grades.length === 0) {
 ...
 */
export async function analyzeGrades(timeRange, supabaseUrl, supabaseKey) {
...
//# sourceMappingURL=grading-analytics.js.map