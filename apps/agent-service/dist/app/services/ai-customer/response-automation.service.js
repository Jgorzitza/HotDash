...
export class ResponseAutomationService {
     ...
     */
    async generateAutomatedResponse(inquiry, analysis) {
        try {
     ...
     */
    findMatchingTemplate(message, analysis) {
        const messageLower = message.toLowerCase();
        for (const template of this.templates) {
            // Check if any trigger keywords match
            const hasMatchingKeyword = template.triggerKeywords.some(keyword => messageLower.includes(keyword.toLowerCase()));
     ...
     */
    async generateFromTemplate(template, inquiry, analysis) {
     ...
     */
    async generateCustomResponse(message, analysis) {
     ...
     */
    calculateTemplateConfidence(template, analysis) {
     ...
     */
    getVariableValue(variable, inquiry, analysis) {
        switch (variable) {
            case 'customer_name':
     ...
     * Personalize response
     */
    personalizeResponse(response, inquiry, analysis) {
     ...
     */
    requiresApproval(response, analysis, confidence) {
        ...
        const hasHighRiskKeywords = highRiskKeywords.some(keyword => response.toLowerCase().includes(keyword));
     ...
     * Get approval reason
     */
    getApprovalReason(response, analysis, confidence) {
        if (confidence < 0.8) {
            return 'Low confidence in response quality';
        }
        const highRiskKeywords = ['refund', 'cancel', 'complaint', 'escalate', 'manager'];
        const foundKeywords = highRiskKeywords.filter(keyword => response.toLowerCase().includes(keyword));
     ...
     * Approve a response
     */
    async approveResponse(responseId, approverId, finalResponse) {
        try {
     ...
     * Reject a response
     */
    async rejectResponse(responseId, approverId, rejectionReason) {
        try {
            const { data: response, error } = await this.supabase
                .from('automated_responses')
                .update({
     ...
     */
    async updateTemplateSuccess(templateId, success) {
        try {
...
//# sourceMappingURL=response-automation.service.js.map