 ...
 */
export async function optimizeTemplates(timeRange, supabaseUrl, supabaseKey) {
    ...
    try {
        ...
        const records = data
            ...
            if (!payload.grades || !payload.template)
                return null;
            return {
                id: row.id,
        ...
        const recommendations = generateRecommendations(templatePerformances, patterns, topTemplates, lowPerformingTemplates);
        // Prepare A/B test candidates
        const abTestCandidates = prepareABTests(lowPerformingTemplates, topTemplates, patterns);
        return {
            topTemplates,
 ...
 */
function calculateTemplatePerformances(records) {
 ...
 */
function calculateAvgGrade(records) {
    if (records.length === 0)
        return 0;
 ...
 */
function extractPatterns(records, performances) {
 ...
 */
function generateRecommendations(performances, patterns, topTemplates, lowPerformingTemplates) {
    const recommendations = [];
    // Top template recommendation
    if (topTemplates.length > 0) {
        recommendations.push(`Prioritize using "${topTemplates[0].templateName}" template (${topTemplates[0].averageGrade}/5.0 avg grade, ${topTemplates[0].usageCount} uses)`);
    }
    // Pattern-based recommendations
    if (patterns.length > 0) {
        const topPattern = patterns[0];
        recommendations.push(`High performers use ${topPattern.description.toLowerCase()} (${topPattern.frequency}/${topTemplates.length} templates, +${topPattern.avgGradeImprovement} grade improvement)`);
    }
    // Low performer recommendations
    if (lowPerformingTemplates.length > 0) {
        recommendations.push(`Review and improve "${lowPerformingTemplates[0].templateName}" template (${lowPerformingTemplates[0].averageGrade}/5.0 avg grade)`);
    }
    // Trend-based recommendations
    const improvingTemplates = performances.filter((p) => p.trend === "improving");
    if (improvingTemplates.length > 0) {
        recommendations.push(`${improvingTemplates.length} template(s) showing improvement - monitor and document successful changes`);
    }
    const decliningTemplates = performances.filter((p) => p.trend === "declining");
    if (decliningTemplates.length > 0) {
        recommendations.push(`${decliningTemplates.length} template(s) declining in quality - immediate review recommended`);
    }
    ...
    if (topUsagePercent < 50) {
        recommendations.push(`High-performing templates account for only ${topUsagePercent.toFixed(0)}% of usage - increase their utilization`);
    }
 ...
 * Prepare A/B test candidates
 */
function prepareABTests(lowPerformers, topPerformers, patterns) {
    const candidates = [];
    ...
    for (let i = 0; i < Math.min(lowPerformers.length, 3); i++) {
        ...
        candidates.push({
            ...
            confidenceLevel: expectedImprovement > 0.5
                ? "high"
 ...
 */
function createEmptyOptimization(timeRange) {
    return {
        topTemplates: [],
        lowPerformingTemplates: [],
        patterns: [],
...
//# sourceMappingURL=template-optimizer.js.map