 ...
 * ```
 */
export async function getAgentPerformance(agent, days = 7) {
    try {
        const since = new Date();
        since.setDate(since.getDate() - days);
        // Determine action filter based on agent type
        const actionFilter = agent === "customer" ? "chatwoot.approve_send" : "ceo.execute_action";
        ...
        const draftApprovalRate = draftsGenerated > 0 ? (draftsApproved / draftsGenerated) * 100 : 0;
        const avgDraftGenerationTime = generationTimeCount > 0 ? totalGenerationTime / generationTimeCount : 0;
        const avgApprovalTime = approvalTimeCount > 0
            ? totalApprovalTime / approvalTimeCount / 60 // Convert seconds to minutes
            : 0;
        const avgEditDistance = editDistanceCount > 0 ? totalEditDistance / editDistanceCount : 0;
 ...
 */
function calculateImprovementTrend(gradesOverTime) {
    ...
    const avgFirstHalf = firstHalf.reduce((sum, item) => sum + item.grade, 0) / firstHalf.length;
    const avgSecondHalf = secondHalf.reduce((sum, item) => sum + item.grade, 0) / secondHalf.length;
...
//# sourceMappingURL=agent-performance.js.map