 ...
 */
export async function queryActionAttribution(actionIds, startDate, endDate) {
 ...
 */
export async function calculateAttributionWindows(actionIds, endDate) {
    const windows = {};
 ...
 */
export async function generateAttributionPanelData(actions, startDate, endDate) {
    ...
    const averageROI = totalActions > 0
        ...
        : 0;
    const topPerformer = processedActions.reduce((top, current) => current.actualImpact['28d'].revenue > (top?.actualImpact['28d'].revenue || 0) ? current : top, null);
 ...
 */
function calculateConfidenceScore(expected, actual) {
    let score = 0;
    let factors = 0;
 ...
 * Get date N days ago
 */
function getDateDaysAgo(endDate, days) {
 ...
 */
function getMockAttributionData(actionIds) {
    const mockData = {};
