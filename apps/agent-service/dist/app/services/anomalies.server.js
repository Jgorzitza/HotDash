 ...
 */
export function detectAnomaly(currentValue, config, baselineValue) {
 ...
 */
export async function calculateBaseline(shopDomain, factType, metricPath, // e.g., "totalRevenue" or nested "metadata.breachRate"
windowDays = 7) {
 ...
 */
function extractMetricValue(json, path) {
 ...
 */
export async function forecast(shopDomain, factType, metricPath, params) {
    const cutoffDate = new Date();
    ...
    if (params.method === "simple_average") {
        ...
        const stdDev = Math.sqrt(values.reduce((sum, val) => sum + Math.pow(val - average, 2), 0) /
            values.length);
 ...
 */
export async function recordAnomalyDetection(shopDomain, anomalyType, anomalyResult, additionalMetadata) {
...
//# sourceMappingURL=anomalies.server.js.map