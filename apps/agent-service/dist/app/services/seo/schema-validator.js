 ...
 */
function identifySchemaType(schema) {
 ...
 */
function checkRequiredFields(schema, schemaType) {
    const issues = [];
...
function validateSchema(schema, url) {
    ...
    const hasRequiredFields = issues.filter((i) => i.type === "error" && i.message.includes("Required field")).length === 0;
    return {
        url,
        schemaType,
        isValid: !hasErrors,
 ...
 */
export async function validateSchemaMarkup(urls) {
    ...
    try {
        ...
        for (let i = 0; i < urls.length; i += batchSize) {
            ...
            const batchResults = await Promise.all(batch.map((url) => validatePageSchema(url)));
        ...
        const pagesWithErrors = allResults.filter((r) => r.issues.some((i) => i.type === "error")).length;
        const pagesWithWarnings = allResults.filter((r) => r.issues.some((i) => i.type === "warning")).length;
        const schemaTypeDistribution = {
            Product: 0,
            Organization: 0,
            WebSite: 0,
            BreadcrumbList: 0,
