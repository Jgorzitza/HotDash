 ...
 */
export async function fetchEnhancedMetrics(startDate, endDate, dimensions = [
    "query",
    "page",
], limit = 1000) {
    ...
    try {
        ...
        const metrics = (response.data.rows || []).map((row) => {
            ...
            dimensions.forEach((dim, index) => {
                if (dim === "query")
                    result.query = keys[index];
                else if (dim === "page")
                    result.page = keys[index];
                else if (dim === "device")
                    result.device = keys[index];
                else if (dim === "country")
                    result.country = keys[index];
            });
            return result;
        });
        return metrics;
 ...
 */
export async function fetchHistoricalTrend(query, page, startDate, endDate) {
    const startTime = Date.now();
 ...
 */
export async function refreshDailyData() {
    const startTime = Date.now();
    try {
 ...
 */
export async function compareTimeRanges(currentStart, currentEnd, previousStart, previousEnd) {
    // Fetch both periods
...
//# sourceMappingURL=search-console-enhanced.js.map