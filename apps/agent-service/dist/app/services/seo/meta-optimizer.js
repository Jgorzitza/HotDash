...
function analyzePageContent(content) {
    ...
    const headings = [...h1Matches, ...h2Matches].map(h => h.replace(/<[^>]+>/g, '').trim());
 ...
 */
function generateOptimizedTitle(originalTitle, headings, keywords, topQueries) {
 ...
 */
function generateOptimizedDescription(originalDescription, content, keywords, topQueries) {
 ...
 */
function calculateOptimizationScore(title, description, keywords) {
    let score = 100;
    // Title scoring
    if (title.length < 30)
        score -= 15;
 ...
 */
export async function optimizeMetaTags(pageContent) {
    ...
    try {
        ...
        const optimizedTitle = generateOptimizedTitle(pageContent.title, analysis.headings, analysis.keywords, topQueries);
        // Generate optimized description
        const optimizedDescription = generateOptimizedDescription(pageContent.description, pageContent.content, analysis.keywords, topQueries);
        // Calculate score
        const score = calculateOptimizationScore(optimizedTitle, optimizedDescription, analysis.keywords);
 ...
 */
export async function batchOptimizeMetaTags(pages) {
    const results = new Map();
    // Process pages in parallel (limit concurrency to avoid overwhelming the system)
    const batchSize = 5;
    for (let i = 0; i < pages.length; i += batchSize) {
        const batch = pages.slice(i, i + batchSize);
        const batchResults = await Promise.all(batch.map(page => optimizeMetaTags(page)));
