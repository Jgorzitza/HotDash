 ...
 */
function calculateOverallScore(rankings, webVitals, siteHealth) {
    ...
    // Weighted average
    const score = Math.round(rankingsScore * 0.4 + vitalsScore * 0.3 + healthScore * 0.3);
    let grade;
    if (score >= 90)
        grade = "A";
    else if (score >= 80)
 ...
 */
function identifyKeyChanges(rankings, webVitals, siteHealth) {
    const changes = [];
    // Ranking changes
 ...
 */
function identifyWinsAndIssues(rankings) {
 ...
 */
function generateRecommendations(rankings, webVitals, siteHealth, opportunities) {
    const recommendations = [];
    // Rankings recommendations
    if (rankings.avgPosition > 15) {
        recommendations.push("Focus on improving keyword rankings - currently averaging position " +
            Math.round(rankings.avgPosition));
    }
    // Web Vitals recommendations
    if (webVitals.mobile.lcp > 2500) {
        recommendations.push("Optimize mobile LCP (currently " +
 ...
 */
export async function generateWeeklyReport(domain, options) {
 ...
 */
export async function getReportSummary(domain) {
    const report = await generateWeeklyReport(domain);
    return report.summary;
}
//# sourceMappingURL=seo-reporting.js.map