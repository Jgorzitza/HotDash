 ...
 */
function calculatePageAuthority(url, incomingLinks, allPages, iterations = 5) {
    if (incomingLinks.size === 0)
        return MIN_PAGE_AUTHORITY;
 ...
 */
function generateLinkingRecommendations(page, allPages, contentMap) {
 ...
 */
export async function analyzeInternalLinking(pages) {
    ...
    try {
        ...
        pageProfiles.forEach((profile) => {
            profile.recommendations = generateLinkingRecommendations(profile, pageProfiles, contentMap);
        });
        // Calculate summary
        const totalInternalLinks = pageProfiles.reduce((sum, p) => sum + p.outgoingLinks, 0);
        const avgIncomingLinks = pageProfiles.reduce((sum, p) => sum + p.incomingLinks, 0) /
            pageProfiles.length;
 ...
 */
export async function getPageLinkingRecommendations(url, allPages) {
