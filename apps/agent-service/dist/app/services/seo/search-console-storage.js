...
import { getSearchAnalytics, getTopQueries, getLandingPages, } from "../../lib/seo/search-console";
// ============================================================================
// Store Functions
// ============================================================================
/**
 ...
 */
export async function storeSearchConsoleMetrics(metrics, periodDays = 30) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    console.log(`[Search Console Storage] Storing metrics for ${today.toISOString().split("T")[0]} (${periodDays}d)`);
    ...
    console.log(`[Search Console Storage] ✅ Metrics stored: ${metrics.clicks} clicks, ${metrics.impressions} impressions`);
}
/**
 ...
 * @param periodDays - Period (default: 30 days)
 */
export async function storeTopQueries(queries, periodDays = 30) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    console.log(`[Search Console Storage] Storing ${queries.length} top queries for ${today.toISOString().split("T")[0]}`);
 ...
 * @param periodDays - Period (default: 30 days)
 */
export async function storeLandingPages(pages, periodDays = 30) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    console.log(`[Search Console Storage] Storing ${pages.length} landing pages for ${today.toISOString().split("T")[0]}`);
...
export async function storeSearchConsoleSummary() {
    console.log("[Search Console Storage] ========================================");
    ...
    try {
        ...
        console.log(`[Search Console Storage] Fetched: ${metrics.clicks} clicks, ${queries.length} queries, ${pages.length} pages`);
        // 2. Store to Supabase
        await Promise.all([
        ...
        console.log("[Search Console Storage] ✅ Complete summary stored successfully");
        return {
            success: true,
            metrics,
...
export async function getHistoricalMetrics(days = 30) {
    console.log(`[Search Console Storage] Fetching ${days} days of historical metrics`);
    const records = await prisma.seoSearchConsoleMetrics.findMany({
        where: { periodDays: 30 },
        orderBy: { date: "desc" },
        take: days,
    });
    console.log(`[Search Console Storage] Found ${records.length} historical metric records`);
    return records;
}
/**
 ...
 */
export async function getQueryTrend(query, days = 30) {
    console.log(`[Search Console Storage] Fetching ${days}-day trend for query: "${query}"`);
    const records = await prisma.seoSearchQuery.findMany({
        where: { query, periodDays: 30 },
        orderBy: { date: "desc" },
        take: days,
    });
    console.log(`[Search Console Storage] Found ${records.length} records for query "${query}"`);
    return records;
}
/**
 ...
 */
export async function getLandingPageTrend(url, days = 30) {
    console.log(`[Search Console Storage] Fetching ${days}-day trend for page: "${url}"`);
    const records = await prisma.seoLandingPage.findMany({
        where: { url, periodDays: 30 },
        orderBy: { date: "desc" },
        take: days,
    });
    console.log(`[Search Console Storage] Found ${records.length} records for page "${url}"`);
    return records;
}
/**
 ...
 */
export async function getTopQueriesByDate(date = new Date(), limit = 10) {
    const dateOnly = new Date(date);
    dateOnly.setHours(0, 0, 0, 0);
    console.log(`[Search Console Storage] Fetching top ${limit} queries for ${dateOnly.toISOString().split("T")[0]}`);
    const records = await prisma.seoSearchQuery.findMany({
        where: { date: dateOnly, periodDays: 30 },
        orderBy: { rank: "asc" },
        take: limit,
    });
    console.log(`[Search Console Storage] Found ${records.length} queries for date ${dateOnly.toISOString().split("T")[0]}`);
    return records;
}
/**
 ...
 */
export async function getTopLandingPagesByDate(date = new Date(), limit = 10) {
    const dateOnly = new Date(date);
    dateOnly.setHours(0, 0, 0, 0);
    console.log(`[Search Console Storage] Fetching top ${limit} pages for ${dateOnly.toISOString().split("T")[0]}`);
    const records = await prisma.seoLandingPage.findMany({
        where: { date: dateOnly, periodDays: 30 },
        orderBy: { rank: "asc" },
        take: limit,
    });
    console.log(`[Search Console Storage] Found ${records.length} pages for date ${dateOnly.toISOString().split("T")[0]}`);
    return records;
}
//# sourceMappingURL=search-console-storage.js.map