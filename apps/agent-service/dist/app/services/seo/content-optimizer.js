 ...
 */
function extractPhrases(text, minLength, maxLength) {
 ...
 */
function analyzeHeadings(html, targetKeyword) {
 ...
 */
function analyzeLinks(html, baseUrl, wordCount) {
 ...
 */
function calculateSEOScore(readability, keywords, headings, links, images) {
    ...
    if (headings.h2Count >= 2)
        headingsScore += 5;
    if (headings.h3Count >= 1)
        headingsScore += 5;
    // Links (0-20): Proper internal linking
    let linksScore = 0;
    if (links.hasProperLinking)
        linksScore += 15;
    else if (links.internalLinks > 0)
        linksScore += 8;
    if (links.externalLinks >= 1 && links.externalLinks <= 3)
        linksScore += 5;
    // Images (0-20): Alt text quality
    let imagesScore = 0;
    if (images.altTextQuality === "good")
        imagesScore = 20;
    else if (images.altTextQuality === "fair")
        imagesScore = 12;
 ...
 */
export async function analyzeContent(url, html, targetKeyword) {
 ...
 */
export async function analyzeContentFromURL(url, targetKeyword) {
    try {
...
//# sourceMappingURL=content-optimizer.js.map