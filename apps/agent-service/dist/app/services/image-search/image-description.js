 ...
 */
export async function batchGenerateDescriptions(imageUrls, context) {
    const results = [];
    // Process in batches of 5 to avoid rate limits
    const batchSize = 5;
    for (let i = 0; i < imageUrls.length; i += batchSize) {
        const batch = imageUrls.slice(i, i + batchSize);
        const batchResults = await Promise.all(batch.map((url) => generateImageDescription(url, context)));
 ...
 */
export function generateSearchableDescription(fullDescription, labels) {
...
//# sourceMappingURL=image-description.js.map