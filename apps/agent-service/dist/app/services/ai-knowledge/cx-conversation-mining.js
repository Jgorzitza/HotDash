 ...
 */
async function extractChatwootConversations(days = 30) {
...
export async function embedConversations() {
    ...
    for (const conv of conversations) {
        ...
        const sanitizedMessages = sanitizeConversation(conv.messages.map((m) => ({
            content: m.content,
 ...
 */
export async function detectRecurringThemes(minOccurrences = 3, days = 7) {
    const since = new Date();
    since.setDate(since.getDate() - days);
    const themes = [];
    ...
    `;
        if (similar.length >= minOccurrences) {
            ...
            const topProduct = Array.from(productCounts.entries()).sort((a, b) => b[1] - a[1])[0];
            if (topProduct) {
                themes.push({
                    theme: pattern,
                    ...
                    firstSeen: new Date(Math.min(...similar.map((s) => s.created_at.getTime()))),
                    lastSeen: new Date(Math.max(...similar.map((s) => s.created_at.getTime()))),
                });
            }
        }
...
export async function runNightlyCXMining() {
    console.log("[CX Mining] Starting nightly conversation analysis", new Date().toISOString());
    try {
        // 1. Embed new conversations
...
//# sourceMappingURL=cx-conversation-mining.js.map