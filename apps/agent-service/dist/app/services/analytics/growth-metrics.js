 ...
 */
export async function getGrowthMetrics(shopDomain = "occ", days = 30) {
 ...
 */
export async function getWeeklyGrowthReport(shopDomain = "occ", weeks = 4) {
    const reports = [];
    ...
    for (let i = 0; i < weeks; i++) {
        ...
        const weekMetrics = await getMetricsForPeriod(shopDomain, weekStart, weekEnd);
        const prevWeekStart = new Date(weekStart);
        prevWeekStart.setDate(prevWeekStart.getDate() - 7);
        const prevWeekEnd = new Date(weekEnd);
        prevWeekEnd.setDate(prevWeekEnd.getDate() - 7);
        const prevWeekMetrics = await getMetricsForPeriod(shopDomain, prevWeekStart, prevWeekEnd);
        // Calculate week-over-week growth
        const growth = {
            impressions: calculateGrowth(weekMetrics.totalImpressions, prevWeekMetrics.totalImpressions),
            clicks: calculateGrowth(weekMetrics.totalClicks, prevWeekMetrics.totalClicks),
            conversions: calculateGrowth(weekMetrics.totalConversions, prevWeekMetrics.totalConversions),
            revenue: calculateGrowth(weekMetrics.totalRevenue, prevWeekMetrics.totalRevenue),
        };
        reports.push({
 ...
 */
export async function getTrendAnalysis(shopDomain = "occ", days = 30) {
    const trends = [];
    ...
    const currentMetrics = await getMetricsForPeriod(shopDomain, currentStart, currentEnd);
    const previousMetrics = await getMetricsForPeriod(shopDomain, previousStart, previousEnd);
 ...
 */
export async function getDashboardMetrics(shopDomain = "occ") {
 ...
 */
async function getMetricsForPeriod(shopDomain, start, end) {
    const socialMetrics = await prisma.dashboardFact.findMany({
        where: {
            shopDomain,
            factType: "social_performance",
 ...
 */
function aggregateSocialMetrics(metrics) {
 ...
 */
function aggregateAdsMetrics(metrics) {
    return metrics.reduce((acc, metric) => {
        const value = metric.value;
        return {
            impressions: acc.impressions + (value.impressions || 0),
 ...
 */
function identifyTopPerformers(trends) {
    return trends
        .filter((t) => t.trend === "up")
...
//# sourceMappingURL=growth-metrics.js.map