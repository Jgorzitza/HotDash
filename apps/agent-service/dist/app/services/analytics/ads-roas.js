 ...
 * ROAS = Revenue / Spend
 */
export async function calculateCampaignROAS(campaignId, campaignName, platform, spend, revenue, impressions, clicks, conversions, shopDomain = "occ") {
    // Calculate metrics
 ...
 */
function determineStatus(roas) {
    if (roas > 2.0)
        return "profitable"; // Making money
    if (roas >= 1.0)
        return "break-even"; // Recovering costs
    return "unprofitable"; // Losing money
}
/**
 * Generate actionable recommendation based on performance
 */
function generateRecommendation(performance) {
 ...
 */
export async function getROASSummary(shopDomain = "occ", platform, days = 30) {
    ...
    // Calculate totals
    const totals = latestCampaigns.reduce((acc, campaign) => ({
        spend: acc.spend + (campaign.spend || 0),
        revenue: acc.revenue + (campaign.revenue || 0),
    }), { spend: 0, revenue: 0 });
    const overallROAS = totals.spend > 0 ? Number((totals.revenue / totals.spend).toFixed(2)) : 0;
    const profitableCampaigns = latestCampaigns.filter((c) => c.status === "profitable").length;
    const unprofitableCampaigns = latestCampaigns.filter((c) => c.status === "unprofitable").length;
 ...
 */
export async function getCampaignPerformance(campaignId, shopDomain = "occ", days = 90) {
    const since = new Date();
    since.setDate(since.getDate() - days);
 ...
 */
export async function compareCampaigns(campaignIds, shopDomain = "occ") {
    const campaigns = await prisma.dashboardFact.findMany({
        where: {
            shopDomain,
            factType: "ads_roas",
...
//# sourceMappingURL=ads-roas.js.map