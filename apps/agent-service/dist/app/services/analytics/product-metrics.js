...
export class ProductMetricsService {
     ...
     * @returns WAU count
     */
    async calculateWAU(endDate = new Date()) {
     ...
     * @returns MAU count
     */
    async calculateMAU(endDate = new Date()) {
     ...
     */
    async calculateProductHealthScore() {
        ...
        const featureMetrics = await productAnalyticsService.getFeatureAdoptionMetrics(startDate, endDate);
        ...
        const totalEngagement = segmentAnalytics.reduce((sum, s) => sum + s.avgEngagementScore * s.userCount, 0);
        ...
        const modalMetrics = await productAnalyticsService.getModalActionMetrics(startDate, endDate);
    ...
    async getProductMetrics() {
        ...
        const segmentAnalytics = await userSegmentationService.getSegmentAnalytics();
        const totalSessions = segmentAnalytics.reduce((sum, s) => sum + s.avgSessionsPerUser * s.userCount, 0);
        ...
        const activeFeatures = featureMetrics.filter((f) => f.adoptionRate >= 0.1).length;
...
//# sourceMappingURL=product-metrics.js.map