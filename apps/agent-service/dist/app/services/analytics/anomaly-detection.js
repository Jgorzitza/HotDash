 ...
 */
export async function detectAnomalies(metric, shopDomain = "occ", days = 30) {
    // Get historical data
    ...
    for (const dataPoint of recentData) {
        ...
        if (absZScore > 2.5) {
            const anomaly = {
                ...
                recommendation: generateAnomalyRecommendation(metric, zScore > 0 ? "spike" : "drop", absZScore),
 ...
 */
export async function detectAllAnomalies(shopDomain = "occ", days = 30) {
    const metrics = [
        "revenue",
        "ctr",
        "conversions",
    ...
    const criticalCount = allAnomalies.filter((a) => a.significance === "critical").length;
    return {
 ...
 */
async function getMetricHistory(metric, shopDomain, days) {
 ...
 */
function classifySignificance(absZScore) {
    if (absZScore > 4.0)
        return "critical";
    if (absZScore > 3.0)
        return "moderate";
 ...
 */
function generateAnomalyRecommendation(metric, type, zScore) {
    if (metric === "revenue" && type === "drop") {
        return `CRITICAL: Revenue drop detected (${zScore.toFixed(1)}σ below normal). Immediate investigation required. Check for order cancellations or payment issues.`;
    }
    if (metric === "ctr" && type === "spike") {
        return `Positive: CTR spike detected (${zScore.toFixed(1)}σ above normal). Analyze what drove this increase and replicate the strategy.`;
    }
    if (metric === "ctr" && type === "drop") {
        return `Warning: CTR drop detected (${zScore.toFixed(1)}σ below normal). Review ad creative, targeting, and bidding strategy.`;
    }
    if (metric === "conversions" && type === "drop") {
        return `Alert: Conversion drop detected (${zScore.toFixed(1)}σ below normal). Check checkout flow, pricing, and landing pages.`;
    }
    if (metric === "conversions" && type === "spike") {
        return `Positive: Conversion spike detected. Identify and maintain successful strategies.`;
    }
    if (type === "spike") {
        return `${metric} spike detected (${zScore.toFixed(1)}σ above normal). Monitor to ensure data accuracy and identify drivers.`;
    }
    return `${metric} drop detected (${zScore.toFixed(1)}σ below normal). Investigate root cause and implement corrective actions.`;
}
/**
 * Generate summary of all anomalies
 */
function generateAlertSummary(anomalies) {
