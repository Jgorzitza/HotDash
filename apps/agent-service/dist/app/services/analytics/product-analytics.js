...
export class ProductAnalyticsService {
     ...
     */
    async getFeatureAdoptionMetrics(startDate, endDate) {
        ...
        for (const [featureName, data] of featureMap.entries()) {
            ...
            const avgUsagePerUser = activeUsers > 0 ? data.usageCount / activeUsers : 0;
            metrics.push({
                featureName,
                totalUsers,
                activeUsers,
     ...
     */
    async getTileEngagementMetrics(startDate, endDate) {
        // Get tile events
        ...
        for (const [tileName, data] of tileMap.entries()) {
            ...
            const avgTimeToClick = data.timesToClick.length > 0
     ...
     */
    async getModalActionMetrics(startDate, endDate) {
        ...
        for (const [modalType, data] of modalMap.entries()) {
            ...
            const approvalRate = totalDecisions > 0 ? data.approves / totalDecisions : 0;
            const avgDecisionTime = data.decisionTimes.length > 0
     ...
     */
    async getSettingsChangeMetrics(startDate, endDate) {
     ...
     */
    async getTopFeatures(limit = 10, startDate, endDate) {
     ...
     */
    async getUnusedFeatures(threshold = 0.1, startDate, endDate) {
        const start = startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000); // 30 days ago
        const end = endDate || new Date();
        const allMetrics = await this.getFeatureAdoptionMetrics(start, end);
        // Filter for low adoption or no recent usage (last 7 days)
        const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        return allMetrics.filter((m) => m.adoptionRate < threshold || (m.lastUsed && m.lastUsed < sevenDaysAgo));
    }
    /**
    ...
    async generateInsights(startDate, endDate) {
        const insights = [];
        // Get metrics
        const featureMetrics = await this.getFeatureAdoptionMetrics(startDate, endDate);
        const tileMetrics = await this.getTileEngagementMetrics(startDate, endDate);
        const unusedFeatures = await this.getUnusedFeatures(0.1, startDate, endDate);
        ...
        const highRejectionModals = modalMetrics.filter((m) => m.approvalRate < 0.5); // < 50% approval
