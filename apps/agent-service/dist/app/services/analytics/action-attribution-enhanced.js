 ...
 */
export async function getEnhancedActionAttribution(actionKey, periodDays) {
    ...
    try {
        ...
        const recentRevenue = trendData.reduce((sum, row) => sum + parseFloat(row.metricValues?.[4]?.value || "0"), 0);
        const olderRevenue = response.rows.slice(-7).reduce((sum, row) => sum + parseFloat(row.metricValues?.[4]?.value || "0"), 0);
        ...
        const attributionPath = Array.from(new Set(response.rows.map(row => `${row.dimensionValues?.[3]?.value || 'unknown'}/${row.dimensionValues?.[4]?.value || 'unknown'}`)));
 ...
 */
export async function generateAttributionInsights(attributionData) {
 ...
 */
export async function generateAttributionReport(timeframe) {
...
// Helper Functions
// ============================================================================
function identifyEmergingTrends(data) {
    const trends = [];
...
function generateOptimizationOpportunities(data) {
    const opportunities = [];
    data.forEach(action => {
...
function calculatePredictedROI(data) {
    if (data.length === 0)
        return 0;
...
//# sourceMappingURL=action-attribution-enhanced.js.map