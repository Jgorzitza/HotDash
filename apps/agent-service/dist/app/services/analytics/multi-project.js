 ...
 */
export async function getProjectMetrics(shopDomain, days = 30) {
 ...
 * CEO/agency view
 */
export async function getMultiProjectSummary(days = 30) {
    const since = new Date();
    since.setDate(since.getDate() - days);
    ...
    const projectMetrics = await Promise.all(uniqueShops.map((shop) => getProjectMetrics(shop, days)));
    ...
    const aggregateMetrics = projectMetrics.reduce((acc, project) => ({
    ...
    aggregateMetrics.avgCTR =
        aggregateMetrics.totalImpressions > 0
            ? Number(((aggregateMetrics.totalClicks / aggregateMetrics.totalImpressions) *
                100).toFixed(2))
            : 0;
    aggregateMetrics.avgConversionRate =
        aggregateMetrics.totalClicks > 0
            ? Number(((aggregateMetrics.totalConversions / aggregateMetrics.totalClicks) *
                100).toFixed(2))
            : 0;
 ...
 */
export async function compareProjects(project1, project2, days = 30) {
 ...
 */
export async function getTopProjectsByMetric(metric, limit = 10, days = 30) {
 ...
 */
export async function getProjectRankings(days = 30) {
    const summary = await getMultiProjectSummary(days);
 ...
 */
function getPerformanceGrade(roas) {
    if (roas >= 4.0)
        return "A";
    if (roas >= 3.0)
        return "B";
