...
export async function exportTrafficToCSV() {
...
export async function exportProductsToCSV() {
    ...
    const rows = products.map((p) => `"${p.productName}",${p.productId},${p.views},${p.addToCarts},${p.purchases},${p.revenue.toFixed(2)},${p.addToCartRate.toFixed(2)}%,${p.purchaseRate.toFixed(2)}%,${p.avgPrice.toFixed(2)}`);
    return [headers, ...rows].join("\n");
}
export async function exportUTMToCSV() {
    const utm = await getUTMBreakdown();
    const headers = "Source,Medium,Campaign,Sessions,Users,Conversions,Revenue,Conversion Rate";
    const rows = utm.map((u) => `"${u.source}","${u.medium}","${u.campaign}",${u.sessions},${u.users},${u.conversions},${u.revenue.toFixed(2)},${u.conversionRate.toFixed(2)}%`);
    return [headers, ...rows].join("\n");
}
export function generateCSVFilename(reportType) {
