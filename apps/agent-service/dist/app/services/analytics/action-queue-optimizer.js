 ...
 */
export function calculateV1Score(factors) {
 ...
 */
export function calculateV2Score(factors) {
    const easeMultiplier = getEaseMultiplier(factors.ease);
 ...
 */
export function calculateV3Score(factors) {
    const easeMultiplier = getEaseMultiplier(factors.ease);
...
export class ActionQueueOptimizer {
     ...
     */
    static calculateScore(factors, version = 'v3_ml') {
        let score;
        let mlScore;
        switch (version) {
            case 'v1_basic':
     ...
     */
    static async rerankActions(version = 'v3_ml') {
        ...
        for (const action of actions) {
            const factors = {
                expectedImpact: action.expected_impact?.delta || 0,
     ...
     */
    static async updateHistoricalMetrics(actionType) {
        ...
        const successCount = executedActions.filter(a => a.execution_result && a.execution_result.success === true).length;
        const avgRealizedROI = executedActions.reduce((sum, a) => sum + (a.realized_revenue_28d ? Number(a.realized_revenue_28d) : 0), 0) / executionCount;
    ...
    static async runABTest() {
        ...
        const avgScoreDelta = (v3Results.reduce((sum, r) => sum + r.score, 0) / v3Results.length -
...
//# sourceMappingURL=action-queue-optimizer.js.map