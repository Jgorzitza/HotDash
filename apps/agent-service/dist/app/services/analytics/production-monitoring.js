...
export class ProductionMonitoringService {
     ...
     */
    aggregateAnalyticsMetrics(facts) {
     ...
     */
    calculateHealthStatus(analytics, performance, funnel) {
        const issues = [];
        let score = 100;
     ...
     */
    async checkHealthAndAlerts(metrics) {
        // Check for critical issues
        if (metrics.health.status === 'critical') {
            this.alertManager.createAlert('critical', 'custom', 'Analytics health critical', {
        ...
        if (metrics.performance.errorRate > 2) {
            this.alertManager.createAlert('warning', 'error_rate', `Error rate elevated: ${metrics.performance.errorRate.toFixed(2)}%`, { errorRate: metrics.performance.errorRate, threshold: 2 });
        }
        // Check response time
        if (metrics.performance.avgResponseTime > 1000) {
            this.alertManager.createAlert('warning', 'performance', `Response time degraded: ${metrics.performance.avgResponseTime.toFixed(0)}ms`, { responseTime: metrics.performance.avgResponseTime, threshold: 1000 });
        }
    }
    /**
     ...
     * Generate recommendations
     */
    generateRecommendations(metrics, anomalies, alerts) {
