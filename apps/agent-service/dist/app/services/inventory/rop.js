...
import { calculateSeasonalAdjustedSales, } from "~/lib/inventory/seasonality";
/**
...
export function calculateReorderPoint(params) {
    const { avgDailySales, leadTimeDays, maxDailySales, maxLeadDays, category = "general", currentMonth, } = params;
    // Validate inputs
    ...
    const adjustedDailySales = calculateSeasonalAdjustedSales(avgDailySales, category, currentMonth);
    // Calculate seasonality factor for transparency
    const seasonalityFactor = avgDailySales > 0 ? adjustedDailySales / avgDailySales : 1.0;
    // Also adjust max daily sales proportionally for safety stock calculation
    const adjustedMaxDailySales = maxDailySales > 0 ? maxDailySales * seasonalityFactor : maxDailySales;
 ...
 */
export function getInventoryStatus(currentInventory, rop) {
