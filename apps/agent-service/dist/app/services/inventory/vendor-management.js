 ...
 */
export function calculateLeadTime(orderDate, deliveryDate) {
 ...
 */
export function isOnTimeDelivery(expectedDate, actualDate, gracePeriodDays = 1) {
 ...
 */
export function calculateVendorPerformance(vendor, orders) {
    ...
    const expectedLeadTimes = completedOrders.map((o) => calculateLeadTime(o.order_date, o.expected_delivery_date));
    const expectedLeadTime = expectedLeadTimes.reduce((sum, lt) => sum + lt, 0) /
        expectedLeadTimes.length;
    // Calculate lead time variance (standard deviation)
    const leadTimeVariance = Math.sqrt(leadTimes
    ...
    for (const order of completedOrders) {
        if (order.actual_delivery_date) {
            if (isOnTimeDelivery(order.expected_delivery_date, order.actual_delivery_date)) {
                onTimeCount++;
            }
            else {
                lateCount++;
    ...
    const lastOrderDate = sortedOrders.length > 0 ? sortedOrders[0].order_date : undefined;
    const deliveredOrders = completedOrders.filter((o) => o.actual_delivery_date);
    const sortedDeliveries = deliveredOrders.sort((a, b) => b.actual_delivery_date.getTime() - a.actual_delivery_date.getTime());
    const lastDeliveryDate = sortedDeliveries.length > 0
 ...
 */
export function calculateVendorScore(metrics, benchmarkLeadTime, benchmarkCost) {
    ...
    const compositeScore = reliabilityScore * 0.5 + // 50% weight on reliability
 ...
 */
export function compareVendorsForSKU(sku, vendors, benchmarkLeadTime, benchmarkCost) {
    // Calculate benchmark values if not provided
    const avgLeadTime = benchmarkLeadTime ??
        vendors.reduce((sum, v) => sum + v.metrics.average_lead_time_days, 0) /
            vendors.length;
    const avgCost = benchmarkCost ??
...
export function rankVendors(vendors) {
    ...
    const avgLeadTime = vendors.reduce((sum, v) => sum + v.metrics.average_lead_time_days, 0) /
        vendors.length;
    const avgCost = vendors.reduce((sum, v) => sum + v.metrics.average_cost_per_unit, 0) /
        vendors.length;
 ...
 */
export function identifyVendorIssues(metrics) {
    const issues = [];
    // Check reliability
    ...
    if (metrics.last_order_date) {
        const daysSinceLastOrder = (Date.now() - metrics.last_order_date.getTime()) / (1000 * 60 * 60 * 24);
 ...
 */
export async function getVendorInfo(productId) {
...
//# sourceMappingURL=vendor-management.js.map