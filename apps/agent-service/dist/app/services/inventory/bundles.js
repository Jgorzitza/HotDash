 ...
 */
export function parseBundleMetafield(metafieldValue) {
 ...
 */
export async function getBundleComponents(bundleProductId) {
 ...
 */
export async function calculateBundleROP(bundleProductId, params) {
    ...
    const componentCapacities = await Promise.all(components.map(async (c) => {
 ...
 */
export async function getBundleInfo(productId) {
    ...
    const minAvailable = Math.min(...componentCapacities.map((cc) => cc.availableBundles));
    const limitingComponent = componentCapacities.find((cc) => cc.availableBundles === minAvailable);
    return {
        productId,
