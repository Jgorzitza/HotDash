...
export function createPurchaseOrder(params) {
    const { po_number, vendor_id, vendor_name, sku, product_name, quantity, cost_per_unit, expected_lead_time_days, notes, } = params;
    const total_cost = quantity * cost_per_unit;
    const created_date = new Date();
    // Calculate expected delivery date if lead time provided
    let expected_delivery_date = undefined;
    if (expected_lead_time_days && expected_lead_time_days > 0) {
        expected_delivery_date = new Date(created_date);
        expected_delivery_date.setDate(expected_delivery_date.getDate() + expected_lead_time_days);
    }
    return {
        po_number,
        vendor_id,
 ...
 */
export function markPOAsOrdered(po, ordered_date, expected_delivery_date) {
    if (po.status !== "draft") {
    ...
    return {
        ...
        expected_delivery_date: expected_delivery_date ?? po.expected_delivery_date ?? new Date(),
    };
}
/**
 * Mark PO as shipped
 ...
 * @returns Updated purchase order
 */
export function markPOAsShipped(po, shipped_date) {
 ...
 * @returns Updated purchase order
 */
export function markPOAsReceived(po, received_date) {
 ...
 * @returns Updated purchase order
 */
export function markPOAsCancelled(po, notes) {
 ...
 */
export function getPOTrackingDetails(po) {
    const now = new Date();
    const details = { ...po };
    ...
    if (po.ordered_date && po.expected_delivery_date) {
        details.expected_lead_time = calculateLeadTime(po.ordered_date, po.expected_delivery_date);
    }
    // Calculate actual lead time for received orders
    if (po.ordered_date && po.actual_delivery_date) {
        details.actual_lead_time = calculateLeadTime(po.ordered_date, po.actual_delivery_date);
        // Calculate variance
 ...
 */
export function getPOSummary(pos) {
    let total_value = 0;
    let draft_count = 0;
    let ordered_count = 0;
    let shipped_count = 0;
    ...
    const average_lead_time = leadTimes.length > 0
 ...
 */
export function getOverduePOs(pos) {
    return pos
 ...
 */
export function getPOsExpectedSoon(pos, days = 7) {
    return pos
        .filter((po) => po.status === "ordered" || po.status === "shipped")
        .map((po) => getPOTrackingDetails(po))
        .filter((details) => details.days_until_expected !== undefined &&
        details.days_until_expected >= 0 &&
        details.days_until_expected <= days)
        .sort((a, b) => (a.days_until_expected ?? Infinity) -
 ...
 */
export function calculateLeadTimeAccuracy(pos) {
    ...
    const accuracy_percentage = receivedPOs.length > 0
 ...
 * @returns CSV string
 */
export function exportPOsToCSV(pos) {
    const headers = [
        "PO Number",
        "Vendor",
        "SKU",
 ...
 */
export async function getPOTracking(productId) {
...
//# sourceMappingURL=po-tracking.js.map