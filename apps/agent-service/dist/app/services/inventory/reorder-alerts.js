 ...
 */
export function calculateEOQ(annualDemand, costPerUnit, setupCost = 50, holdingCostRate = 0.25) {
    // Validate inputs
 ...
 */
export function calculateDaysUntilStockout(currentStock, avgDailySales) {
    if (currentStock <= 0) {
        return 0;
    }
    if (avgDailySales <= 0) {
 ...
 */
export function determineAlertUrgency(currentStock, rop, daysUntilStockout) {
 ...
 */
export function calculateRecommendedOrderQty(currentStock, rop, safetyStock, eoq) {
...
export async function generateReorderAlert(product) {
    const { productId, productName, sku, currentStock, avgDailySales, leadTimeDays, maxDailySales, maxLeadDays, category = "general", costPerUnit = 25, // Default cost if not provided
     } = product;
    ...
    // Calculate days until stockout
    const daysUntilStockout = calculateDaysUntilStockout(currentStock, forecast.daily_forecast);
    // Determine urgency level
    const urgency = determineAlertUrgency(currentStock, ropResult.reorderPoint, daysUntilStockout);
    ...
    // Calculate recommended order quantity
    const recommendedOrderQty = calculateRecommendedOrderQty(currentStock, ropResult.reorderPoint, ropResult.safetyStock, eoqQty);
