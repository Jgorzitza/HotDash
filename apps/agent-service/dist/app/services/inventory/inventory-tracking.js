...
export class InventoryTrackingService {
    ...
    setupSocketHandlers() {
        this.io.on('connection', (socket) => {
            ...
            socket.on('subscribe-variant', (variantId) => {
            ...
            socket.on('unsubscribe-variant', (variantId) => {
            ...
            socket.on('request-inventory-update', async (variantId) => {
                try {
            ...
            socket.on('request-bulk-inventory-update', async (variantIds) => {
                try {
     ...
     */
    async trackInventoryChange(update) {
        try {
     ...
     */
    async getRealtimeInventoryData(variantId) {
        try {
            ...
            const availableStock = variant.inventory_levels.reduce((sum, level) => sum + level.available, 0);
            const committedStock = variant.inventory_levels.reduce((sum, level) => sum + (level.committed || 0), 0);
            const incomingStock = variant.inventory_levels.reduce((sum, level) => sum + (level.incoming || 0), 0);
            const realtimeData = {
     ...
     */
    async getBulkRealtimeInventoryData(variantIds) {
        try {
            ...
            const realtimeData = variants.map(variant => {
                const currentStock = variant.onHand || 0;
                const availableStock = variant.inventory_levels.reduce((sum, level) => sum + level.available, 0);
                const committedStock = variant.inventory_levels.reduce((sum, level) => sum + (level.committed || 0), 0);
                const incomingStock = variant.inventory_levels.reduce((sum, level) => sum + (level.incoming || 0), 0);
     ...
     */
    async checkAndCreateAlerts(variantId, currentStock) {
        try {
            const variant = await prisma.product_variant.findUnique({
                where: { id: variantId },
     ...
     */
    async getActiveAlerts(variantId) {
        try {
     ...
     */
    async getInventoryChangeHistory(variantId, limit = 50) {
        try {
     ...
     */
    async updateTrackingConfig(newConfig) {
        try {
...
//# sourceMappingURL=inventory-tracking.js.map