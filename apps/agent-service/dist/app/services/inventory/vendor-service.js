...
export class VendorService {
     ...
     */
    async getVendorWithMetrics(vendorId) {
        try {
     ...
     */
    async updateVendorReliability(vendorId, expectedDeliveryDate, actualDeliveryDate) {
        try {
            const vendor = await prisma.vendors.findUnique({
                where: { id: vendorId },
            });
            ...
            const actualLeadTime = Math.ceil((actualDeliveryDate.getTime() - expectedDeliveryDate.getTime()) / (1000 * 60 * 60 * 24));
     ...
     */
    async getBestVendorForProduct(variantId) {
        try {
     ...
     */
    async getVendorOptions(variantId) {
        try {
            const vendorMappings = await prisma.vendor_product_mappings.findMany({
                where: { variantId },
                include: {
            ...
            const vendorOptions = vendorMappings
    ...
    /**
       ...
       */
    async getAllVendorsWithMetrics() {
        try {
 ...
 */
export async function updateVendorReliability(vendorId, metrics) {
    await logDecision({
        scope: "build",
        actor: "inventory",
        action: "update_vendor_reliability",
 ...
 */
export async function getBestVendorForProduct(productId) {
    await logDecision({
        scope: "build",
        actor: "inventory",
        action: "get_best_vendor_for_product",
