 ...
 */
export async function generateOptimizationReport(products) {
    const today = new Date();
    // Dead stock identification
    const deadStockItems = products
        .filter((p) => {
        const daysSinceLastSale = p.lastSaleDate
            ? Math.floor((today.getTime() - p.lastSaleDate.getTime()) /
                (1000 * 60 * 60 * 24))
            : 999;
        ...
        recommendation: "Consider clearance sale, bundle deals, or discontinuation",
    }));
    const deadStockValue = deadStockItems.reduce((sum, item) => sum + item.estimatedValue, 0);
    // Overstock detection
    const overstockItems = products
        .filter((p) => {
        const daysOfSupply = p.avgDailySales > 0 ? p.currentStock / p.avgDailySales : 999;
        ...
        return {
            ...
            recommendation: "Reduce order quantities, implement just-in-time ordering",
        };
    });
    const tiedUpCapital = overstockItems.reduce((sum, item) => sum + item.tiedUpCapital, 0);
    // Recommendations
