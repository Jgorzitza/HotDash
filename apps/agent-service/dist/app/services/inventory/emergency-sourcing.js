...
export class EmergencySourcingService {
     ...
     */
    async analyzeEmergencySourcing(variantId, sku, productName, currentStock, demandForecast, averageLandedCost) {
        try {
            ...
            const opportunityCost = this.calculateOpportunityCost(stockoutRisk, demandForecast, averageLandedCost);
            ...
            const recommendedAction = this.determineRecommendedAction(netBenefit, stockoutRisk, currentStock);
            // Calculate urgency
     ...
     */
    async generateEmergencySourcingAction(analysis) {
        try {
     ...
     */
    calculateStockoutRisk(currentStock, demandForecast) {
        if (currentStock <= 0)
            return 1.0; // 100% risk if out of stock
        if (demandForecast <= 0)
            return 0.0; // No risk if no demand
     ...
     */
    calculateOpportunityCost(stockoutRisk, demandForecast, averageLandedCost) {
     ...
     * Get local vendor options for emergency sourcing
     */
    async getLocalVendorOptions(variantId) {
     ...
     */
    findBestEmergencyVendor(vendors) {
        if (vendors.length === 0)
            return null;
     ...
     * Calculate incremental cost for emergency sourcing
     */
    calculateIncrementalCost(vendor, averageLandedCost, demandForecast) {
     ...
     */
    determineRecommendedAction(netBenefit, stockoutRisk, currentStock) {
     ...
     */
    calculateUrgency(stockoutRisk, currentStock, demandForecast) {
        if (currentStock <= 0)
            return 'critical';
        if (stockoutRisk > 0.8)
            return 'high';
    ...
    calculateConfidence(analysis) {
        ...
        if (analysis.demandForecast > 0)
            confidence += 0.2;
        if (analysis.localVendorOptions.length > 0)
            confidence += 0.2;
        if (analysis.netBenefit > 0)
            confidence += 0.1;
 ...
 */
export async function analyzeEmergencySourcing(input) {
    await logDecision({
        scope: "build",
        actor: "inventory",
 ...
 */
export async function calculateEmergencySourcing(input) {
    await logDecision({
        scope: "build",
        actor: "inventory",
 ...
 */
export async function getEmergencySourcingHistory(filters) {
    await logDecision({
        scope: "build",
        actor: "inventory",
 ...
 */
export async function batchCalculateEmergencySourcing(items) {
    await logDecision({
        scope: "build",
        actor: "inventory",
 ...
 */
export async function updateEmergencySourcingStatus(id, status) {
    await logDecision({
        scope: "build",
        actor: "inventory",
 ...
 * Generate emergency sourcing action card
 */
export async function generateEmergencySourcingAction(input) {
    await logDecision({
        scope: "build",
        actor: "inventory",
        action: "generate_emergency_sourcing_action",
...
//# sourceMappingURL=emergency-sourcing.js.map