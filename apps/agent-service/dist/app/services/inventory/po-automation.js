 ...
 */
export function generatePONumber(vendorId, sequence) {
 ...
 */
export function groupAlertsByVendor(alerts) {
 ...
 */
export function calculateEstimatedTax(subtotal, taxRate = 0.08) {
 ...
 */
export function requiresHITLApproval(totalCost, items) {
    // Always require approval for POs >= $1000
    if (totalCost >= 1000) {
        return true;
    }
    // Require approval for critical items > $500
    const hasCriticalExpensiveItem = items.some((item) => item.urgency === "critical" && item.lineTotal > 500);
 ...
 */
export function generatePOForVendor(vendorAlerts, sequence) {
    ...
    // Create PO items
    const items = vendorAlerts.map((alert) => ({
 ...
 */
export async function autoGeneratePurchaseOrders(alerts) {
    if (alerts.length === 0) {
        return {
            totalPOs: 0,
            autoApprovedCount: 0,
    ...
    const autoApprovedCount = purchaseOrders.filter((po) => !po.requiresApproval).length;
    const pendingApprovalCount = purchaseOrders.filter((po) => po.requiresApproval).length;
 ...
 */
export async function approvePurchaseOrder(poNumber) {
 ...
 */
export async function rejectPurchaseOrder(poNumber, reason) {
...
//# sourceMappingURL=po-automation.js.map