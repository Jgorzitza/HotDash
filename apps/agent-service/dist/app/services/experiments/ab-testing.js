...
export class ABTestingService {
     ...
     */
    async trackExposure(experimentId, variantId, userId) {
     ...
     */
    async trackConversion(experimentId, variantId, userId, conversionName, value) {
        await this.trackEvent({
            test_id: experimentId,
            variant: variantId,
            user_id: userId,
     ...
     */
    async trackEngagement(experimentId, variantId, userId, engagementName, metadata) {
        await this.trackEvent({
            test_id: experimentId,
            variant: variantId,
            user_id: userId,
     ...
     */
    getExperimentConfig(userId, experimentId) {
     ...
     */
    weightedSelection(hash, variants) {
    ...
    calculateSampleSize(baselineRate, minDetectableEffect, alpha = 0.05, power = 0.8) {
        ...
        const pBar = (p1 + p2) / 2;
        return Math.ceil((2 * Math.pow(zAlpha + zBeta, 2) * pBar * (1 - pBar)) /
            Math.pow(p2 - p1, 2));
    }
}
// ============================================================================
