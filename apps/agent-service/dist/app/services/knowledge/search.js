...
export async function semanticSearch(request) {
    const { query, limit = 5, minSimilarity = 0.7, categories, tags, } = request;
    try {
 ...
 */
export async function findSimilarArticles(articleId, limit = 5, minSimilarity = 0.7) {
    try {
        ...
        // SECURITY FIX: Use Prisma.sql template literal to prevent SQL injection
        const embeddingVector = sourceArticle.embedding;
        const results = await prisma.$queryRaw `
      SELECT
        id,
        shop_domain,
 ...
 */
export async function searchByTags(tags, limit = 10) {
    try {
        const articles = await prisma.knowledge_base.findMany({
            where: {
                tags: {
 ...
 */
function mapToKBArticle(record) {
    return {
        id: record.id,
