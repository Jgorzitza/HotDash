 ...
 */
export async function getRecommendedArticles(question, limit = 3) {
    try {
 ...
 */
export async function recommendArticlesForUpdate() {
    try {
 ...
 */
export async function recommendArticlesForArchival() {
    try {
        ...
        for (const article of staleArticles) {
            const daysSinceUse = Math.floor((Date.now() - (article.last_indexed_at || article.created_at || new Date()).getTime()) /
                (1000 * 60 * 60 * 24));
            recommendations.push({
                type: "archive",
