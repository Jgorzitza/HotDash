 ...
 */
export async function buildRAGContext(customerMessage, options = {}) {
    const { maxContext = 3, minConfidence = 0.5, categories, tags, } = options;
    try {
 ...
 */
export function formatRAGContextForPrompt(context) {
 ...
 * @returns RAG context
 */
export async function buildCategoryContext(customerMessage, category, maxContext = 3) {
 ...
 * @returns RAG context
 */
export async function buildTagContext(customerMessage, tags, maxContext = 3) {
    return buildRAGContext(customerMessage, {
        maxContext,
        tags,
    });
...
export function getContextStats(context) {
    ...
    const avgConfidence = articles.reduce((sum, a) => sum + a.confidenceScore, 0) / articles.length;
 ...
 */
export function validateContextQuality(context, minArticles = 1, minAvgConfidence = 0.5) {
    ...
    if (stats.avgConfidence < minAvgConfidence) {
        console.warn(`[KB RAG] Context confidence too low: ${stats.avgConfidence.toFixed(2)} < ${minAvgConfidence}`);
        return false;
    }
    return true;
}
 ...
 */
export function mergeContexts(contexts, maxArticles = 5) {
    const allArticles = [];
