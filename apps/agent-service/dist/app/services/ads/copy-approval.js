 ...
 */
export function getCampaignApprovals(campaignId) {
    return Array.from(approvals.values()).filter((a) => a.request.campaignId === campaignId);
}
/**
 ...
 */
export function approveAdCopy(id, reviewedBy, reviewNotes) {
 ...
 * @throws Error if approval not found or already processed
 */
export function rejectAdCopy(id, reviewedBy, reviewNotes) {
    const approval = approvals.get(id);
    if (!approval) {
        throw new Error(`Approval not found: ${id}`);
    }
 ...
 */
export function markAsApplied(id) {
    const approval = approvals.get(id);
    if (!approval) {
        throw new Error(`Approval not found: ${id}`);
    }
 ...
 */
export function validateAdCopy(copy) {
    const errors = [];
    // Validate headlines
 ...
 */
export function generateCopyDiff(current, proposed) {
    const diffs = [];
    // Compare headlines
    ...
    if (JSON.stringify(current.descriptions) !==
...
//# sourceMappingURL=copy-approval.js.map