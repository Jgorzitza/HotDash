 ...
 */
export function checkBudgetDepletion(campaigns, performances, thresholds = DEFAULT_THRESHOLDS) {
    const alerts = [];
 ...
 */
export function checkLowCtr(performances, thresholds = DEFAULT_THRESHOLDS) {
    const alerts = [];
 ...
 */
export function checkHighCpc(performances, targetCpcCents, thresholds = DEFAULT_THRESHOLDS) {
    const alerts = [];
    for (const perf of performances) {
        ...
        if (cpcMultiplier >= thresholds.highCpcMultiplier) {
            let severity = "low";
            if (cpcMultiplier >= 2.0)
                severity = "high";
            else if (cpcMultiplier >= 1.75)
                severity = "medium";
            alerts.push({
                campaignId: perf.campaignId,
                campaignName: perf.campaignName,
 ...
 */
export function checkConversionDrops(currentPeriod, previousPeriod, thresholds = DEFAULT_THRESHOLDS) {
    const alerts = [];
    for (const current of currentPeriod) {
        ...
        if (previous.conversions < 5)
            continue;
        const dropPercent = ((previous.conversions - current.conversions) / previous.conversions) *
            100;
 ...
 */
export function generateAllAlerts(campaigns, currentPerformances, previousPerformances, targetCpcCents, thresholds = DEFAULT_THRESHOLDS) {
    const alerts = [];
 ...
 */
export function shouldAutoPause(alerts) {
    // Auto-pause if:
    // - Budget exceeded (100%+)
