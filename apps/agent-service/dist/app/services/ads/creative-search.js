 ...
 * ```
 */
export async function searchSimilarCreatives(request) {
    ...
    try {
        ...
        if (request.referenceImageId) {
            ...
            const response = await fetch(`${process.env.APP_URL || 'http://localhost:3000'}/api/search/images?q=${encodeURIComponent(request.query)}&limit=${(request.limit || 10) * 2}&project=occ`);
            if (!response.ok) {
                throw new Error(`Image search failed: ${response.statusText}`);
            }
            const data = await response.json();
        ...
        throw new Error(`Creative search failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 * ```
 */
export async function findSimilarHighPerformers(request) {
    const startTime = Date.now();
    try {
        ...
        throw new Error(`Failed to find high performers: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 * ```
 */
export async function searchCreativesByText(query, filters, limit = 10) {
 ...
 * ```
 */
export async function getTopPerformingCreatives(filters, limit = 20) {
 ...
 * @private
 */
function filterByPerformance(results, filters) {
    if (!filters)
        return results;
    return results.filter(result => {
        const metadata = result.metadata;
        // Check if it's an ad creative
        if (metadata.type !== 'ad_creative')
            return false;
        // Apply performance filters
        if (filters.minRoas && metadata.roas < filters.minRoas)
            return false;
        if (filters.minCtr && metadata.ctr < filters.minCtr)
            return false;
        if (filters.minConversions && metadata.conversions < filters.minConversions)
            return false;
        if (filters.maxCpc && metadata.cpc > filters.maxCpc)
            return false;
        // Apply platform filters
        if (filters.platforms && !filters.platforms.includes(metadata.platform))
            return false;
        if (filters.formats && !filters.formats.includes(metadata.format))
            return false;
        if (filters.placements && !filters.placements.includes(metadata.placement))
            return false;
        // Apply date filters
        if (filters.dateRangeStart && metadata.dateRange.start < filters.dateRangeStart)
            return false;
        if (filters.dateRangeEnd && metadata.dateRange.end > filters.dateRangeEnd)
            return false;
        // Apply campaign filters
        if (filters.campaignIds && !filters.campaignIds.includes(metadata.campaignId))
            return false;
        if (filters.excludeCampaignIds && filters.excludeCampaignIds.includes(metadata.campaignId))
            return false;
        return true;
    });
}
 ...
 */
export async function compareCreatives(imageIdA, imageIdB) {
...
//# sourceMappingURL=creative-search.js.map