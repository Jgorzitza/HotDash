 ...
 */
export function identifyPauseCandidates(campaigns, thresholds = DEFAULT_AUTOMATION_THRESHOLDS) {
    const actions = [];
 ...
 */
export function identifyBudgetIncreaseCandidates(campaigns, thresholds = DEFAULT_AUTOMATION_THRESHOLDS) {
    const actions = [];
    for (const campaign of campaigns) {
 ...
 */
export function identifyBudgetDecreaseCandidates(campaigns, thresholds = DEFAULT_AUTOMATION_THRESHOLDS) {
    const actions = [];
    for (const campaign of campaigns) {
        // Skip campaigns with low spend
        if (campaign.costCents < thresholds.minSpendForAction) {
 ...
 */
export function identifyKeywordPauseCandidates(keywords, thresholds = DEFAULT_AUTOMATION_THRESHOLDS) {
    const actions = [];
 ...
 */
export function generateAutomationRecommendations(campaigns, keywords, thresholds = DEFAULT_AUTOMATION_THRESHOLDS) {
    const actions = [];
 ...
 */
export function formatForApprovalQueue(action) {
    return {
        kind: "growth",
 ...
 */
export function getAutomationStats(actions) {
    const byType = {};
    ...
    for (const action of actions) {
        ...
        if (action.type === "pause_campaign" || action.type === "pause_keyword") {
            const currentCost = action.evidence.currentMetrics.cost ||
...
//# sourceMappingURL=campaign-automation.js.map