...
export class BingIntegration {
    ...
    async verifySite() {
        try {
            ...
            if (sitesResponse.success && sitesResponse.data) {
                const existingSite = sitesResponse.data.find(site => site.url === this.config.domain || site.url === `https://${this.config.domain}`);
    ...
    async getSearchMetrics(startDate, endDate) {
        try {
            ...
            const response = await this.client.getSearchMetrics(this.config.siteId, startDate, endDate, "day");
            if (!response.success || !response.data) {
                return {
                    success: false,
     ...
     */
    async getQueryPerformance(startDate, endDate, rowLimit = 1000) {
        try {
            if (!this.config.siteId) {
                return {
                    success: false,
            ...
            const response = await this.client.getQueryPerformance(this.config.siteId, startDate, endDate, rowLimit);
            if (!response.success || !response.data) {
                return {
                    success: false,
 ...
 */
export function createBingIntegration(config) {
