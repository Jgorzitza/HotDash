...
const SALES_CACHE_KEY = (shopDomain) => `shopify:sales:${shopDomain}`;
...
export async function getSalesPulseSummary(context) {
    ...
    for (const { node } of edges) {
        ...
        if (node.displayFulfillmentStatus &&
...
export async function getPendingFulfillments(context) {
    const { admin, shopDomain } = context;
    ...
    const mappedIssues = edges.map((edge) => {
        ...
        const createdAt = order.fulfillments?.edges?.[0]?.node?.events?.edges?.[0]?.node
            ?.createdAt ??
            order.createdAt ??
    ...
    const fulfillmentIssues = mappedIssues.filter((issue) => Boolean(issue.displayStatus) && issue.displayStatus !== "FULFILLED");
    const fact = await recordDashboardFact({
        shopDomain,
