 ...
 */
async function fetchVariantInventory(context, variantGid) {
    try {
 ...
 */
export async function calculateBundleStock(context, productId) {
    ...
    const limitingComponent = componentAvailability.find((c) => c.possibleBundles === virtualStock);
    return {
        virtualStock,
        componentAvailability,
        limitingComponent,
 ...
 */
export async function decrementBundleComponents(context, productId, qtySold) {
    const errors = [];
    let adjustments = 0;
    // 1. Get BOM components
    const bom = await getBOMComponents(context, productId);
            ...
            if (available > 0) {
                ...
                try {
                    ...
                    if (userErrors.length > 0) {
                        errors.push(`Adjust ${variantGid}: ${userErrors.map((e) => e.message).join(", ")}`);
                    }
                    else {
                        adjustments++;
 ...
 */
export async function calculateBundleStockFromTags(context, productId) {
...
//# sourceMappingURL=bundle-inventory.js.map