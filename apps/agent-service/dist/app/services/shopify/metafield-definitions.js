 ...
 */
export async function createBOMMetafieldDefinitions(context) {
    const errors = [];
    const definitions = [
        {
            name: "BOM Components",
    ...
    for (const def of definitions) {
        try {
            const response = await context.admin.graphql(CREATE_METAFIELD_DEFINITION_MUTATION, {
                variables: { definition: def },
            });
            const json = await response.json();
            const userErrors = json.data?.metafieldDefinitionCreate?.userErrors || [];
            if (userErrors.length > 0) {
                errors.push(`${def.key}: ${userErrors.map((e) => e.message).join(", ")}`);
            }
        }
        catch (error) {
 ...
 */
export async function setBOMComponents(context, productId, components, parameters) {
    try {
        const bomData = {
 ...
 */
export async function getBOMComponents(context, productId) {
    try {
 ...
 * @param productId Product GID
 */
export async function markAsComponent(context, productId) {
    try {
        const response = await context.admin.graphql(PRODUCT_UPDATE_MUTATION, {
            variables: {
                input: {
...
//# sourceMappingURL=metafield-definitions.js.map