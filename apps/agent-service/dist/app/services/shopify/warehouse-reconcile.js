 ...
 */
async function getProductsWithNegativeCanadaInventory(context) {
 ...
 */
async function reconcileVariant(context, inventoryItemId, variantId, canadaNegativeQty) {
    try {
        // 1. Get current Main WH inventory
        const mainResponse = await context.admin.graphql(GET_INVENTORY_ITEM_LEVELS_QUERY, {
            variables: {
                id: inventoryItemId,
            },
        });
        ...
        const mainLevel = inventoryLevels.find((edge) => edge.node.location.id === MAIN_WH_ID);
        const mainAvailableQty = mainLevel?.node.quantities?.find((q) => q.name === "available")
            ?.quantity || 0;
        ...
        // 3. Apply adjustments
        const adjustResponse = await context.admin.graphql(INVENTORY_ADJUST_MUTATION, {
            variables: {
                input: {
                    reason: "correction",
                    name: "available",
 ...
 */
export async function runNightlyWarehouseReconciliation(context) {
    ...
    for (const product of negativeProducts) {
        try {
            const result = await reconcileVariant(context, product.inventoryItemId, product.variantId, product.available);
            results.push(result);
            if (!result.adjusted) {
...
//# sourceMappingURL=warehouse-reconcile.js.map