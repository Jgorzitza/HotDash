...
export class ActionQueueService {
     ...
     */
    static async createAction(input) {
        try {
            const query = `
    ...
    static async getActions(options = {}) {
        try {
            const { limit = 10, offset = 0, status = 'pending', agent, type, risk_tier, priority, category, sort_by = 'score', sort_order = 'desc' } = options;
            // Build where clause
            const where = { status };
            if (agent)
                where.agent = agent;
    ...
    static async getActionCount(options = {}) {
        try {
            const { status = 'pending', agent, type, risk_tier } = options;
            const where = { status };
            if (agent)
                where.agent = agent;
            if (type)
    ...
    static async getActionById(id) {
        try {
            const { rows } = await prisma.query('SELECT * FROM action_queue WHERE id = $1', [id]);
            if (rows.length === 0) {
                return null;
            }
            return this.mapDbRowToActionQueueItem(rows[0]);
     ...
     */
    static async updateAction(id, updates) {
        try {
            const updateFields = [];
            const params = [];
            let paramIndex = 1;
     ...
     * Approve an action
     */
    static async approveAction(id, operatorId) {
     ...
     * Reject an action
     */
    static async rejectAction(id, operatorId) {
        return this.updateAction(id, {
            status: 'rejected',
            approved_by: operatorId,
            approved_at: new Date()
     ...
     */
    static async executeAction(id, operatorId) {
     ...
     */
    static async executeActionByType(action) {
     ...
     */
    static mapDbRowToActionQueueItem(row) {
        return {
            id: row.id,
            type: row.type,
            target: row.target,
...
//# sourceMappingURL=action-queue.service.js.map