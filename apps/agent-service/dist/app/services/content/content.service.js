...
export class ContentService {
    ...
    static async getContentTypes() {
        try {
            const { rows } = await prisma.query('SELECT * FROM content_types ORDER BY created_at DESC');
    ...
    static async getContentTypeById(id) {
        try {
            const { rows } = await prisma.query('SELECT * FROM content_types WHERE id = $1', [id]);
            if (rows.length === 0) {
                return null;
            }
            return this.mapDbRowToContentType(rows[0]);
     ...
     */
    static async createContentEntry(input) {
        try {
            const query = `
    ...
    static async getContentEntries(options = {}) {
        try {
            const { content_type_id, status, limit = 20, offset = 0, search } = options;
            let query = `
     ...
     */
    static async getContentEntryById(id) {
        try {
            const { rows } = await prisma.query('SELECT * FROM content_entries WHERE id = $1', [id]);
            if (rows.length === 0) {
                return null;
            }
            return this.mapDbRowToContentEntry(rows[0]);
     ...
     */
    static async updateContentEntry(id, input) {
        try {
            const updateFields = [];
            const params = [];
            let paramIndex = 1;
     ...
     */
    static async deleteContentEntry(id) {
        try {
            const { rows } = await prisma.query('DELETE FROM content_entries WHERE id = $1 RETURNING id', [id]);
            if (rows.length === 0) {
                throw new Error('Content entry not found');
            }
        }
     ...
     */
    static async publishContentEntry(id, published_by) {
     ...
     */
    static async getContentEntryVersions(id) {
        try {
            const { rows } = await prisma.query('SELECT * FROM content_entry_versions WHERE content_entry_id = $1 ORDER BY version DESC', [id]);
     ...
     */
    static mapDbRowToContentType(row) {
        return {
            id: row.id,
            name: row.name,
     ...
     */
    static mapDbRowToContentEntry(row) {
        return {
            id: row.id,
