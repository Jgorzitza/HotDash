...
export class ContentApprovalWorkflowService {
    ...
    static async submitForApproval(input) {
        try {
            ...
            if (input.content_entry_id) {
                const { rows } = await prisma.query(`SELECT id, version FROM content_approvals 
           WHERE content_entry_id = $1 
           ORDER BY version DESC LIMIT 1`, [input.content_entry_id]);
                if (rows.length > 0) {
    ...
    static async getApprovalById(id) {
        try {
            const { rows } = await prisma.query('SELECT * FROM content_approvals WHERE id = $1', [id]);
            if (rows.length === 0) {
                return null;
            }
            return this.mapDbRowToContentApproval(rows[0]);
     ...
     * Approve content
     */
    static async approveContent(id, reviewed_by, options) {
        try {
            // Update approval status
            const { rows } = await prisma.query(`UPDATE content_approvals 
         SET status = 'approved', reviewed_by = $1, reviewed_at = NOW()
         WHERE id = $2
         RETURNING *`, [reviewed_by, id]);
            if (rows.length === 0) {
     ...
     * Reject content
     */
    static async rejectContent(id, reviewed_by, rejection_reason) {
        try {
            const { rows } = await prisma.query(`UPDATE content_approvals 
         SET status = 'rejected', reviewed_by = $1, reviewed_at = NOW(), rejection_reason = $2
         WHERE id = $3
         RETURNING *`, [reviewed_by, rejection_reason, id]);
            if (rows.length === 0) {
                throw new Error('Approval not found');
            }
            return this.mapDbRowToContentApproval(rows[0]);
     ...
     */
    static async requestChanges(id, reviewed_by, requested_changes) {
        try {
            const { rows } = await prisma.query(`UPDATE content_approvals 
         SET status = 'changes_requested', reviewed_by = $1, reviewed_at = NOW(), requested_changes = $2
         WHERE id = $3
         RETURNING *`, [reviewed_by, requested_changes, id]);
            if (rows.length === 0) {
                throw new Error('Approval not found');
            }
            return this.mapDbRowToContentApproval(rows[0]);
     ...
     */
    static async getApprovalHistory(content_entry_id) {
        try {
            const { rows } = await prisma.query(`SELECT * FROM content_approvals 
         WHERE content_entry_id = $1 
         ORDER BY version DESC`, [content_entry_id]);
     ...
     */
    static mapDbRowToContentApproval(row) {
        return {
            id: row.id,
