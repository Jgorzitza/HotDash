...
export class SchedulingService {
     ...
     * Schedule content for future publishing
     */
    static async scheduleContent(input) {
        try {
            const query = `
     ...
     */
    static async getScheduledContentById(id) {
        try {
            const { rows } = await prisma.query('SELECT * FROM scheduled_content WHERE id = $1', [id]);
            if (rows.length === 0) {
                return null;
            }
            return this.mapDbRowToScheduledContent(rows[0]);
    ...
    static async getScheduledContent(options = {}) {
        try {
            const { status, content_type, platform, start_date, end_date, limit = 50, offset = 0 } = options;
     ...
     */
    static async getContentForDateRange(startDate, endDate) {
        try {
            const { rows } = await prisma.query(`SELECT * FROM scheduled_content 
         WHERE scheduled_for >= $1 AND scheduled_for <= $2
         ORDER BY scheduled_for ASC`, [startDate, endDate]);
     ...
     */
    static async updateScheduledContent(id, updates) {
        try {
            const updateFields = [];
            const params = [];
            let paramIndex = 1;
     ...
     */
    static async cancelScheduledContent(id) {
        try {
     ...
     */
    static mapDbRowToScheduledContent(row) {
        return {
            id: row.id,
