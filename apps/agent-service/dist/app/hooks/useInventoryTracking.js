...
export const useInventoryTracking = (options) => {
    const { variantIds, autoConnect = true, autoRefresh = true, refreshInterval = 30000, onInventoryUpdate, onAlert, onError, onConnect, onDisconnect, } = options;
    ...
    const refreshInventory = useCallback(async (variantIdsToRefresh) => {
    ...
    // Update inventory
    const updateInventory = useCallback(async (variantId, newStock, reason) => {
        if (!socket || !isConnected)
            return;
        try {
    ...
    const subscribeToVariant = useCallback((variantId) => {
        if (socket && isConnected) {
            socket.emit('subscribe-variant', variantId);
        }
    }, [socket, isConnected]);
    // Unsubscribe from variant
    const unsubscribeFromVariant = useCallback((variantId) => {
        if (socket && isConnected) {
...
//# sourceMappingURL=useInventoryTracking.js.map