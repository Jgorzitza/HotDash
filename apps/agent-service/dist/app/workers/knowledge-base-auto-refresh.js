...
import { createKnowledgeBaseWatcher } from './knowledge-base-watcher';
 ...
 */
export async function startAutoRefresh(config = {}) {
    if (isRunning) {
        console.log('[KB Auto-Refresh] Already running');
        return;
    }
    const { watchPath = path.join(process.cwd(), 'data/support'), debounceMs = 5000, enabled = true, } = config;
    if (!enabled) {
...
//# sourceMappingURL=knowledge-base-auto-refresh.js.map