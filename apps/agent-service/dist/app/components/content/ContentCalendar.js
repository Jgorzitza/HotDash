...
import { Card, Text, Button, Badge, InlineStack, BlockStack, Modal, TextField, Select, Banner, } from '@shopify/polaris';
export function ContentCalendar({ calendar, month, year, stats, onNavigate, onSchedule, onCancel, onReschedule, }) {
      ...
      <Card>
        <div style={{ padding: '16px' }}>
          ...
          <div style={{
            ...
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (<div key={day} style={{ textAlign: 'center', fontWeight: 'bold' }}>
                <Text as="span" variant="bodyMd">{day}</Text>
              </div>))}
          </div>

          {/* Calendar Days */}
          {calendar.map((week, weekIdx) => (<div key={weekIdx} style={{
                display: 'grid',
                gridTemplateColumns: 'repeat(7, 1fr)',
                gap: '8px',
                marginBottom: '8px'
            }}>
              {week.days.map((day, dayIdx) => (<CalendarDayCell key={dayIdx} day={day} onClick={() => handleDayClick(day.date)} onItemClick={(item) => setSelectedItem(item)}/>))}
            </div>))}
        </div>
      </Card>

      {/* Schedule Modal */}
      <Modal open={showScheduleModal} onClose={() => {
            setShowScheduleModal(false);
            setSelectedDate('');
        }} title="Schedule Content" primaryAction={{
            content: 'Schedule',
            onAction: () => {
                // Handle schedule
                setShowScheduleModal(false);
        ...
        <Modal.Section>
          <BlockStack gap="400">
            <TextField label="Title" value="" onChange={() => { }} autoComplete="off"/>
            <Select label="Content Type" options={[
            { label: 'Social Post', value: 'social_post' },
            { label: 'Blog Post', value: 'blog_post' },
            { label: 'Email Campaign', value: 'email_campaign' },
        ]} value="social_post" onChange={() => { }}/>
            <TextField label="Scheduled Date/Time" type="datetime-local" value={selectedDate} onChange={setSelectedDate} autoComplete="off"/>
          </BlockStack>
        </Modal.Section>
      </Modal>

      {/* Item Detail Modal */}
      {selectedItem && (<Modal open={!!selectedItem} onClose={() => setSelectedItem(null)} title={selectedItem.title}>
          <Modal.Section>
            <BlockStack gap="400">
              <InlineStack gap="200">
              ...
              {selectedItem.status === 'scheduled' && (<InlineStack gap="200">
              ...
              {selectedItem.status === 'failed' && selectedItem.error_message && (<Banner tone="critical">
...
function CalendarDayCell({ day, onClick, onItemClick }) {
    ...
    return (<div onClick={onClick} style={{
      ...
      <div style={{ marginBottom: '4px' }}>
        ...
        {day.items.length > 0 && (<Badge tone="info">{day.items.length}</Badge>)}
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {day.items.slice(0, 2).map((item, idx) => (<div key={idx} onClick={(e) => {
                e.stopPropagation();
                onItemClick(item);
            }} style={{
                fontSize: '12px',
        ...
        {day.items.length > 2 && (<Text as="span" variant="bodySm" tone="subdued">
