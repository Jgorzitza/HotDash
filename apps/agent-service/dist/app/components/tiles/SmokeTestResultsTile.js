 ...
 */
export function SmokeTestResultsTile({ data }) {
    const { latest, trends } = data;
    const passRate = latest.total > 0
        ? ((latest.passed / latest.total) * 100).toFixed(1)
        : "0.0";
    const statusColor = latest.status === "healthy" ? "var(--occ-status-success)" :
        latest.status === "degraded" ? "var(--occ-status-warning)" :
            "var(--occ-status-critical)";
    const statusIcon = latest.status === "healthy" ? "✅" :
        latest.status === "degraded" ? "⚠️" :
            "❌";
    return (<div style={{ display: "flex", flexDirection: "column", gap: "var(--occ-space-3)" }}>
      {/* Status Header */}
      <div style={{
            display: "flex",
            alignItems: "center",
      ...
      <div style={{
        ...
        <MetricBox label="Pass Rate" value={`${passRate}%`} trend={trends.passRate >= 95 ? "up" : "down"}/>
        <MetricBox label="Duration" value={`${(latest.duration / 1000).toFixed(1)}s`} subtitle={`Avg: ${(trends.avgDuration / 1000).toFixed(1)}s`}/>
        <MetricBox label="Passed" value={latest.passed.toString()} color="var(--occ-status-success)"/>
        <MetricBox label="Failed" value={latest.failed.toString()} color={latest.failed > 0 ? "var(--occ-status-critical)" : "var(--occ-text-secondary)"}/>
      </div>
      ...
      {/* Trends */}
      {data.history.length > 1 && (<div style={{
                fontSize: "var(--occ-font-size-sm)",
                color: "var(--occ-text-secondary)",
            }}>
          <div>
...
function MetricBox({ label, value, subtitle, trend, color }) {
    return (<div style={{
            padding: "var(--occ-space-2)",
            backgroundColor: "var(--occ-surface-subdued)",
            borderRadius: "var(--occ-border-radius)",
      ...
      <div style={{
        ...
        {value}
        {trend && (<span style={{ fontSize: "var(--occ-font-size-sm)" }}>
            {trend === "up" ? "↑" : "↓"}
          </span>)}
      </div>
      {subtitle && (<div style={{
...
//# sourceMappingURL=SmokeTestResultsTile.js.map