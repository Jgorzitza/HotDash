...
export function ActionAttributionDashboard({ attributionData = [], loading = false, error, onRefresh, }) {
    const [viewMode, setViewMode] = useState("overview");
    const [selectedTimeframe, setSelectedTimeframe] = useState("28d");
    ...
    return (<div style={{ padding: "var(--occ-space-4)" }}>
      ...
      <div style={{ marginBottom: "var(--occ-space-4)" }}>
        <div style={{
          ...
          <h2 style={{ margin: 0 }}>Action Attribution & ROI Dashboard</h2>
          <div style={{
            display: "flex",
            alignItems: "center",
            gap: "var(--occ-space-2)",
        }}>
            <select value={selectedTimeframe} onChange={(e) => setSelectedTimeframe(e.target.value)} style={{
...
function OverviewView({ attributionData, timeframe, }) {
    ...
    return (<div>
      ...
      <div style={{
        ...
        <MetricCard title="Total Revenue" value={`$${totalRevenue.toLocaleString()}`} subtitle={`${timeframe} attribution window`} color="var(--occ-color-success)"/>
        <MetricCard title="Total Sessions" value={totalSessions.toLocaleString()} subtitle="Attributed sessions" color="var(--occ-color-info)"/>
        <MetricCard title="Total Conversions" value={totalConversions.toLocaleString()} subtitle="Attributed purchases" color="var(--occ-color-warning)"/>
        <MetricCard title="Conversion Rate" value={`${averageConversionRate.toFixed(1)}%`} subtitle="Average across all actions" color="var(--occ-color-primary)"/>
      </div>
      ...
      <div>
        ...
        <div style={{ display: "grid", gap: "var(--occ-space-2)" }}>
          {attributionData
            ...
            .map((data, index) => {
            const timeframeData = getTimeframeData(data);
            return (<div key={data.roi7d.actionKey} style={{
                  ...
                  <div style={{
                    ...
                    color: "var(--occ-text-secondary)",
                        ...
                        {timeframeData.sessions} sessions •{" "}
                        {timeframeData.purchases} conversions
                      </div>
                    </div>
...
function ROITrackingView({ attributionData, timeframe, }) {
    const getTimeframeData = (data) => {
        switch (timeframe) {
            case "7d":
                return data.roi7d;
    ...
    return (<div>
      ...
      <div style={{ display: "grid", gap: "var(--occ-space-3)" }}>
        {attributionData.map((data) => {
            ...
            return (<div key={timeframeData.actionKey} style={{
                    padding: "var(--occ-space-4)",
                    border: "1px solid var(--occ-border-default)",
                    borderRadius: "var(--occ-radius-md)",
                    backgroundColor: "var(--occ-bg-surface)",
...
function AttributionAnalysisView({ attributionData, timeframe, }) {
    const getTimeframeData = (data) => {
        switch (timeframe) {
            case "7d":
                return data.roi7d;
    ...
    const calculateAttributionEfficiency = (data) => {
        const roi7d = data.roi7d;
        const roi14d = data.roi14d;
        const roi28d = data.roi28d;
...
function PerformanceMetricsView({ attributionData, timeframe, }) {
    const getTimeframeData = (data) => {
        switch (timeframe) {
            case "7d":
                return data.roi7d;
    ...
    return (<div>
      ...
      <div style={{
        ...
        <MetricCard title="Total Revenue" value={`$${metrics.totalRevenue.toLocaleString()}`} subtitle={`${timeframe} attribution window`} color="var(--occ-color-success)"/>
        <MetricCard title="Total Sessions" value={metrics.totalSessions.toLocaleString()} subtitle="Attributed sessions" color="var(--occ-color-info)"/>
        <MetricCard title="Total Conversions" value={metrics.totalConversions.toLocaleString()} subtitle="Attributed purchases" color="var(--occ-color-warning)"/>
        <MetricCard title="Average CVR" value={`${metrics.averageConversionRate.toFixed(1)}%`} subtitle="Conversion rate" color="var(--occ-color-primary)"/>
        <MetricCard title="Average AOV" value={`$${metrics.averageOrderValue.toFixed(0)}`} subtitle="Order value" color="var(--occ-color-secondary)"/>
        <MetricCard title="Revenue/Session" value={`$${metrics.revenuePerSession.toFixed(2)}`} subtitle="Per session" color="var(--occ-color-accent)"/>
      </div>

      {/* Action Performance Comparison */}
      <div>
        <h4 style={{ margin: 0, marginBottom: "var(--occ-space-3)" }}>
          Action Performance Comparison
        </h4>
        <div style={{ display: "grid", gap: "var(--occ-space-2)" }}>
          {attributionData
            ...
            .map((data, index) => {
            const timeframeData = getTimeframeData(data);
            const performanceScore = (timeframeData.conversionRate *
                timeframeData.averageOrderValue) /
                100;
            return (<div key={timeframeData.actionKey} style={{
                    padding: "var(--occ-space-3)",
                    border: "1px solid var(--occ-border-default)",
...
function RecommendationsView({ attributionData, timeframe, }) {
    const getTimeframeData = (data) => {
        switch (timeframe) {
            case "7d":
                return data.roi7d;
    ...
    return (<div>
      ...
      <div style={{ display: "grid", gap: "var(--occ-space-3)" }}>
        {recommendations.map((rec, index) => (<div key={index} style={{
                padding: "var(--occ-space-4)",
                border: `1px solid ${rec.type === "success"
                    ? "var(--occ-border-success)"
                    : rec.type === "warning"
                        ? "var(--occ-border-warning)"
                        : "var(--occ-border-info)"}`,
                borderRadius: "var(--occ-radius-md)",
                backgroundColor: `${rec.type === "success"
...
//# sourceMappingURL=ActionAttributionDashboard.js.map