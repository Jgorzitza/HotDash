...
export function SpecialistAgentsTile({ showHistory = false }) {
    ...
    const getRunTypeDisplay = (runType) => {
        switch (runType) {
      ...
      <div style={{ padding: '20px' }}>
        <Stack vertical spacing="loose">
          <InlineStack align="space-between">
            ...
            <div>
              <Button size="slim" variant="primary" onClick={handleRunAll} loading={running} disabled={runningRuns.length > 0}>
                Run All
              </Button>
            </div>
          </InlineStack>
          
          {runningRuns.length > 0 && (<div style={{
          ...
          <Stack vertical spacing="tight">
            {recentRuns.map((run) => (<div key={run.id} style={{
                ...
                <Stack vertical spacing="tight">
                  ...
                  {run.end_time && (<Text as="p" tone="subdued">
                      Completed: {new Date(run.end_time).toLocaleString()}
                    </Text>)}
                  
                  {run.error_message && (<Text as="p" tone="critical">
                      Error: {run.error_message}
                    </Text>)}
                  
                  {run.status === 'running' && (<div style={{ marginTop: '8px' }}>
                      <ProgressBar progress={75} size="small"/>
                    </div>)}
                  
                  {run.status === 'completed' && (<Button size="slim" variant="secondary" onClick={() => handleRunAgent(run.agent_name)} disabled={running}>
                      Run Again
                    </Button>)}
                </Stack>
              </div>))}
          ...
          {runs.length === 0 && (<div style={{ textAlign: 'center', padding: '20px' }}>
...
//# sourceMappingURL=SpecialistAgentsTile.js.map