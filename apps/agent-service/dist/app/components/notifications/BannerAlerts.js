...
export function BannerAlerts({ alerts, onDismiss }) {
    ...
    return (<div style={{
      ...
      {visibleAlerts.map((alert) => (<Banner key={alert.id} alert={alert} onDismiss={handleDismiss}/>))}
    </div>);
}
function Banner({ alert, onDismiss }) {
    ...
    return (<div role="alert" style={{
      ...
      <div style={{ flex: 1 }}>
        ...
        {alert.action && (<a href={alert.action.url} style={{
      ...
      {alert.dismissible !== false && (<button onClick={() => onDismiss(alert.id)} style={{
...
//# sourceMappingURL=BannerAlerts.js.map