...
import { Modal, Banner, Card, Text, BlockStack, InlineStack, Divider, Badge, SkeletonBodyText, } from "@shopify/polaris";
...
export function SalesPulseModal({ summary, open, onClose, }) {
    const fetcher = useFetcher();
    const [note, setNote] = useState("");
    const [selectedAction, setSelectedAction] = useState("acknowledge");
    // WoW variance state
    const [revenueVariance, setRevenueVariance] = useState(null);
    const [ordersVariance, setOrdersVariance] = useState(null);
    ...
    return (<Modal open={open} onClose={onClose} title="Sales Pulse — Details" primaryAction={{
      ...
      <Modal.Section>
        <BlockStack gap="400">
          ...
          <BlockStack gap="300">
              ...
              <BlockStack gap="200">
                <InlineStack align="space-between" blockAlign="center">
                  ...
                  <InlineStack gap="200" blockAlign="center">
                    ...
                    {varianceLoading ? (<SkeletonBodyText lines={1}/>) : revenueVariance ? (<Badge tone={revenueVariance.trend === "up" ? "success" : revenueVariance.trend === "down" ? "critical" : "info"}>
                        WoW: {revenueVariance.variance > 0 ? "+" : ""}{revenueVariance.variance}% {revenueVariance.trend === "up" ? "↗" : revenueVariance.trend === "down" ? "↘" : "→"}
                      </Badge>) : (<Text as="span" tone="subdued" variant="bodySm">(WoW unavailable)</Text>)}
                  </InlineStack>
                </InlineStack>
                ...
                <InlineStack align="space-between" blockAlign="center">
                  ...
                  <InlineStack gap="200" blockAlign="center">
                    ...
                    {ordersVariance && (<Badge tone={ordersVariance.trend === "up" ? "success" : ordersVariance.trend === "down" ? "critical" : "info"}>
            ...
            {revenueVariance && (<Text as="p" variant="bodySm" tone="subdued">
          ...
          <section className="occ-modal__section">
            ...
            <ul className="occ-modal__list">
              {summary.topSkus.map((sku) => (<li key={`${sku.sku}-${sku.title}`}>
                  <span>{sku.title}</span>
                  <span>
          ...
          <section className="occ-modal__section">
            ...
            {summary.pendingFulfillment.length === 0 ? (<p className="occ-text-secondary">No blockers detected.</p>) : (<ul className="occ-modal__list">
                {summary.pendingFulfillment.map((order) => (<li key={order.orderId}>
          ...
          <section className="occ-modal__section">
            ...
            <label className="occ-field">
              ...
              <select className="occ-select" value={selectedAction} onChange={(event) => setSelectedAction(event.currentTarget.value)} disabled={isSubmitting}>
                {Object.entries(ACTION_LABELS).map(([action, label]) => (<option key={action} value={action}>
                    {label}
                  </option>))}
              </select>
            </label>
            <label className="occ-field">
              ...
              <textarea className="occ-textarea" rows={3} placeholder="Add context for the decision log" value={note} onChange={(event) => setNote(event.currentTarget.value)} disabled={isSubmitting}/>
            </label>
            {fetcher.data?.error ? (<p className="occ-feedback occ-feedback--error" role="alert">
                {fetcher.data.error}
              </p>) : null}
          </section>
        </div>
        ...
        <div className="occ-modal__footer">
          <div className="occ-modal__footer-actions">
            <button type="button" className="occ-button occ-button--primary" onClick={() => submit(selectedAction)} disabled={isSubmitting} aria-live="polite" aria-atomic="true">
              {ACTION_LABELS[selectedAction]}
            </button>
          </div>
          <button type="button" className="occ-button occ-button--plain" onClick={onClose} disabled={isSubmitting}>
            Cancel
          </button>
        </div>
      </dialog>
...
//# sourceMappingURL=SalesPulseModal.js.map