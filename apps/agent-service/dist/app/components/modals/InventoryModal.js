...
import { Modal, TextField, Select, Banner, Card, Text, BlockStack, InlineStack, Divider, Badge, ProgressBar, TextContainer, } from "@shopify/polaris";
...
export function InventoryModal({ alert, open, onClose }) {
    const fetcher = useFetcher();
    const [selectedAction, setSelectedAction] = useState("create_po");
    ...
    const stockPercentage = alert.threshold > 0
        ...
        : 0;
    return (<Modal open={open} onClose={onClose} title={`Inventory Alert — ${alert.title}`} primaryAction={{
      ...
      <Modal.Section>
        <BlockStack gap="400">
          ...
          <BlockStack gap="300">
            ...
            <Card>
              <BlockStack gap="300">
                ...
                {alert.daysOfCover != null && (<InlineStack align="space-between" blockAlign="center">
          ...
          <BlockStack gap="300">
            ...
            <TextField label="Reorder Quantity" type="number" value={String(reorderQuantity)} onChange={(value) => setReorderQuantity(Number(value))} min={1} step={1} disabled={isSubmitting} autoComplete="off" helpText="Enter quantity in units"/>
            <Select label="Vendor" options={vendorOptions} value={vendor} onChange={setVendor} disabled={isSubmitting} helpText="Select vendor for purchase order"/>
            <Text as="p" variant="bodySm" tone="subdued">
          ...
          <BlockStack gap="300">
            ...
            <Select label="What do you want to do?" options={actionOptions} value={selectedAction} onChange={(value) => setSelectedAction(value)} disabled={isSubmitting}/>
            <TextField label="Notes (audit trail)" value={note} onChange={setNote} multiline={3} placeholder="Add context for the decision log" autoComplete="off" disabled={isSubmitting}/>
          </BlockStack>
        </BlockStack>
      </Modal.Section>
    </Modal>);
}
//# sourceMappingURL=InventoryModal.js.map