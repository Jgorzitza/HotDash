...
import { Modal, TextContainer, TextField, RangeSlider, Banner, Scrollable, Card, Text, BlockStack, InlineStack, Divider, } from "@shopify/polaris";
import { useModalFocusTrap } from "../../hooks/useModalFocusTrap";
import { useToast } from "../../hooks/useToast";
export function CXEscalationModal({ conversation, open, onClose, }) {
    ...
    return (<Modal open={open} onClose={onClose} title={`CX Escalation — ${conversation.customerName}`} primaryAction={{
      ...
      <Modal.Section>
        <BlockStack gap="400">
          ...
          {fetcher.data?.error && (<Banner tone="critical" onDismiss={() => { }}>
          ...
          <BlockStack gap="300">
            ...
            <Card>
              <Scrollable shadow style={{ maxHeight: "300px" }}>
                <BlockStack gap="200">
                  {messageHistory.length === 0 ? (<Text as="p" tone="subdued">
                      No recent messages available.
                    </Text>) : (messageHistory.map((message) => (<Card key={message.id}>
                        <BlockStack gap="100">
          ...
          <BlockStack gap="300">
            ...
            {hasSuggestion ? (<TextField label="" value={reply} onChange={setReply} multiline={6} autoComplete="off" disabled={isSubmitting} helpText="Edit the AI-suggested reply before sending"/>) : (<Banner tone="info">
          ...
          <BlockStack gap="300">
            ...
            <TextField label="" value={note} onChange={setNote} multiline={3} placeholder="Add context for audit trail" autoComplete="off" disabled={isSubmitting}/>
          </BlockStack>
          ...
          <BlockStack gap="300">
            ...
            <BlockStack gap="400">
              <RangeSlider label="Tone" value={toneGrade} onChange={setToneGrade} min={1} max={5} step={1} output disabled={isSubmitting} helpText="Grade response tone from 1 (poor) to 5 (excellent)"/>
              <RangeSlider label="Accuracy" value={accuracyGrade} onChange={setAccuracyGrade} min={1} max={5} step={1} output disabled={isSubmitting} helpText="Grade response accuracy from 1 (poor) to 5 (excellent)"/>
              <RangeSlider label="Policy Compliance" value={policyGrade} onChange={setPolicyGrade} min={1} max={5} step={1} output disabled={isSubmitting} helpText="Grade policy compliance from 1 (poor) to 5 (excellent)"/>
            </BlockStack>
          </BlockStack>
        </BlockStack>
      </Modal.Section>
...
//# sourceMappingURL=CXEscalationModal.js.map