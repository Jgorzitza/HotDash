 ...
 */
export function BannerAlert({ id, tone, title, message, action, dismissible = true, onDismiss, }) {
    ...
    if (isDismissed)
        return null;
    return (<Banner title={title} tone={tone} action={action} onDismiss={dismissible ? handleDismiss : undefined}>
      {message && <p>{message}</p>}
    </Banner>);
}
/**
 ...
 */
export function generateBannerAlerts(status) {
    const alerts = [];
 ...
 */
export function BannerAlerts({ alerts, onDismiss, }) {
    ...
    if (visibleAlerts.length === 0)
        return null;
    return (<div style={{ display: "flex", flexDirection: "column", gap: "1rem", marginBottom: "1rem" }}>
      {visibleAlerts.map((alert) => (<BannerAlert key={alert.id} {...alert} onDismiss={handleDismiss}/>))}
    </div>);
}
/**
 ...
 */
export function useBannerAlerts(status) {
