 ...
 */
export function Toast({ message, type = "info", duration, onDismiss, onRetry, active = true, }) {
    ...
    // Auto-dismiss timer
    useEffect(() => {
        if (!isActive || autoDismissDuration === 0)
            return;
    ...
    if (isError && onRetry) {
        return (<Frame>
        <PolarisToast content={message} error={true} onDismiss={handleDismiss} action={{
                content: "Retry",
    ...
    return (<Frame>
      <PolarisToast content={message} error={isError} onDismiss={handleDismiss} duration={autoDismissDuration}/>
    </Frame>);
}
/**
 * Toast Manager Hook
...
export function useToastManager() {
    ...
    const showError = useCallback((message, onRetry, duration) => {
