...
export function ApprovalEvidenceSection({ evidence }) {
    ...
    const summaryCard = (evidence.what_changes ||
        evidence.why_now ||
        evidence.impact_forecast) && (<Card>
      <BlockStack gap="200">
        {evidence.what_changes && (<Text as="p">What changes: {evidence.what_changes}</Text>)}
        {evidence.why_now && <Text as="p">Why now: {evidence.why_now}</Text>}
        {evidence.impact_forecast && (<Text as="p">Impact forecast: {evidence.impact_forecast}</Text>)}
      </BlockStack>
    </Card>);
    ...
    if (Array.isArray(evidence.diffs) && evidence.diffs.length > 0) {
        ...
        tabContents.push(<Card key="diffs">
        <BlockStack gap="200">
          {evidence.diffs.map((d, i) => (<BlockStack key={i} gap="100">
    ...
    if (Array.isArray(evidence.samples) && evidence.samples.length > 0) {
        ...
        tabContents.push(<Card key="samples">
        <BlockStack gap="200">
          {evidence.samples.map((s, i) => (<BlockStack key={i} gap="100">
              <Text as="p" variant="bodySm" fontWeight="semibold">
                {s.label}
              </Text>
              <pre style={{
    ...
    if (Array.isArray(evidence.queries) && evidence.queries.length > 0) {
        ...
        tabContents.push(<Card key="queries">
        <BlockStack gap="200">
          {evidence.queries.map((q, i) => (<BlockStack key={i} gap="100">
              <Text as="p" variant="bodySm" fontWeight="semibold">
                {q.label}
              </Text>
              <Text as="p" variant="bodySm" tone="subdued">
    ...
    if (Array.isArray(evidence.screenshots) && evidence.screenshots.length > 0) {
        ...
        tabContents.push(<Card key="screenshots">
        <BlockStack gap="200">
          {evidence.screenshots.map((s, i) => (<BlockStack key={i} gap="100">
              <Text as="p" variant="bodySm" fontWeight="semibold">
                {s.label}
              </Text>
...
//# sourceMappingURL=ApprovalEvidenceSection.js.map