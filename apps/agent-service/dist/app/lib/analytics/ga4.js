...
export async function getRevenueMetrics() {
    ...
    try {
        ...
        const aovChange = calculatePercentageChange(currentData.aov, previousData.aov);
        const transactionsChange = calculatePercentageChange(currentData.transactions, previousData.transactions);
 ...
 */
async function fetchRevenueData(client, startDate, endDate) {
...
export async function getTrafficMetrics() {
    ...
    try {
        ...
        // Calculate trends
        const sessionsChange = calculatePercentageChange(currentData.totalSessions, previousData.totalSessions);
        const organicChange = calculatePercentageChange(currentData.organicSessions, previousData.organicSessions);
 ...
 */
async function fetchTrafficData(client, startDate, endDate) {
    const { BetaAnalyticsDataClient } = await import("@google-analytics/data");
    const gaClient = new BetaAnalyticsDataClient();
    const config = getGaConfig();
    const [response] = await gaClient.runReport({
...
export async function getConversionMetrics() {
    ...
    try {
        ...
        // Calculate trend
        const conversionRateChange = calculatePercentageChange(currentData.conversionRate, previousData.conversionRate);
        const duration = Date.now() - startTime;
 ...
 */
async function fetchConversionData(client, startDate, endDate) {
    const { BetaAnalyticsDataClient } = await import("@google-analytics/data");
    const gaClient = new BetaAnalyticsDataClient();
    const config = getGaConfig();
    const [response] = await gaClient.runReport({
...
export async function getTrafficBreakdown() {
    ...
    try {
        ...
        const channels = currentData.map((current) => {
 ...
 */
async function fetchTrafficBreakdownData(client, startDate, endDate) {
    const { BetaAnalyticsDataClient } = await import("@google-analytics/data");
    const gaClient = new BetaAnalyticsDataClient();
    const config = getGaConfig();
    const [response] = await gaClient.runReport({
