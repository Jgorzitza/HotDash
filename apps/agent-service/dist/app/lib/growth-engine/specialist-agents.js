...
- Include rollback plans for all actions`,
    ...
    async runDailyAnalysis() {
        ...
        const mockAction = createActionItem('seo_optimization', '/products/powder-board-xl', 'Optimize page title and meta description for "powder board" query to improve CTR from 3.75% to 5.5%', {
            ...
            delta: 2500,
            unit: '$'
        }, 0.8, 'medium', 'perf', true, 'Revert page changes to previous version', 'GSC 48-72h lag', 'analytics-agent');
...
- Include vendor selection and lead time considerations`,
    ...
    async runHourlyAnalysis() {
        ...
        // 5. Emit actions to Action Queue
        const mockAction = createActionItem('inventory_reorder', 'BOARD-XL', 'Reorder 24 units of Powder Board XL - current stock: 6 units, velocity: 2.4/day, ROP: 10 units', {
            ...
            delta: -0.8,
            unit: 'probability'
        }, 0.9, 'simple', 'safety', true, 'Cancel PO if demand changes', 'Real-time', 'inventory-agent');
        actions.push(mockAction);
        return actions;
    }
}
...
- Include technical implementation details`,
    ...
    async runDailyAnalysis() {
        ...
        // 5. Emit actions to Action Queue
        const mockAction = createActionItem('performance_optimization', '/products/powder-board-xl', 'Optimize LCP (Largest Contentful Paint) from 4.2s to 2.5s by compressing hero image and optimizing loading', {
            ...
            delta: 0.15,
            unit: 'percentage'
        }, 0.7, 'hard', 'perf', true, 'Revert image optimizations and restore original', 'Real-time', 'content-seo-perf-agent');
        actions.push(mockAction);
        return actions;
    }
}
...
- Include escalation procedures for high-risk items`,
    ...
    async runContinuousMonitoring() {
        ...
        // 5. Emit actions to Action Queue
        const mockAction = createActionItem('fraud_alert', 'ORDER-12345', 'Investigate potential fraud - order value $500, new customer, multiple failed payment attempts', {
            ...
            delta: 0.8,
            unit: 'probability'
        }, 0.9, 'simple', 'policy', true, 'Release order if investigation clears customer', 'Real-time', 'risk-agent');
        actions.push(mockAction);
        return actions;
    }
}
...
//# sourceMappingURL=specialist-agents.js.map