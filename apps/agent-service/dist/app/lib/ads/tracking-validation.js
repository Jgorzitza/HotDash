 ...
 */
export function validateUTMParameters(params) {
    const errors = [];
    const warnings = [];
    // Required parameters
 ...
 */
export function buildURLWithUTM(baseUrl, params) {
    try {
 ...
 * @returns Validation result
 */
export function validateConversionEvent(event) {
    const errors = [];
    const warnings = [];
    if (!event.eventId) {
 ...
 * @returns Validation result
 */
export function validateTrackingPixel(pixelData) {
    const errors = [];
    const warnings = [];
 ...
 */
export function validateROASCalculation(revenue, spend, expectedROAS, tolerance = 10) {
    const errors = [];
    const warnings = [];
    if (revenue < 0) {
    ...
    if (spend === 0) {
        ...
        if (expectedROAS !== undefined) {
            ...
            if (percentDifference > tolerance) {
                warnings.push(`Calculated ROAS (${calculatedROAS.toFixed(2)}) differs from expected (${expectedROAS.toFixed(2)}) by ${percentDifference.toFixed(1)}%`);
            }
        }
 ...
 * @returns Validation result
 */
export function testUTMTracking(testURL, expectedParams) {
    const errors = [];
    const warnings = [];
    // Parse UTM from URL
...
//# sourceMappingURL=tracking-validation.js.map