...
export class SEOOptimizer {
    ...
    generateMetaTags(pageData) {
        const { title, description, keywords, canonicalUrl, image, type = 'website', publishedTime, modifiedTime, author, section, tags, noindex, nofollow } = pageData;
    ...
    generateJSONLD(structuredData) {
        return structuredData.map(data => `<script type="application/ld+json">${JSON.stringify(data, null, 2)}</script>`).join('\n      ');
    }
    /**
     * SEO audit function to check page optimization
     */
    auditPageSEO(pageData, htmlContent) {
        const issues = [];
        ...
        if (pageData.keywords && pageData.keywords.length > 0) {
            ...
            const hasKeywordInTitle = pageData.keywords.some(keyword => titleLower.includes(keyword.toLowerCase()));
        ...
        if (pageData.keywords && pageData.keywords.length > 0) {
            ...
            const hasKeywordInDesc = pageData.keywords.some(keyword => descLower.includes(keyword.toLowerCase()));
            if (!hasKeywordInDesc) {
...
//# sourceMappingURL=seo-optimization.js.map