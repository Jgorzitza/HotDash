...
export function getSecurityHeaders(config = {}) {
    const { enableHSTS = true, enableCSP = true, enableFrameProtection = true, environment = process.env.NODE_ENV === 'production' ? 'production' : 'development', } = config;
 ...
 */
export function mergeSecurityHeaders(existingHeaders, config = {}) {
    const securityHeaders = getSecurityHeaders(config);
 ...
 */
export function getSecureCookieOptions(environment = 'production') {
    return {
        httpOnly: true,
