...
export default function InventoryAlertsPage() {
    ...
    const getAlertTypeBadge = (alertType) => {
        ...
        const config = typeMap[alertType] || { label: alertType, tone: 'info' };
    ...
    const handleCreatePO = async (ropId) => {
        // TODO: Navigate to PO creation or trigger PO automation
        console.log('Create PO for ROP:', ropId);
    };
    return (<Page title="Inventory Alerts" subtitle="Real-time inventory monitoring and reorder notifications" primaryAction={{
      ...
      <BlockStack gap="400">
        ...
        {/* Active Alerts Table */}
        {data.alertSummary.total > 0 && (<Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">Active Alerts</Text>
              <DataTable columnContentTypes={['text', 'text', 'text', 'text', 'numeric', 'numeric', 'text', 'text']} headings={['Severity', 'Type', 'SKU', 'Product', 'Current', 'Threshold', 'Created', 'Action']} rows={alertRows}/>
            </BlockStack>
          </Card>)}

        {/* ROP Suggestions Table */}
        {data.ropSummary.total > 0 && (<Card>
            <BlockStack gap="400">
              <Text as="h2" variant="headingMd">Reorder Point Notifications</Text>
              <DataTable columnContentTypes={['text', 'text', 'numeric', 'numeric', 'numeric', 'text', 'text', 'text']} headings={['SKU', 'Product', 'Current', 'ROP', 'Recommended Qty', 'Days to Stockout', 'Created', 'Action']} rows={ropRows}/>
            </BlockStack>
          </Card>)}

        {/* Empty State */}
        {data.alertSummary.total === 0 && data.ropSummary.total === 0 && (<Banner tone="success">
...
//# sourceMappingURL=inventory.alerts.js.map