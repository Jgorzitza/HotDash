...
import { Page, Layout, Card, Text, Badge, InlineStack, BlockStack, Box, Banner, DataTable, ProgressBar, } from '@shopify/polaris';
...
export async function loader({ request }) {
    ...
    const dashboard = generateMonitoringDashboard(mockCampaigns, mockPerformances, DEFAULT_MONITORING_CONFIG);
...
export default function AdsDashboard() {
    ...
    return (<Page title="Ad Campaign Monitoring" subtitle="Real-time performance tracking and optimization">
      <Layout>
        {/* Summary Cards */}
        <Layout.Section>
        ...
        {/* Recent Alerts */}
        {dashboard.recentAlerts.length > 0 && (<Layout.Section>
            <Card>
              <BlockStack gap="400">
                <Text variant="headingMd" as="h2">
                  Recent Alerts
                </Text>
                {dashboard.recentAlerts.map((alert, index) => (<Banner key={index} tone={alert.severity === 'high' ? 'critical' : 'warning'}>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <DataTable columnContentTypes={['text', 'text', 'numeric', 'numeric', 'numeric', 'numeric', 'text']} headings={['Campaign', 'Status', 'ROAS', 'Spend', 'Revenue', 'Health', 'Alerts']} rows={dashboard.campaigns.map(campaign => {
            const healthScore = getCampaignHealthScore(campaign);
            return [
                campaign.campaignName,
                <Badge key="status" tone={campaign.status === 'healthy' ? 'success' :
                ...
                <Box key="health">
                      ...
                      <ProgressBar progress={healthScore} tone={healthScore >= 80 ? 'success' : healthScore >= 60 ? 'primary' : 'critical'} size="small"/>
                    </Box>,
                campaign.alerts.length > 0 ? (<Badge key="alerts" tone="warning">
                        {campaign.alerts.length} alert{campaign.alerts.length > 1 ? 's' : ''}
                      </Badge>) : (<Text key="alerts" variant="bodySm" as="span" tone="subdued">
                        None
                      </Text>),
            ];
        })}/>
        ...
        {/* Top Recommendations */}
        {dashboard.topRecommendations.length > 0 && (<Layout.Section>
            <Card>
              <BlockStack gap="400">
                <Text variant="headingMd" as="h2">
                  Top Optimization Recommendations
                </Text>
                {dashboard.topRecommendations.slice(0, 5).map((rec, index) => (<Card key={index}>
                    <BlockStack gap="200">
...
//# sourceMappingURL=ads.dashboard.js.map