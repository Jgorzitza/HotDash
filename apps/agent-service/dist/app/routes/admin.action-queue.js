...
export default function ActionQueueDashboard() {
    const { actions, total, error } = useLoaderData();
    ...
    return (<s-page heading="Action Queue" subtitle={`${total} recommended actions`}>
      <s-button slot="secondary-actions" onClick={() => revalidator.revalidate()}>
        Refresh
      </s-button>

      {error && (<s-banner tone="critical">
          <s-text>Error loading actions: {error}</s-text>
        </s-banner>)}

      {actions.length === 0 ? (<EmptyActionQueueState />) : (<>
          <ActionQueueTable actions={actions} onViewDetails={handleViewDetails} onApprove={handleApprove} onDismiss={handleDismiss}/>
          
          {selectedAction && (<ActionDetailsModal action={selectedAction} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onApprove={() => handleApprove(selectedAction.id)} onDismiss={() => handleDismiss(selectedAction.id)}/>)}
        </>)}
    </s-page>);
}
/**
...
function EmptyActionQueueState() {
    return (<s-section accessibilityLabel="Empty state section">
      <s-grid gap="base" justifyItems="center" paddingBlock="large-400">
        <s-box maxInlineSize="200px" maxBlockSize="200px">
          <s-image aspectRatio="1/0.5" src="https://cdn.shopify.com/static/images/polaris/patterns/callout.png" alt="No actions available"/>
        </s-box>
...
function ActionQueueTable({ actions, onViewDetails, onApprove, onDismiss }) {
    return (<s-section padding="none" accessibilityLabel="Action queue table">
      <s-table>
        <s-table-header-row>
        ...
        <s-table-body>
          {actions.map((action, index) => (<ActionQueueRow key={action.id} action={action} rank={index + 1} onViewDetails={onViewDetails} onApprove={onApprove} onDismiss={onDismiss}/>))}
        </s-table-body>
      </s-table>
    </s-section>);
}
function ActionQueueRow({ action, rank, onViewDetails, onApprove, onDismiss }) {
...
function ActionDetailsModal({ action, isOpen, onClose, onApprove, onDismiss }) {
    if (!isOpen)
        return null;
    return (<s-modal open={isOpen} onClose={onClose} variant="large">
      <s-stack direction="block" gap="base">
        ...
        <s-stack direction="block" gap="small">
          ...
          <s-text>
            ...
            : `${action.expected_impact.delta}${action.expected_impact.unit}`} in {action.expected_impact.metric}
          </s-text>
        </s-stack>
...
//# sourceMappingURL=admin.action-queue.js.map