...
export async function action({ request }) {
    try {
        ...
        const { imageUrl, imageId, limit = 10, minSimilarity = 0.7, project = "occ" } = body;
        // Validate input
        if (!imageUrl && !imageId) {
            return Response.json({ error: "Missing imageUrl or imageId in request body" }, { status: 400 });
        }
        await logDecision({
            scope: "build",
            actor: "inventory",
        ...
        if (imageId) {
            ...
            if (embeddingError || !embeddingData) {
                return Response.json({ error: `Image not found or not processed: ${embeddingError?.message}` }, { status: 404 });
            }
            // Parse embedding from pgvector format
            const embeddingStr = embeddingData.embedding;
            searchEmbedding = JSON.parse(embeddingStr.replace(/[\[\]]/g, '').split(',').map(n => n.trim()).join(','));
            const photo = embeddingData.customer_photos;
        ...
        // Case 2: Search by new image URL
            ...
            return Response.json({ error: "Invalid request: must provide imageUrl or imageId" }, { status: 400 });
        }
        ...
        const { data: searchResults, error: searchError } = await supabase.rpc("search_inventory_images", {
...
export async function loader() {
    return Response.json({ error: "Method not allowed. Use POST to search by image." }, { status: 405 });
}
//# sourceMappingURL=api.inventory.search-by-image.js.map