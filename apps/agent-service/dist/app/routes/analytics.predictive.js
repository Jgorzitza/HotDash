...
export async function loader({ request }) {
    try {
        ...
        return Response.json({ insights, error: null });
    }
    catch (error) {
...
export default function PredictiveAnalyticsDashboard() {
    const { insights, error } = useLoaderData();
    ...
    return (<div style={{
      ...
      <div style={{
        ...
        <SummaryCard title="Total Customers" value={insights.customerBehavior.totalCustomers} subtitle="Analyzed"/>
        <SummaryCard title="At Risk" value={insights.customerBehavior.atRiskCount} subtitle="Customers" variant="warning"/>
        <SummaryCard title="High Value" value={insights.customerBehavior.highValueCount} subtitle="Customers" variant="success"/>
        <SummaryCard title="Recommendations" value={insights.actionRecommendations.length} subtitle="Actions"/>
      </div>
      ...
      <div style={{ marginBottom: 'var(--occ-space-4)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--occ-space-2)' }}>
          {insights.actionRecommendations.map((rec, idx) => (<ActionCard key={idx} recommendation={rec}/>))}
        </div>
      </div>
...
function ForecastCard({ title, forecast }) {
    return (<div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--occ-space-2)' }}>
        <MetricRow label="Revenue" value={`$${forecast.predictions.revenue.currentValue.toFixed(2)}`} trend={forecast.predictions.revenue.trend}/>
        <MetricRow label="Conversions" value={forecast.predictions.conversions.currentValue.toFixed(0)} trend={forecast.predictions.conversions.trend}/>
        <MetricRow label="Confidence" value={`${(forecast.overallConfidence * 100).toFixed(0)}%`} trend="stable"/>
      </div>
    </div>);
}
...
//# sourceMappingURL=analytics.predictive.js.map