...
import { getActionAttribution, updateActionROI, } from "~/services/analytics/action-attribution";
// ============================================================================
...
export async function loader({ params }) {
    ...
    try {
        ...
        if (!action.actionKey) {
            return Response.json({ error: "Action has no tracking key (actionKey)" }, { status: 400 });
        }
...
export async function action({ params }) {
    ...
    try {
        ...
        const result = await updateActionROI(actionRecord.id, actionRecord.actionKey);
        return Response.json({
            success: true,
...
//# sourceMappingURL=api.actions.$id.attribution.js.map