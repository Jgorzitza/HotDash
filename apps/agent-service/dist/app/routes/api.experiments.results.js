...
export async function loader({ params }) {
    try {
        ...
        if (!experimentId) {
            return Response.json({ error: "Missing experimentId parameter" }, { status: 400 });
        }
        // Get experiment configuration
        const experiment = abTestingService.getExperiment(experimentId);
        if (!experiment) {
            return Response.json({ error: `Experiment not found: ${experimentId}` }, { status: 404 });
        }
        // Calculate statistical significance
        const significance = await abTestingService.calculateSignificance(experimentId);
        ...
        const results = {
            ...
            conversionRates: Object.fromEntries(Object.entries(significance.conversionRates).map(([variant, rate]) => [
                variant,
                (rate * 100).toFixed(2) + "%",
            ])),
...
//# sourceMappingURL=api.experiments.results.js.map