...
import { Page, Layout, Card, Text, Badge, InlineStack, BlockStack, DataTable, Banner, } from "@shopify/polaris";
...
export default function LlamaIndexMCPDashboard() {
    const { metrics, error, serverRunning } = useLoaderData();
    ...
    if (error) {
        return (<Page title="LlamaIndex MCP Monitoring">
        <Layout>
          <Layout.Section>
    ...
    return (<Page title="LlamaIndex MCP Monitoring" subtitle={`Server uptime: ${metrics.uptime} • Last updated: ${new Date(metrics.timestamp).toLocaleTimeString()}`}>
      <Layout>
        {/* Status Banner */}
        <Layout.Section>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <DataTable columnContentTypes={['text', 'numeric', 'numeric', 'text', 'numeric', 'numeric']} headings={['Tool', 'Calls', 'Errors', 'Error Rate', 'Avg Latency', 'P95 Latency']} rows={tableRows}/>
            </BlockStack>
          </Card>
        </Layout.Section>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <InlineStack gap="200">
                {metrics.tools.map(tool => (<Badge key={tool}>{tool}</Badge>))}
              </InlineStack>
            </BlockStack>
          </Card>
        </Layout.Section>
...
//# sourceMappingURL=admin.analytics.llamaindex-mcp.js.map