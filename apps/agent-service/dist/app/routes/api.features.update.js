...
export async function action({ request }) {
    ...
    try {
        ...
        if (!flagId || !updateAction) {
            return Response.json({ error: "Missing required fields: flagId, action" }, { status: 400 });
        }
        ...
        switch (updateAction) {
            ...
            case "updateRollout":
                if (typeof value !== "number") {
                    return Response.json({ error: "Rollout percentage must be a number (0-100)" }, { status: 400 });
                }
                await featureFlagService.updateRolloutPercentage(flagId, value);
                break;
            case "addTargetUser":
                if (typeof value !== "string") {
                    return Response.json({ error: "Target user must be a string" }, { status: 400 });
                }
            ...
            case "removeTargetUser":
                ...
                break;
            default:
                return Response.json({ error: `Invalid action: ${updateAction}` }, { status: 400 });
        }
        // Return updated flag
...
//# sourceMappingURL=api.features.update.js.map