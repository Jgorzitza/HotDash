...
export async function loader({ params, request }) {
    try {
        ...
        if (!flagId || !userId) {
            return Response.json({ error: "Missing required parameters: flagId, userId" }, { status: 400 });
        }
        const check = await featureFlagService.checkFeature(flagId, userId, userSegment);
...
export async function action({ request }) {
    ...
    try {
        ...
        if (!flagId || !userId) {
            return Response.json({ error: "Missing required fields: flagId, userId" }, { status: 400 });
        }
        const check = await featureFlagService.checkFeature(flagId, userId, userSegment);
        return Response.json({
            success: true,
...
//# sourceMappingURL=api.features.check.js.map