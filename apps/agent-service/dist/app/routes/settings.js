...
import { BlockStack, Checkbox, Toast } from "@shopify/polaris";
...
export async function action({ request }) {
    ...
    try {
        if (action === "save-preferences") {
            const visibleTiles = JSON.parse(formData.get("visible_tiles") || "[]");
            const theme = formData.get("theme");
            const defaultView = formData.get("default_view");
            const autoRefresh = formData.get("auto_refresh") === "true";
            const refreshInterval = parseInt(formData.get("refresh_interval") || "60");
...
export default function SettingsPage() {
    ...
    const [visibleTiles, setVisibleTiles] = useState(data.preferences.visibleTiles);
    ...
    const handleTileVisibilityChange = async (tileId, checked) => {
        let newVisibleTiles;
        if (checked) {
            // Add tile
    ...
    return (<s-page heading="Settings" backAction={{ content: "Dashboard", url: "/" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: "var(--occ-space-2)" }} role="tablist">
          {tabs.map((tab, index) => (<button key={tab.id} role="tab" aria-selected={selectedTab === index} aria-label={tab.label} onClick={() => setSelectedTab(index)} style={{
      ...
      {/* Dashboard Tab */}
      {selectedTab === 0 && (<div id="dashboard-content" style={{ padding: "var(--occ-space-5)" }}>
          <div style={{ marginBottom: "var(--occ-space-6)" }}>
            ...
            <BlockStack gap="300">
              {[
                ...
                return (<Checkbox key={tile.id} label={tile.label} checked={isChecked} disabled={isDisabled} helpText={isDisabled ? "Minimum 2 tiles required" : undefined} onChange={(checked) => handleTileVisibilityChange(tile.id, checked)}/>);
            })}
            </BlockStack>
          </div>
        </div>)}
      ...
      {/* Appearance Tab */}
      {selectedTab === 1 && (<div id="appearance-content" style={{ padding: "var(--occ-space-5)" }}>
          <div style={{ marginBottom: "var(--occ-space-6)" }}>
            ...
            <BlockStack gap="300">
              {[
                { value: "light", label: "Light", helpText: "Always use light theme" },
                { value: "dark", label: "Dark", helpText: "Always use dark theme" },
                { value: "auto", label: "Auto", helpText: "Match system preference" },
            ].map((themeOption) => (<label key={themeOption.value} style={{
                    display: "flex",
                    alignItems: "flex-start",
                    gap: "var(--occ-space-2)",
                    cursor: "pointer",
                }}>
                  <input type="radio" name="theme" value={themeOption.value} checked={theme === themeOption.value} onChange={() => handleThemeChange(themeOption.value)} style={{ marginTop: "4px" }}/>
                  <div>
          ...
          <div>
            ...
            ].map((view) => (<label key={view.value} style={{
                    display: "flex",
                    alignItems: "center",
                    gap: "var(--occ-space-2)",
                }}>
                  <input type="radio" name="default-view" value={view.value} defaultChecked={data.preferences.defaultView === view.value}/>
      ...
      {/* Notifications Tab */}
      {selectedTab === 2 && (<div id="notifications-content" style={{ padding: "var(--occ-space-5)" }}>
          <h2 style={{ marginBottom: "var(--occ-space-4)" }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              {browserNotifications.permission === "default" && (<button onClick={browserNotifications.requestPermission} style={{
              ...
              {browserNotifications.permission === "denied" && (<p style={{
                    margin: 0,
            ...
            <div style={{
              ...
              <label style={{
                ...
                <input type="checkbox" defaultChecked name="desktop-notifications" disabled={browserNotifications.permission !== "granted"}/>
              ...
              <label style={{
                ...
                <input type="checkbox" defaultChecked name="sound-enabled" onChange={(e) => {
      ...
      {/* Account Tab */}
      {selectedTab === 3 && (<div id="account-content" style={{ padding: "var(--occ-space-5)" }}>
          <h2 style={{ marginBottom: "var(--occ-space-4)" }}>
            Account Information
          </h2>
      ...
      <div style={{
        ...
        <button onClick={handleSavePreferences} disabled={isSubmitting} style={{
            padding: "var(--occ-space-3) var(--occ-space-4)",
