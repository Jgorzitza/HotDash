...
import { TileCard, SalesPulseTile, FulfillmentHealthTile, InventoryHeatmapTile, CXEscalationsTile, SEOContentTile, OpsMetricsTile, IdeaPoolTile, ApprovalsQueueTile, CEOAgentTile, UnreadMessagesTile, 
// Phase 7-8: Growth analytics tiles (ENG-023 to ENG-026)
SocialPerformanceTile, SEOImpactTile, AdsROASTile, GrowthMetricsTile, } from "../components/tiles";
...
import { getOpsAggregateMetrics, } from "../services/metrics/aggregate";
...
export const loader = async ({ request, }) => {
    ...
    const [ideaPool, approvalsQueue, ceoAgent, unreadMessages, socialPerformance, seoImpact, adsRoas, growthMetrics, growthEngineAnalytics,] = await Promise.all([
...
async function resolveTile(resolver) {
    try {
...
async function resolveEscalations(shopDomain) {
    try {
...
async function resolveApiTile(apiPath) {
    try {
        const t0 = performance.now();
...
async function resolveApprovalsQueue() {
    try {
        const { getApprovalCounts, getPendingApprovals } = await import("~/services/approvals");
...
function buildMockDashboard() {
    ...
    const breachThirtyMinutesAfter = new Date(new Date(threeHoursAgo).getTime() + 30 * 60 * 1000).toISOString();
    const salesSummary = {
    ...
    const fact = (id) => ({ id, createdAt: now });
    // Phase 3 mock data
    const ideaPoolData = {
        ideas: [
            {
...
export default function OperatorDashboard() {
    ...
    const visibleTileIds = tileOrder.filter((tileId) => data.visibleTiles.includes(tileId));
    // Track refreshing tiles (Phase 5 - ENG-025)
    const [refreshingTiles, setRefreshingTiles] = useState(new Set());
    // Handle tile refresh events from SSE
    useEffect(() => {
        if (lastMessage?.type === "tile-refresh") {
            const tileId = lastMessage.data.tileId;
            if (tileId) {
    ...
    const handleRefreshTile = useCallback((tileId) => {
        setRefreshingTiles((prev) => new Set([...prev, tileId]));
    ...
    const handleDragEnd = useCallback((event) => {
        ...
        if (over && active.id !== over.id) {
            setTileOrder((items) => {
                ...
                tileOrderFetcher.submit({ tileOrder: JSON.stringify(newOrder) }, { method: "POST", action: "/api/preferences/tile-order" });
                return newOrder;
            });
        }
    }, [tileOrderFetcher]);
    ...
    const systemStatus = {
        ...
        connectionStatus: sseStatus === "connected"
            ? "online"
    ...
    const tileMap = {
        "ops-metrics": (<TileCard title="Ops Pulse" tile={data.opsMetrics} render={(metrics) => <OpsMetricsTile metrics={metrics}/>} testId="tile-ops-metrics" showRefreshIndicator isRefreshing={refreshingTiles.has("ops-metrics")} onRefresh={() => handleRefreshTile("ops-metrics")} autoRefreshInterval={300}/>),
        "sales-pulse": (<TileCard title="Sales Pulse" tile={data.sales} render={(summary) => <SalesPulseTile summary={summary} enableModal/>} testId="tile-sales-pulse" showRefreshIndicator isRefreshing={refreshingTiles.has("sales-pulse")} onRefresh={() => handleRefreshTile("sales-pulse")} autoRefreshInterval={60}/>),
        fulfillment: (<TileCard title="Fulfillment Health" tile={data.fulfillment} render={(issues) => <FulfillmentHealthTile issues={issues}/>} testId="tile-fulfillment-health" showRefreshIndicator isRefreshing={refreshingTiles.has("fulfillment")} onRefresh={() => handleRefreshTile("fulfillment")} autoRefreshInterval={120}/>),
        inventory: (<TileCard title="Inventory Heatmap" tile={data.inventory} render={(alerts) => <InventoryHeatmapTile alerts={alerts}/>} testId="tile-inventory-heatmap" showRefreshIndicator isRefreshing={refreshingTiles.has("inventory")} onRefresh={() => handleRefreshTile("inventory")} autoRefreshInterval={300}/>),
        "cx-escalations": (<TileCard title="CX Escalations" tile={data.escalations} render={(conversations) => (<CXEscalationsTile conversations={conversations} enableModal/>)} testId="tile-cx-escalations" showRefreshIndicator isRefreshing={refreshingTiles.has("cx-escalations")} onRefresh={() => handleRefreshTile("cx-escalations")} autoRefreshInterval={30}/>),
        "seo-content": (<TileCard title="SEO & Content Watch" tile={data.seo} render={(anomalies) => <SEOContentTile anomalies={anomalies}/>} testId="tile-seo-content" showRefreshIndicator isRefreshing={refreshingTiles.has("seo-content")} onRefresh={() => handleRefreshTile("seo-content")} autoRefreshInterval={600}/>),
        "idea-pool": (<TileCard title="Idea Pool" tile={data.ideaPool} render={(ideaPool) => <IdeaPoolTile ideaPool={ideaPool}/>} testId="tile-idea-pool" showRefreshIndicator isRefreshing={refreshingTiles.has("idea-pool")} onRefresh={() => handleRefreshTile("idea-pool")} autoRefreshInterval={300}/>),
        "approvals-queue": (<TileCard title="Approvals Queue" tile={data.approvalsQueue} render={(approvalsData) => <ApprovalsQueueTile {...approvalsData}/>} testId="tile-approvals-queue" showRefreshIndicator isRefreshing={refreshingTiles.has("approvals-queue")} onRefresh={() => handleRefreshTile("approvals-queue")} autoRefreshInterval={60}/>),
        "ceo-agent": (<TileCard title="CEO Agent" tile={data.ceoAgent} render={(stats) => <CEOAgentTile stats={stats}/>} testId="tile-ceo-agent" showRefreshIndicator isRefreshing={refreshingTiles.has("ceo-agent")} onRefresh={() => handleRefreshTile("ceo-agent")} autoRefreshInterval={120}/>),
        "unread-messages": (<TileCard title="Unread Messages" tile={data.unreadMessages} render={(unread) => <UnreadMessagesTile unread={unread}/>} testId="tile-unread-messages" showRefreshIndicator isRefreshing={refreshingTiles.has("unread-messages")} onRefresh={() => handleRefreshTile("unread-messages")} autoRefreshInterval={60}/>),
        // Phase 7-8: Growth analytics tiles (ENG-023 to ENG-026)
        "social-performance": (<TileCard title="Social Performance" tile={data.socialPerformance} render={(socialData) => <SocialPerformanceTile data={socialData}/>} testId="tile-social-performance" showRefreshIndicator isRefreshing={refreshingTiles.has("social-performance")} onRefresh={() => handleRefreshTile("social-performance")} autoRefreshInterval={300}/>),
        "seo-impact": (<TileCard title="SEO Impact" tile={data.seoImpact} render={(seoData) => <SEOImpactTile data={seoData}/>} testId="tile-seo-impact" showRefreshIndicator isRefreshing={refreshingTiles.has("seo-impact")} onRefresh={() => handleRefreshTile("seo-impact")} autoRefreshInterval={600}/>),
        "ads-roas": (<TileCard title="Ads ROAS" tile={data.adsRoas} render={(adsData) => <AdsROASTile data={adsData}/>} testId="tile-ads-roas" showRefreshIndicator isRefreshing={refreshingTiles.has("ads-roas")} onRefresh={() => handleRefreshTile("ads-roas")} autoRefreshInterval={300}/>),
        "growth-metrics": (<TileCard title="Growth Metrics" tile={data.growthMetrics} render={(growthData) => <GrowthMetricsTile data={growthData}/>} testId="tile-growth-metrics" showRefreshIndicator isRefreshing={refreshingTiles.has("growth-metrics")} onRefresh={() => handleRefreshTile("growth-metrics")} autoRefreshInterval={600}/>),
        // Phase 9-12: Advanced Growth Engine analytics (ENG-024)
        "growth-engine-analytics": (<TileCard title="Growth Engine Analytics" tile={data.growthEngineAnalytics} render={(analyticsData) => (<GrowthEngineAnalyticsTile analytics={analyticsData.analytics} timeframe={analyticsData.timeframe} period={analyticsData.period} generatedAt={analyticsData.generatedAt}/>)} testId="tile-growth-engine-analytics" showRefreshIndicator isRefreshing={refreshingTiles.has("growth-engine-analytics")} onRefresh={() => handleRefreshTile("growth-engine-analytics")} autoRefreshInterval={300}/>),
    };
    return (<s-page heading="Operator Control Center">
      {/* Banner Alerts (Phase 4 - ENG-012) */}
      {bannerAlerts.length > 0 && <BannerAlerts alerts={bannerAlerts}/>}
      
      {/* Connection Status Indicator (ENG-020) */}
      <ConnectionStatusIndicator status={sseStatus} quality={connectionQuality} lastMessage={lastMessage}/>

      {data.mode === "mock" && (<div style={{
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
        <SortableContext items={visibleTileIds} strategy={verticalListSortingStrategy}>
          <div className="occ-tile-grid">
            {visibleTileIds.length > 0 ? (visibleTileIds.map((tileId) => (<SortableTile key={tileId} id={tileId}>
                  {tileMap[tileId]}
                </SortableTile>))) : (<div style={{
...
//# sourceMappingURL=app._index.js.map