...
export async function loader({ params }) {
    const { id } = params;
    if (!id) {
        return ({ valid: false, errors: ["Missing approval ID"] },
            { status: 400 },
        );
    }
    try {
        ...
        if (!approval) {
            return ({ valid: false, errors: ["Approval not found"] },
                { status: 404 },
            );
        }
        const errors = [];
        ...
        const hasDryRunErrors = approval.actions?.some((action) => action.dry_run_status && action.dry_run_status !== "success");
        if (hasDryRunErrors) {
...
//# sourceMappingURL=api.approvals.$id.validate.js.map