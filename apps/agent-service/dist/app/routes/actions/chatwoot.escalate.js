...
export const action = async ({ request }) => {
    ...
    if (isFormData) {
        ...
        selectedReply = formData.get("selectedReply");
        ...
        try {
            switch (actionType) {
                case "approve_send": {
                    ...
                    if (!replyBody) {
                        throw jsonResponse({ error: "Reply text is required for approve_send" }, { status: 400 });
                    }
                ...
                default:
                    throw jsonResponse({ error: `Unknown action type: ${actionType}` }, { status: 400 });
            }
        }
        catch (error) {
            if (error instanceof Response)
                throw error;
            throw jsonResponse({ error: "Failed to execute Chatwoot action" }, {
                status: 502,
    ...
    if (!templateId) {
        throw jsonResponse({ error: "templateId is required for legacy API" }, { status: 400 });
    }
    ...
    try {
        ...
        throw jsonResponse({ error: "Failed to send Chatwoot reply" }, {
            status: 502,
            statusText: error instanceof Error ? error.message : String(error),
        });
    }
...
//# sourceMappingURL=chatwoot.escalate.js.map