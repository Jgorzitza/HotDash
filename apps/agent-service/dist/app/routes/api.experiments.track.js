...
export async function action({ request }) {
    ...
    try {
        ...
        const { experimentId, variantId, userId, eventType, eventName, value, metadata, } = body;
        // Validate inputs
        if (!experimentId || !variantId || !userId || !eventType || !eventName) {
            return Response.json({
                error: "Missing required fields: experimentId, variantId, userId, eventType, eventName",
            }, { status: 400 });
        }
        // Validate event type
        ...
        switch (eventType) {
            case "exposure":
                ...
                break;
            case "conversion":
                await abTestingService.trackConversion(experimentId, variantId, userId, eventName, value);
                break;
            case "engagement":
                await abTestingService.trackEngagement(experimentId, variantId, userId, eventName, metadata);
                break;
        }
        return Response.json({
            success: true,
...
//# sourceMappingURL=api.experiments.track.js.map