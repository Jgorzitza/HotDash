...
export async function action({ request }) {
    ...
    try {
        ...
        if (!body.type || !body.request) {
            return data({ error: 'Type and request are required' }, { status: 400 });
        }
        if (body.type !== 'product_description' && body.type !== 'blog_post') {
            return data({ error: 'Invalid content type' }, { status: 400 });
        }
        // Generate variations
        const variations = await aiContentGenerator.generateVariations(body.type, body.request, body.count || 3);
        return {
            success: true,
            variations,
        };
...
//# sourceMappingURL=api.content.generate-variations.js.map