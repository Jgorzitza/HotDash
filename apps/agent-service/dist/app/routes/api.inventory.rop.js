 ...
 */
import { batchCalculateROP, getROPSuggestions } from "~/services/inventory/rop-engine";
...
export async function loader({ request }) {
    try {
        ...
        const suggestions = await Promise.all(productIds.map(async (productId) => {
...
export async function action({ request }) {
    try {
        ...
        const { productIds, shopDomain = 'hotrodan.myshopify.com', calculationMethod = 'standard', promotionalUplift = 0, seasonalAdjustment = 0, historicalDays = 30 } = body;
        ...
        const results = await batchCalculateROP(productIds, shopDomain, calculationMethod);
        return Response.json({
            success: true,
            data: {
                results,
...
//# sourceMappingURL=api.inventory.rop.js.map