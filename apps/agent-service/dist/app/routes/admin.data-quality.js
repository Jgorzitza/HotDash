...
import { Page, Layout, Card, Text, BlockStack, InlineGrid, Badge, DataTable, Banner, ProgressBar, } from "@shopify/polaris";
import { DataQualityService } from "~/services/data/data-quality.service";
import { authenticate } from "~/shopify.server";
export const loader = async ({ request }) => {
    await authenticate.admin(request);
    // Fetch all data quality metrics
    const [validationResults, qualityMetrics, anomalies, freshnessChecks] = await Promise.all([
...
export default function DataQualityDashboard() {
    const { validationResults, qualityMetrics, anomalies, freshnessChecks, overallScore, timestamp } = useLoaderData();
    ...
    return (<Page title="Data Quality Monitoring" subtitle={`Last updated: ${new Date(timestamp).toLocaleString()}`}>
      <Layout>
        {/* Critical Alerts */}
        {(criticalAnomalies > 0 || staleDataSources > 0 || failedValidations > 0) && (<Layout.Section>
            <Banner title="Data Quality Issues Detected" tone="critical">
              <BlockStack gap="200">
                {criticalAnomalies > 0 && (<Text as="p">
                    {criticalAnomalies} critical anomal{criticalAnomalies === 1 ? 'y' : 'ies'} detected
                  </Text>)}
                {staleDataSources > 0 && (<Text as="p">
                    {staleDataSources} data source{staleDataSources === 1 ? ' is' : 's are'} stale
                  </Text>)}
                {failedValidations > 0 && (<Text as="p">
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <DataTable columnContentTypes={['text', 'text', 'text', 'numeric', 'numeric', 'numeric', 'numeric']} headings={[
            'Data Source',
            'Table',
            'Metric Type',
            'Total Records',
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <DataTable columnContentTypes={['text', 'text', 'text', 'text', 'text']} headings={[
            'Rule Name',
            'Type',
            'Severity',
            'Last Run',
        ...
        {/* Anomalies */}
        {anomalies.length > 0 && (<Layout.Section>
            <Card>
              <BlockStack gap="400">
                <Text as="h2" variant="headingLg">
                  Detected Anomalies
                </Text>
                <DataTable columnContentTypes={['text', 'text', 'text', 'numeric', 'numeric', 'text', 'text']} headings={[
                'Type',
                'Data Source',
                'Table',
                'Expected',
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <DataTable columnContentTypes={['text', 'text', 'text', 'numeric', 'text']} headings={[
            'Check Name',
            'Data Source',
            'Table',
...
//# sourceMappingURL=admin.data-quality.js.map