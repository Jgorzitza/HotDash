 ...
 */
import { applyCXThemeAction, } from "~/services/content/cx-action-applier";
    ...
    try {
        ...
        if (!body.action.metadata?.implementationType) {
            return Response.json({
                success: false,
                error: "action.metadata.implementationType is required",
            }, { status: 400 });
        }
        if (!body.action.draftCopy) {
            return Response.json({
                success: false,
                error: "action.draftCopy is required (no content to apply)",
            }, { status: 400 });
        }
        // Apply the CX theme action
        const result = await applyCXThemeAction(body.action, body.productId, request);
        if (!result.success) {
        ...
        console.log(`[CX Actions API] ✅ Applied ${result.contentType} to product ${result.productId}`);
