...
export async function action({ request }) {
    ...
    if (!validViews.includes(defaultView)) {
        return Response.json({ error: `defaultView must be one of: ${validViews.join(", ")}` }, { status: 400 });
    }
    ...
    //   });
    console.log(`[Preferences] Default view saved for ${session.shop}:`, defaultView);
    return Response.json({ ok: true });
}
//# sourceMappingURL=api.preferences.default-view.js.map