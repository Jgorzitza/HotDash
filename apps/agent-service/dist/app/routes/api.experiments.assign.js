...
export async function action({ request }) {
    ...
    try {
        ...
        if (!userId || !experimentId) {
            return Response.json({ error: "Missing required fields: userId, experimentId" }, { status: 400 });
        }
        ...
        if (experiment.status !== "running") {
            return Response.json({ error: `Experiment is not running: ${experiment.status}` }, { status: 400 });
        }
        // Assign variant (deterministic)
        const assignment = abTestingService.assignVariant(userId, experiment);
        // Track exposure
        await abTestingService.trackExposure(experimentId, assignment.variantId, userId);
...
//# sourceMappingURL=api.experiments.assign.js.map