...
import { Page, Layout, Card, BlockStack, InlineStack, Text, Badge, DataTable, Button, Divider, Banner, ProgressBar, } from "@shopify/polaris";
import { trackRankings, detectRankingAlerts, getActiveAlerts } from "~/services/seo/ranking-tracker";
 ...
 */
function calculateHealthScore(rankings, alerts, anomalies) {
    let score = 100;
...
function formatTimeRemaining(slaDeadline) {
    const now = new Date();
...
export default function SEOMonitoringDashboard() {
    ...
    return (<Page title="SEO Health Monitoring" subtitle="Real-time SEO monitoring with critical issue alerts" backAction={{ content: "Dashboard", url: "/" }}>
      <Layout>
        {/* Health Score */}
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <ProgressBar progress={data.healthScore} tone={getHealthScoreColor(data.healthScore)} size="small"/>
        ...
        {criticalAlerts.length > 0 && (<Layout.Section>
        ...
        {data.alerts.length > 0 && (<Layout.Section>
            <Card>
              <BlockStack gap="400">
                ...
                <Divider />
                <DataTable columnContentTypes={['text', 'text', 'numeric', 'numeric', 'numeric', 'text', 'text']} headings={['Severity', 'Keyword', 'Current', 'Previous', 'Change', 'Time Left', 'Action']} rows={alertRows}/>
              </BlockStack>
            </Card>
          </Layout.Section>)}
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <Divider />
              {data.rankings.length > 0 ? (<DataTable columnContentTypes={['text', 'numeric', 'numeric', 'numeric', 'numeric', 'numeric', 'numeric']} headings={['Keyword', 'Position', 'Previous', 'Change', 'Clicks', 'Impressions', 'CTR']} rows={rankingRows}/>) : (<Text as="p" variant="bodyLg" alignment="center">
...
//# sourceMappingURL=seo.monitoring.js.map