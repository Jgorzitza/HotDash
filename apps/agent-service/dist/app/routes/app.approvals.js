...
export default function ApprovalsRoute() {
    const { approvals, total, counts, error } = useLoaderData();
    ...
    const visible = useMemo(() => approvals.filter((a) => !suppressedIds.has(a.id)), [approvals, suppressedIds]);
    function openDetails(approval) {
    ...
    async function handleRequestChanges(note) {
        if (!selected)
            return;
    ...
    return (<Page title="Approval Queue" subtitle={`${total} ${total === 1 ? "approval" : "approvals"}`}>
      <Layout>
        {/* Active filters */}
        {(stateFilter || kindFilter) && (<Layout.Section>
            <InlineStack gap="200">
        ...
        {visible.length === 0 ? (<Layout.Section>
            <Card>
          ...
          </Layout.Section>) : (visible.map((approval) => (<Layout.Section key={approval.id}>
              <ApprovalCard approval={approval} onDetails={() => openDetails(approval)}/>
            </Layout.Section>)))}

        {/* Pagination controls */}
        {total > 0 && (<Layout.Section>
            <InlineStack gap="200" align="space-between" blockAlign="center">
              ...
              <InlineStack gap="200">
                ...
                <Button disabled={approvals.length < 50} onClick={() => setSearchParams((prev) => {
                const p = new URLSearchParams(prev);
        ...
        {/* Drawer */}
        {selected && (<ApprovalsDrawer open={true} approval={selected} onClose={() => setSelected(null)} onApprove={(grades) => handleApprove(grades)} onReject={(reason) => handleReject(reason)} onRequestChanges={(note) => handleRequestChanges(note)} onApply={() => handleApply()}/>)}
      </Layout>
    </Page>);
}
//# sourceMappingURL=app.approvals.js.map