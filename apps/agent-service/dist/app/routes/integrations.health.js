...
import { Page, Layout, Card, Badge, Text, BlockStack, InlineStack, Banner, ProgressBar, Box, DataTable } from '@shopify/polaris';
...
export async function loader({ request }) {
    try {
        ...
        healthChecks.checks.forEach((check) => {
            ...
            const circuitBreakerOpen = circuitBreakers[check.service];
...
export default function IntegrationHealthDashboard() {
    ...
    const getHealthBadge = (healthy) => {
        return healthy ? (<Badge tone="success">Healthy</Badge>) : (<Badge tone="critical">Unhealthy</Badge>);
    };
    const getCircuitBreakerBadge = (isOpen) => {
        return isOpen ? (<Badge tone="warning">Open</Badge>) : (<Badge tone="success">Closed</Badge>);
    };
    ...
    const calculateHealthScore = (metric) => {
        if (metric.totalRequests === 0)
            return 100;
        const successRate = (metric.successfulRequests / metric.totalRequests) * 100;
        return Math.round(successRate);
    };
    return (<Page title="Integration Health Dashboard" subtitle={`Last updated: ${new Date(data.timestamp).toLocaleString()}`} primaryAction={{
            content: 'Refresh',
      ...
      <Layout>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              {data.error && (<Banner tone="critical">
                  <p>{data.error}</p>
                </Banner>)}
        ...
        {data.healthChecks.checks.map((check) => {
            ...
            const circuitBreakerOpen = data.circuitBreakers[check.service];
            return (<Layout.Section key={check.service} variant="oneThird">
              <Card>
                <BlockStack gap="400">
                  <InlineStack align="space-between">
                  ...
                  <Box>
                    ...
                    <ProgressBar progress={healthScore} tone={healthScore >= 90 ? 'success' : healthScore >= 70 ? 'primary' : 'critical'}/>
                  ...
                  {/* Error Message */}
                  {check.error && (<Banner tone="critical">
                      <p>{check.error}</p>
                    </Banner>)}

                  {/* Last Success/Error */}
                  {metric && (<BlockStack gap="200">
                      {metric.lastSuccess && (<Text as="p" variant="bodySm" tone="subdued">
                          Last success: {new Date(metric.lastSuccess).toLocaleTimeString()}
                        </Text>)}
                      {metric.lastError && (<Text as="p" variant="bodySm" tone="critical">
        ...
        {/* Active Incidents */}
        {data.incidents.length > 0 && (<Layout.Section>
            <Card>
              <BlockStack gap="400">
                <InlineStack align="space-between">
                ...
                <DataTable columnContentTypes={['text', 'text', 'text', 'text', 'text']} headings={['Service', 'Severity', 'Message', 'Time', 'Status']} rows={data.incidents.map(incident => [
                ...
                incident.resolved ? (<Badge tone="success">Resolved</Badge>) : (<Badge tone="attention">Active</Badge>),
            ])}/>
              </BlockStack>
            </Card>
          </Layout.Section>)}
        ...
        {/* Alert Escalation */}
        {data.alerts.length > 0 && (<Layout.Section>
            <Card>
              <BlockStack gap="400">
                <InlineStack align="space-between">
                ...
                <DataTable columnContentTypes={['text', 'text', 'text', 'text', 'text']} headings={['Service', 'Type', 'Message', 'Time', 'Escalated']} rows={data.alerts.map(alert => [
                alert.service.toUpperCase(),
                alert.type.replace(/_/g, ' ').toUpperCase(),
                alert.message,
                new Date(alert.timestamp).toLocaleTimeString(),
                alert.escalated ? (<Badge tone="critical">Yes</Badge>) : (<Badge>No</Badge>),
            ])}/>
...
//# sourceMappingURL=integrations.health.js.map