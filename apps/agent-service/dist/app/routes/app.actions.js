...
import { Page, Card, Text, Button, Badge, InlineStack, Banner } from "@shopify/polaris";
...
export async function action({ request }) {
    ...
    try {
        ...
        const operatorId = formData.get('operatorId') || 'current-user';
        switch (action) {
            case 'approve':
...
export default function ActionsPage() {
    const { actions, stats, filters, error } = useLoaderData();
    ...
    return (<Page title="Action Queue" subtitle="Review and approve actions from the Growth Engine">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        {actions.length === 0 ? (<Card>
          ...
          </Card>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {actions.map((action) => (<Card key={action.id}>
                <div style={{ padding: '20px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    {/* Rollback Plan */}
                    {action.rollback_plan && (<Text as="p" tone="subdued">
                        Rollback: {action.rollback_plan}
                      </Text>)}

                    {/* Actions */}
                    {action.status === 'pending' && (<InlineStack spacing="tight">
                        <Button size="slim" variant="primary" onClick={() => handleAction('approve', action.id)} loading={isSubmitting}>
                          Approve
                        </Button>
                        <Button size="slim" variant="secondary" onClick={() => handleAction('reject', action.id)} loading={isSubmitting}>
                          Reject
                        </Button>
                      </InlineStack>)}

                    {action.status === 'approved' && (<Button size="slim" variant="primary" onClick={() => handleAction('execute', action.id)} loading={isSubmitting}>
                        Execute
                      </Button>)}

                    {/* Execution Result */}
                    {action.status === 'executed' && action.execution_result && (<div style={{
                        padding: '12px',
