...
import { Page, Text, BlockStack, Banner } from "@shopify/polaris";
...
export async function loader({ request }) {
    ...
    try {
        ...
        const { items } = await SchedulingService.getContentForDateRange(firstDay.toISOString(), lastDay.toISOString());
...
export default function ContentCalendarPage() {
    const { calendar, month, year, stats, error } = useLoaderData();
    const actionData = useActionData();
    const submit = useSubmit();
    const handleNavigate = (newMonth, newYear) => {
    ...
    return (<Page title="Content Calendar">
      <BlockStack gap="400">
        ...
        <ContentCalendar calendar={calendar} month={month} year={year} stats={stats} onNavigate={handleNavigate} onSchedule={handleSchedule} onCancel={handleCancel} onReschedule={handleReschedule}/>
      </BlockStack>
    </Page>);
}
