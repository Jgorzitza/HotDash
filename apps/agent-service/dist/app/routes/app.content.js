...
export default function ContentManagementPage() {
    const { entries, contentTypes, stats, filters, error } = useLoaderData();
    ...
    return (<Page title="Content Management System" subtitle="Create, edit, and manage your content">
        ...
        <Card>
          <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {contentTypes.length === 0 ? (<Text as="p" tone="subdued">No content types found. Create your first content type to get started.</Text>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {contentTypes.map((contentType) => (<div key={contentType.id} style={{
                    display: 'flex',
        ...
        <Card>
          <div style={{ padding: '20px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              {entries.length === 0 ? (<Text as="p" tone="subdued">No content entries found. Create your first content entry to get started.</Text>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {entries.map((entry) => (<div key={entry.id} style={{
                    ...
                    borderRadius: '4px'
                      ...
                      <InlineStack spacing="tight">
                        ...
                        {entry.status === 'draft' && (<Button size="slim" variant="secondary" onClick={() => {
                        ...
                        {entry.status === 'published' && (<Button size="slim" variant="tertiary" onClick={() => {
                        const formData = new FormData();
