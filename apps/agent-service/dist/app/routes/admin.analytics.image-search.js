...
import { Page, Layout, Card, Text, Badge, InlineStack, BlockStack, Banner, } from "@shopify/polaris";
import { PrismaClient } from "@prisma/client";
import { logDecision } from "~/services/decisions.server";
const prisma = new PrismaClient();
async function getImageSearchMetrics() {
    const now = new Date();
    ...
    // Get upload metrics
    const totalUploads = await prisma.$queryRaw `
    SELECT COUNT(*) as count FROM customer_photos
  `;
    const todayUploads = await prisma.$queryRaw `
    SELECT COUNT(*) as count FROM customer_photos
    WHERE created_at >= ${todayStart}
  `;
    const weekUploads = await prisma.$queryRaw `
    SELECT COUNT(*) as count FROM customer_photos
    WHERE created_at >= ${weekStart}
  `;
    const monthUploads = await prisma.$queryRaw `
    SELECT COUNT(*) as count FROM customer_photos
...
export default function ImageSearchAnalyticsDashboard() {
    ...
    if (error) {
        return (<Page title="Image Search Analytics">
        <Layout>
          <Layout.Section>
...
//# sourceMappingURL=admin.analytics.image-search.js.map