...
export async function loader({ request }) {
    try {
        ...
        if (!dateRegex.test(startDate) || !dateRegex.test(endDate)) {
            return Response.json({ success: false, error: "Invalid date format. Use YYYY-MM-DD" }, { status: 400 });
        }
        ...
        if (daysDiff > 90) {
            return Response.json({ success: false, error: "Date range cannot exceed 90 days" }, { status: 400 });
        }
        if (start > end) {
            return Response.json({ success: false, error: "startDate must be before endDate" }, { status: 400 });
        }
        ...
        if (!result.success) {
            return Response.json({ success: false, error: result.error || "Failed to get metrics" }, { status: 502 });
        }
        const response = {
            success: true,
...
export async function action() {
    return Response.json({ error: "Method not allowed. Use GET to retrieve metrics." }, { status: 405 });
}
//# sourceMappingURL=api.bing.metrics.js.map