...
import { Page, Layout, Card, Text, BlockStack, InlineStack, Badge, DataTable, TextField, Select, } from "@shopify/polaris";
import { useState } from "react";
import { getKnowledgeBaseStats, checkKnowledgeBaseHealth, listArticles, } from "~/services/knowledge";
/**
...
export default function KnowledgeBasePage() {
    const { stats, health, articles } = useLoaderData();
    ...
    return (<Page title="Knowledge Base" subtitle="AI-powered knowledge base for customer support" primaryAction={{
            content: "Add Article",
            onAction: () => {
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <InlineStack gap="400">
                {Object.entries(stats.articlesByCategory).map(([category, count]) => (<Badge key={category} tone="info">
                    {category}: {count}
                  </Badge>))}
              </InlineStack>
            </BlockStack>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              <InlineStack gap="400">
                <div style={{ flex: 1 }}>
                  <TextField label="Search articles" value={searchQuery} onChange={setSearchQuery} placeholder="Search by question or answer..." autoComplete="off" clearButton onClearButtonClick={() => setSearchQuery("")}/>
                </div>
                <div style={{ width: "200px" }}>
                  <Select label="Category" options={categoryOptions} value={selectedCategory} onChange={setSelectedCategory}/>
                </div>
              </InlineStack>
            </BlockStack>
          </Card>
        ...
        <Layout.Section>
          <Card>
            <BlockStack gap="400">
              ...
              <DataTable columnContentTypes={["text", "text", "text", "numeric", "numeric", "text"]} headings={["Question", "Category", "Tags", "Confidence", "Usage", "Created"]} rows={tableRows} hoverable/>
            </BlockStack>
          </Card>
        </Layout.Section>
      </Layout>
