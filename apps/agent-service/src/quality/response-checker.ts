...
export class ResponseChecker {
  ...
  checkResponse(
    response: string,
    config?: Partial<ResponseCheckConfig>
  ): QualityCheck {
    const fullConfig: ResponseCheckConfig = {
      minLength: 10,
      maxLength: 2000,
   ...
   */
  private isClear(response: string): boolean {
   ...
   */
  private hasActionableSteps(response: string): boolean {
   ...
   */
  private isWellFormatted(response: string): boolean {
   ...
   * Check for placeholder text
   */
  private hasPlaceholders(response: string): boolean {
