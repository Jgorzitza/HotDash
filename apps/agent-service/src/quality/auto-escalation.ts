...
export interface EscalationDecision {
  ...
  alertChannels: string[]; // e.g., ['slack', 'email']
  draftMessage: string; // Internal note for human agent
}
...
export class AutoEscalation {
  ...
  evaluate(params: {
    ...
    let recommendedAgent: 'standard' | 'senior' | 'manager' = 'standard';
   ...
   */
  private hasLegalThreat(message: string): boolean {
    const legalKeywords = [
   ...
   */
  private meetsEscalationThreshold(triggers: EscalationTrigger[]): boolean {
   ...
   */
  private generateEscalationMessage(triggers: EscalationTrigger[], originalMessage: string): string {
    const lines: string[] = [
   ...
   */
  private escalations: { conversationId: number; decision: EscalationDecision; timestamp: Date }[] = [];
