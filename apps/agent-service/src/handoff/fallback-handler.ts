 ...
 */
export const fallbackConditions: FallbackCondition[] = [
  {
    name: 'low_confidence',
  ...
  },
  {
    ...
    check: (ctx, decision) => 
      ctx.sentiment === 'negative' && decision.confidence < 0.7,
    priority: 95,
  ...
  },
  {
    ...
    check: (ctx, decision) => 
      ctx.urgency === 'high' && decision.confidence < 0.8,
    priority: 80,
 ...
 */
export function shouldFallbackToHuman(
  context: ConversationContext,
  decision: HandoffDecision
): { shouldFallback: boolean; reason?: string; priority?: number } {
 ...
 */
export async function executeHumanFallback(
  context: ConversationContext,
  reason: string
): Promise<void> {
  try {
 ...
 */
function getContextSummary(context: ConversationContext): string {
  const parts: string[] = [];
 ...
 */
export async function getFallbackStats(timeRange: string = '7d'): Promise<{
